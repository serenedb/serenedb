...
| `arg_max_nulls_last(arg, val)`                                                                 | Like [`arg_max`](#arg_maxarg-val), but rows whose `arg` expression is `NULL` are kept and sort last rather than being ignored. Takes an optional third argument `n` for the top `n` rows. |
...
| `arg_min_nulls_last(arg, val)`                                                                 | Like [`arg_min`](#arg_minarg-val), but rows whose `arg` expression is `NULL` are kept and sort last rather than being ignored. Takes an optional third argument `n` for the bottom `n` rows. |
