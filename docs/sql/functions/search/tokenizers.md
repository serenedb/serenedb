...
A list carries no token positions, only order, so the two wrappers read every element as one position and treat neighbours as adjacent. A dictionary sees the real positions instead, which differ wherever a stage emits several tokens at one position: the grams of [`generate_ngrams`](#generate_ngrams) are alternatives for the same word, and the expansions of [`expand_solr_synonyms`](#expand_solr_synonyms) are alternatives for the same token. Shingling such a stage therefore belongs in a dictionary: the function `generate_shingles(generate_ngrams(v, 2, 3), 2, 2)` pairs each gram with the next one in the list, where the dictionary `generate_shingles(generate_ngrams(2, 3), 2, 2)` pairs across positions and leaves same-position grams unpaired.
...
The result type follows the tokenizer: `VARCHAR[]` for every text template, `BLOB[]` for [`collate_tokens`](#collate_tokens), [`encode_geopoint`](#encode_geopoint) and [`encode_geojson`](#encode_geojson), whose terms are binary. A template that stores a per-document payload alongside its terms — `generate_shingles` with `store_tokens`, `encode_geojson` with a non-source coding — returns only the terms.
...
#### `split_text_csv(value, delimiter)` {#split_text_csv}

The `split_text_csv` template: cut the value at every occurrence of `delimiter`, honouring `"` quoting — [`split_text_csv`](../../statements/create_text_search_dictionary/csv.md).
...
#### `remove_stopwords(value, stopwords := [], stopwords_path := '', hex := false)` {#remove_stopwords}
...
#### `generate_ngrams(value, min_gram := 2, max_gram := 3, preserve_original := false, input_type := 'utf8', start_marker := '', end_marker := '', mode := 'all')` {#generate_ngrams}
...
#### `generate_sparse_ngrams(value, max_ngram_length := 16, covering := false)` {#generate_sparse_ngrams}
...
#### `generate_wildcard_ngrams(value, ngram_size := 3)` {#generate_wildcard_ngrams}
...
#### `generate_shingles(value, min_gram := 2, max_gram := 2, output_unigrams := true, fallback_unigrams := false, store_tokens := true, frequent_words := [], filler_token := '')` {#generate_shingles}
...
#### `classify_text(value, model_location := '', top_k := 1, threshold := 0)` {#classify_text}
...
#### `find_nearest_words(value, model_location := '', top_k := 1)` {#find_nearest_words}
...
#### `encode_geopoint(value, latitude := [], longitude := [], max_cells := 20, min_level := 4, max_level := 23, level_mod := 1, optimize_for_space := false)` {#encode_geopoint}
...
#### `encode_geojson(value, type := 'shape', coding := 'source', max_cells := 20, min_level := 4, max_level := 23, level_mod := 1, optimize_for_space := false)` {#encode_geojson}
