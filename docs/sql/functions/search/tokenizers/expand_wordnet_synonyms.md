...
**As a function:** `expand_wordnet_synonyms(value, synonyms)` — the value first, then the options in the order below. See [tokenizer functions](./index.md) for how a value, a list and a chain of calls behave.

<SqlLogicTest id="sql/functions/search/tokenizers/expand_wordnet_synonyms/function_form" />
...
<SqlLogicTest id="sql/functions/search/tokenizers/expand_wordnet_synonyms/example_001" />

Words sharing a synset map to its id, so synonyms meet under the same token:

<SqlLogicTest id="sql/functions/search/tokenizers/expand_wordnet_synonyms/example_002" />
...
- [`expand_solr_synonyms`](./expand_solr_synonyms.md) — Solr-format synonyms
