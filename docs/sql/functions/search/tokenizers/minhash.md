...
There is no `minhash` dictionary template, and `NUMHASHES` is not an option of any template. What replaced it is a scalar function, `minhash(tokens, num_hashes)`, which returns the signature as a `LIST(BLOB)`. Signatures are built by calling that function — inside an indexed expression, or as a [`sql`](../../../statements/create_text_search_dictionary/sql.md) stage of a dictionary, `AS (lambda x: minhash(regexp_split_to_array(lower(x), '\W+'), 16))` — and matched as ordinary keyword terms. Signature components are opaque hash values, not readable words: you never search them by hand; you compare whole signatures.

Three of four tokens shared means three of four signature components shared; an unrelated set shares none:

<SqlLogicTest id="sql/functions/search/tokenizers/minhash/function_form" />
...
An expression index stores the signature; the query hashes its own text the same way and matches the components as raw keyword terms through [`ts_tokenize`](../full-text.md#ts_tokenize):
...
The second argument of [`ts_any`](../full-text.md#ts_any) is how many signature components must match, so it is the similarity threshold; [`ts_all`](../full-text.md#ts_all) requires all of them. A `min_match` above the number of components the query produced is clamped to that number, so it means "all of them". The query expression must be the indexed one: the same text hashed with a different `num_hashes` is a different expression, and the query fails with `@@ requires an inverted-indexed column on one side`.
...
CREATE TEXT SEARCH DICTIONARY mh_sql AS
    (lambda x: minhash(regexp_split_to_array(lower(x), '\W+'), 16));
...
- [`sql`](../../../statements/create_text_search_dictionary/sql.md) — template that evaluates the `minhash()` expression as the column's analyzer
- [`keyword`](../../../statements/create_text_search_dictionary/keyword.md) — template that keeps a value verbatim; `ts_tokenize(list, 'keyword')` reuses the name to take each element verbatim, without looking a dictionary up in the catalog
- [Full-Text Search Functions](../full-text.md) — `ts_any`, `ts_all` and `ts_tokenize` reference
