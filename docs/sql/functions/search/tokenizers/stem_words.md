...
On its own `stem_words` treats its whole input as a single token, so it is meant to receive pre-tokenized input. Place it as a stage inside a [`pipeline`](../../../statements/create_text_search_dictionary/pipeline/index.md), after a word splitter such as [`split_text`](./split_text.md) or [`split_text_csv`](./split_text_csv.md), and after the [`normalize_tokens`](./normalize_tokens.md) and [`remove_stopwords`](./remove_stopwords.md) stages, so that it sees folded, filtered words.

**As a function:** `stem_words(value, locale := '')` — the value first, then the options in the order below. See [tokenizer functions](./index.md) for how a value, a list and a chain of calls behave.

<SqlLogicTest id="sql/functions/search/tokenizers/stem_words/function_form" />
...
`stem_words` does not fold case, strip accents or normalize Unicode, and it does not keep the unstemmed form next to the stem. For case-insensitive stems, lowercase upstream with a [`normalize_tokens`](./normalize_tokens.md) step configured `CASE = 'lower'`; [`split_text`](./split_text.md) takes the same option, and neither template folds case by default. The input is expected to be UTF-8 and nothing validates it.

<SqlLogicTest id="sql/functions/search/tokenizers/stem_words/example_001" />

### Stemming each word of a phrase

To stem every word in a phrase, split first and stem second. A [`pipeline`](../../../statements/create_text_search_dictionary/pipeline/index.md) that runs [`split_text_csv`](./split_text_csv.md) then `stem_words` reduces each token independently:

| Input | Pipeline | Output tokens |
|---|---|---|
| `running runners ran` | `split_text_csv` (space) → `stem_words` (`en`) | `run`, `runner`, `ran` |

<SqlLogicTest id="sql/functions/search/tokenizers/stem_words/example_002" />

## See also

- [`split_text`](./split_text.md) — the word splitter that usually precedes `stem_words`
- [pipeline](../../../statements/create_text_search_dictionary/pipeline/index.md) — chain a tokenizer before `stem_words`
- [`normalize_tokens`](./normalize_tokens.md) — fold case before stemming, for case-insensitive stems
