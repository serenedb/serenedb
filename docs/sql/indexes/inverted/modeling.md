...
- **`INCLUDE` codecs**: each `INCLUDE`d column can set a `compression` codec (`auto`, the default, or `uncompressed`, `rle`, `bitpacking`, `zstd`, `alp`, `alprd`, `roaring`, `dict_fsst`, `fsst`, `dict_lz4`, `dict_zstd` or `lz4`) — see [Columnstore Storage & Compression](./columnstore.md).
