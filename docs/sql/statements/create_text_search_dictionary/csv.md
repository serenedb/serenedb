...
The template also composes: it is the usual first stage of a [`pipeline`](./pipeline/index.md) (`split_text_csv(',') | normalize_tokens(case := 'lower')`) or a branch of a [`union`](./union.md).
...
- [`split_text_csv()`](../../functions/search/tokenizers.md#split_text_csv) — the template as a function, applied to a value or a list in any query
