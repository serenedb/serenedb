...
The pieces are emitted verbatim, so chain the template into a [`pipeline`](./pipeline/index.md) if you also need case folding or stemming. Beyond taking several separators, it differs from [`split_text_csv`](./csv.md) in dropping empty tokens and in doing no quote handling, so a `"` is an ordinary byte here.
...
Because the string form is split on commas before the quotes are parsed, a comma cannot be used as a delimiter in it: `DELIMITERS = '","'` fails with `Invalid format of list of words(should be comma-separated and quoted)`. To split on a comma, pass the list form, `DELIMITERS = [',', ';']`, or use the [`split_text_csv`](./csv.md) template.
