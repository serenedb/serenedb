...
In the [expression form](./index.md#expression-form) the nested analyzer is the first argument and may be a chain, and the remaining arguments are the options below in order: `generate_shingles(split_text_csv(' ') | normalize_tokens(case := 'lower'), 2, 2)` sets `MIN_GRAM` and `MAX_GRAM`.
...
- [`split_text_csv`](./csv.md) — a minimal base tokenizer that cuts on one separator
