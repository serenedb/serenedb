...
It targets text where splitting on word boundaries does not help: source code, log lines, URLs, file paths, identifiers, serial numbers. For that kind of data a plain word tokenizer cannot answer "which rows contain `i=42`", and an [`ngram`](./ngram.md) dictionary that could do so would bloat the index with every overlapping window. `sparse_ngram` keeps the index compact by emitting only a small, carefully chosen set of variable-length grams, while still letting any substring query be answered exactly.
...
Rather than configuring the querying dictionary by hand, derive it from the indexing one with [`copy_from`](./copy-from.md) and flip `COVERING`. The copy takes the indexing dictionary's options as they stand when it is created, so the gram settings are written down once; the feature flags are the exception, since they are read only from the statement's own options and never inherited:
...
- [`ngram`](./ngram.md) — every n-gram in a length range, optionally anchored to the start or the end of the token
- [`wildcard`](./wildcard.md) — `LIKE`-style pattern matching over the terms of a nested tokenizer
