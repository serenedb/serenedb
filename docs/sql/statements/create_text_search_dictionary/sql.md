...
- `"$2" is not available in an SQL stage` — any placeholder other than `$1`, positional or named; `column "input" is not available in an SQL stage` — the pseudo-column that earlier versions used, which `$1` replaced.
