...
On its own `stem_words` treats its whole input as a single token, so it is meant to receive pre-tokenized input. Place it as a stage inside a [`pipeline`](./pipeline/index.md), after a word splitter such as [`split_text`](./text.md) or [`split_text_csv`](./csv.md), and after the [`normalize_tokens`](./norm.md) and [`remove_stopwords`](./stopwords.md) stages, so that it sees folded, filtered words.
...
`stem_words` does not fold case, strip accents or normalize Unicode, and it does not keep the unstemmed form next to the stem. For case-insensitive stems, lowercase upstream with a [`normalize_tokens`](./norm.md) step configured `CASE = 'lower'`; [`split_text`](./text.md) takes the same option, and neither template folds case by default. The input is expected to be UTF-8 and nothing validates it.

<SqlLogicTest id="sql/statements/create_text_search_dictionary/stem/example_001" />

### Stemming each word of a phrase

To stem every word in a phrase, split first and stem second. A [`pipeline`](./pipeline/index.md) that runs [`split_text_csv`](./csv.md) then `stem_words` reduces each token independently:
...
- [text](./text.md) — the word splitter that usually precedes `stem_words`
- [pipeline](./pipeline/index.md) — chain a tokenizer before `stem_words`
- [norm](./norm.md) — fold case before stemming, for case-insensitive stems
- [`stem_words()`](../../functions/search/tokenizers.md#stem_words) — the template as a function, applied to a value or a list in any query
