...
On its own `stem_words` treats its whole input as a single token, so it is meant to receive pre-tokenized input. Place it as a stage inside a [`pipeline`](./pipeline/index.md), after a word splitter such as [`split_text`](./text.md) or [`split_csv`](./csv.md), and after the [`normalize_tokens`](./norm.md) and [`remove_stopwords`](./stopwords.md) stages, so that it sees folded, filtered words.
...
To stem every word in a phrase, split first and stem second. A [`pipeline`](./pipeline/index.md) that runs [`split_csv`](./csv.md) then `stem_words` reduces each token independently:

| Input | Pipeline | Output tokens |
|---|---|---|
| `running runners ran` | `split_csv` (space) → `stem_words` (`en`) | `run`, `runner`, `ran` |
