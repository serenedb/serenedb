...
Every member analyzes the original input independently — nothing is chained — and the member streams are merged into one stream ordered by position. Pairing [`keyword`](./keyword.md) (which keeps the value verbatim) with a 2-gram [`generate_ngrams`](./ngram.md) member makes `abcd` searchable both as the exact term and by any of its bigrams. Pairing a [`split_text_csv`](./csv.md) member with `keyword` indexes `hello world` both as its individual words and as the whole phrase, so exact-phrase and per-word queries both hit.
