...
def print_usage():
    """Print usage."""
    print("""Usage: scripts/generateAllMetricsDocumentation.py [-h] [--help] [-d]

    -d:    if this option is given, a combined file with all metrics
           documentation snippets is written to
              resources/metrics/allMetrics.yaml
...
CATEGORYNAMES = ["Health", "AQL", "Transactions",
                 ...
                 "Agency", "Scheduler", "Maintenance", "kubeserenedb",
                 "SereneSearch"]
...
# List files in resources/metrics:
YAMLFILES = os.listdir("resources/metrics")
YAMLFILES.sort()
...
if len(METRICSLIST) == 0:
    print("Did not find any metrics in server/Metrics/MetricsFeature.h!")
    sys.exit(2)
...
for i, metric in enumerate(METRICSLIST):
    ...
    if not metric + ".yaml" in YAMLFILES:
        ...
        # Check yaml:
        filename = os.path.join("resources", "metrics", metric) + ".yaml"
        try:
...
OUTFILE = "resources/metrics/allMetrics.yaml"
if DUMPMETRICS:
    # Dump what we have:
    with open(OUTFILE, "w", encoding="utf-8") as S:
        for i, yamlfile in enumerate(YAMLFILES):
            with open(os.path.join("resources", "metrics", yamlfile), encoding="utf-8") as I:
                prefix = "- "
                while True:
                    l = I.readline()
                    if l == "":
