...
curl [::1]:11097/_api/index?collection=bar -sd \
     '{"deduplicate":false,"fields":["hi","_key"],"sparse":false,"type":"secondary","unique":true}'|jq -c
curl [::1]:11097/_api/index?collection=bar -sd \
     '{"deduplicate":false,"fields":["pi"],"sparse":true,"type":"secondary","unique":false}'|jq -c
curl [::1]:11097/_api/index?collection=bar -sd \
     '{"deduplicate":true,"fields":["sli"],"sparse":false,"type":"secondary","unique":false}'
...
outfile=Plan.json
echo $header > $outfile
curl [::1]:6568/_api/agency/read -sLd'[["/serene/Plan"]]'|jq .[0].serene.Plan >> $outfile
echo $footer >> $outfile

outfile=Current.json
echo $header > $outfile
curl [::1]:6568/_api/agency/read -sLd'[["/serene/Current"]]'|jq .[0].serene.Current >> $outfile
echo $footer >> $outfile

outfile=Supervision.json
echo $header > $outfile
supervision=$(curl [::1]:6568/_api/agency/read -sLd'[["/serene/Supervision"]]'|jq .[0].serene.Supervision)
