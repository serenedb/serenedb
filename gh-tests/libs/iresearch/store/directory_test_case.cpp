...
class DirectoryTestCase : public tests::DirectoryTestCaseBase<> {};

TEST_P(DirectoryTestCase, rename) {
  {
    bool res = true;
    ASSERT_TRUE(_dir->exists(res, "foo"));
    ASSERT_FALSE(res);
  }

  {
    bool res = true;
    ASSERT_TRUE(_dir->exists(res, "bar"));
    ASSERT_FALSE(res);
  }

  {
    auto stream0 = _dir->create("foo");
    ASSERT_NE(nullptr, stream0);
    stream0->WriteByte(0);
    stream0->Flush();
    auto stream1 = _dir->create("bar");
  ...
  ASSERT_TRUE(_dir->rename("foo", "foo1"));
  ASSERT_TRUE(_dir->rename("bar", "bar1"));
  ...
  {
    bool res = false;
    ASSERT_TRUE(_dir->exists(res, "foo1"));
    ASSERT_TRUE(res);
  }

  {
    bool res = false;
    ASSERT_TRUE(_dir->exists(res, "bar1"));
    ASSERT_TRUE(res);
  }

  {
    auto stream0 = _dir->open("foo1", irs::IOAdvice::NORMAL);
    ASSERT_NE(nullptr, stream0);
    ASSERT_EQ(1, stream0->Length());
    ASSERT_EQ(0, stream0->ReadByte());
    auto stream1 = _dir->open("bar1", irs::IOAdvice::NORMAL);
    ASSERT_NE(nullptr, stream1);
    ASSERT_EQ(4, stream1->Length());
    ASSERT_EQ(2, stream1->ReadI32());
  }

  ASSERT_FALSE(_dir->rename("invalid", "foo1"));
  ASSERT_TRUE(_dir->rename("bar1", "foo1"));
  ...
  {
    bool res = false;
    ASSERT_TRUE(_dir->rename("foo1", "foo1"));
    ASSERT_TRUE(_dir->exists(res, "foo1"));
    ASSERT_TRUE(res);
  }

  {
    auto stream1 = _dir->open("foo1", irs::IOAdvice::NORMAL);
    ASSERT_NE(nullptr, stream1);
    ASSERT_EQ(4, stream1->Length());
    ASSERT_EQ(2, stream1->ReadI32());
  }
...
TEST_P(DirectoryTestCase, lock_obtain_release) {
  {
    // single lock
    auto lock0 = _dir->make_lock("lock0");
    ASSERT_FALSE(!lock0);
    bool locked;
    ...
    // another single lock
    auto lock1 = _dir->make_lock("lock11");
    ASSERT_FALSE(!lock1);
  ...
  {
    auto lock0 = _dir->make_lock("lock0");
    auto lock1 = _dir->make_lock("lock1");
    ASSERT_FALSE(!lock0);
    ASSERT_FALSE(!lock1);
    bool locked;
    ASSERT_TRUE(lock0->is_locked(locked) && !locked);
  ...
  {
    auto lock0 = _dir->make_lock("lock");
    auto lock1 = _dir->make_lock("lock");
    ASSERT_FALSE(!lock0);
    ASSERT_FALSE(!lock1);
    bool locked;
    ASSERT_TRUE(lock0->is_locked(locked) && !locked);
...
TEST_P(DirectoryTestCase, create_many_files) {
  ...
  opened_files.reserve(kCount);
  for (size_t i = 0; i < kCount; ++i) {
    SCOPED_TRACE(testing::Message("File count ") << i);
    auto out = _dir->create(names[i]);
    ASSERT_FALSE(!out);
    opened_files.push_back(std::move(out));
  }
  opened_files.clear();
  for (size_t i = 0; i < kCount; ++i) {
    _dir->remove(names[i]);
...
TEST_P(DirectoryTestCase, read_multiple_streams) {
  // write data
  {
    auto out = _dir->create("test");
    ASSERT_FALSE(!out);
    out->WriteV32(0);
    out->WriteV32(1);
    out->WriteV32(2);
  ...
  // read data
  {
    auto in0 = _dir->open("test", irs::IOAdvice::NORMAL);
    ASSERT_FALSE(!in0);
    ASSERT_FALSE(in0->IsEOF());
    auto in1 = _dir->open("test", irs::IOAdvice::NORMAL);
    ASSERT_FALSE(!in1);
    ASSERT_FALSE(in1->IsEOF());
    ASSERT_EQ(0, in0->ReadV32());
    ASSERT_EQ(0, in1->ReadV32());
    ASSERT_FALSE(in0->IsEOF());
    ASSERT_FALSE(in1->IsEOF());
    ASSERT_EQ(1, in0->ReadV32());
    ASSERT_EQ(1, in1->ReadV32());
    ASSERT_FALSE(in0->IsEOF());
    ASSERT_FALSE(in1->IsEOF());
    ASSERT_EQ(2, in0->ReadV32());
    ASSERT_EQ(2, in1->ReadV32());
    ASSERT_FALSE(in0->IsEOF());
    ASSERT_FALSE(in1->IsEOF());
    ASSERT_EQ(300, in0->ReadV32());
    ASSERT_EQ(300, in1->ReadV32());
    ASSERT_FALSE(in0->IsEOF());
    ASSERT_FALSE(in1->IsEOF());
    ASSERT_EQ(4, in0->ReadV32());
    ASSERT_EQ(4, in1->ReadV32());
    ASSERT_FALSE(in0->IsEOF());
    ASSERT_FALSE(in1->IsEOF());
    ASSERT_EQ(50000, in0->ReadV32());
    ASSERT_EQ(50000, in1->ReadV32());
    ASSERT_TRUE(in0->IsEOF());
    ASSERT_TRUE(in1->IsEOF());
  }
  ...
  {
    {
      auto out = _dir->create("test_async");
      ASSERT_FALSE(!out);
    ...
    auto in = _dir->open("test_async", irs::IOAdvice::NORMAL);
    std::mutex in_mtx;
    std::mutex mutex;
...

    {
      ...
      for (auto i = pool.threads(); i; --i) {
        pool.run([&in, &in_mtx, &mutex]() -> void {
          IndexInput::ptr input;

          {
            std::lock_guard lock(in_mtx);
...
TEST_P(DirectoryTestCase, string_read_write) {
  using namespace irs;

  auto u32crc = [](irs::Crc32c& crc, uint32_t v) {
    irs::bstring tmp;
    auto it = std::back_inserter(tmp);
    irs::WriteVarint<uint32_t>(v, it);
  ...
  // strings are smaller than internal buffer size
  {
    ...
    // read strings
    {
      auto in = _dir->open("test", irs::IOAdvice::NORMAL);
      ASSERT_FALSE(!in);
      ASSERT_FALSE(in->IsEOF());
      EXPECT_EQ(strings.size(), in->ReadV32());
      for (const auto& str : strings) {
        const auto read = ReadString<std::string>(*in);
        EXPECT_EQ(str, read);
      }
...
TEST_P(DirectoryTestCase, visit) {
  using namespace irs;
  ...
  // visit empty directory
  {
    ...
    ASSERT_TRUE(_dir->visit(visitor));
    ASSERT_EQ(0, calls_count);
  }

  // add files
  for (const auto& name : names) {
    ASSERT_FALSE(!_dir->create(name));
  }
...
TEST_P(DirectoryTestCase, list) {
  ...
  ASSERT_TRUE(_dir->visit(list_files));
...
TEST_P(DirectoryTestCase, smoke_index_io) {
  using namespace irs;
  ...
  // read from file
  {
    auto in = _dir->open(name, irs::IOAdvice::NORMAL);
    ASSERT_FALSE(!in);
    ...
    // check byte
    EXPECT_EQ(27, in->ReadByte());
    EXPECT_FALSE(in->IsEOF());

    // check short
    EXPECT_EQ(std::numeric_limits<int16_t>::min(), in->ReadI16());
    EXPECT_FALSE(in->IsEOF());
    EXPECT_EQ(std::numeric_limits<uint16_t>::min(), in->ReadI16());
    EXPECT_FALSE(in->IsEOF());
    EXPECT_EQ(0, in->ReadI16());
    EXPECT_FALSE(in->IsEOF());
    EXPECT_EQ(8712, in->ReadI16());
    EXPECT_FALSE(in->IsEOF());
    EXPECT_EQ(std::numeric_limits<int16_t>::max(), in->ReadI16());
    EXPECT_FALSE(in->IsEOF());
    EXPECT_EQ(std::numeric_limits<uint16_t>::max(),
              static_cast<uint16_t>(in->ReadI16()));
    EXPECT_FALSE(in->IsEOF());

    // check int
    EXPECT_EQ(std::numeric_limits<int32_t>::min(), in->ReadI32());
    EXPECT_FALSE(in->IsEOF());
    EXPECT_EQ(std::numeric_limits<uint32_t>::min(), in->ReadI32());
    EXPECT_FALSE(in->IsEOF());
    EXPECT_EQ(0, in->ReadI32());
    EXPECT_FALSE(in->IsEOF());
    EXPECT_EQ(434328, in->ReadI32());
    EXPECT_FALSE(in->IsEOF());
    EXPECT_EQ(std::numeric_limits<int32_t>::max(), in->ReadI32());
    EXPECT_FALSE(in->IsEOF());
    EXPECT_EQ(std::numeric_limits<uint32_t>::max(),
              static_cast<uint32_t>(in->ReadI32()));
    EXPECT_FALSE(in->IsEOF());

    // check long
    EXPECT_EQ(std::numeric_limits<int64_t>::min(), in->ReadI64());
    EXPECT_FALSE(in->IsEOF());
    EXPECT_EQ(std::numeric_limits<uint64_t>::min(), in->ReadI64());
    EXPECT_FALSE(in->IsEOF());
    EXPECT_EQ(0, in->ReadI64());
    EXPECT_FALSE(in->IsEOF());
    EXPECT_EQ(8327932492393LL, in->ReadI64());
    EXPECT_FALSE(in->IsEOF());
    EXPECT_EQ(std::numeric_limits<int64_t>::max(), in->ReadI64());
    EXPECT_FALSE(in->IsEOF());
    EXPECT_EQ(std::numeric_limits<uint64_t>::max(),
              static_cast<uint64_t>(in->ReadI64()));
    EXPECT_FALSE(in->IsEOF());

    // check vint
    EXPECT_EQ(std::numeric_limits<uint32_t>::min(), in->ReadV32());
    EXPECT_FALSE(in->IsEOF());
    EXPECT_EQ(0, in->ReadV32());
    EXPECT_FALSE(in->IsEOF());
    EXPECT_EQ(8748374, in->ReadV32());
    EXPECT_FALSE(in->IsEOF());
    EXPECT_EQ(std::numeric_limits<uint32_t>::max(), in->ReadV32());
    EXPECT_FALSE(in->IsEOF());

    // check vlong
    EXPECT_EQ(std::numeric_limits<uint64_t>::min(), in->ReadV64());
    EXPECT_FALSE(in->IsEOF());
    EXPECT_EQ(0, in->ReadV64());
    EXPECT_FALSE(in->IsEOF());
    EXPECT_EQ(23289LL, in->ReadV64());
    EXPECT_FALSE(in->IsEOF());
    EXPECT_EQ(std::numeric_limits<uint64_t>::max(), in->ReadV64());
    EXPECT_FALSE(in->IsEOF());

    // check string
    const auto str_read = ReadString<std::string>(*in);
    EXPECT_TRUE(in->IsEOF());
    EXPECT_EQ(str, str_read);
  }
}

TEST_P(DirectoryTestCase, smoke_store) {
  auto& dir = *this->_dir;
  ...
  ASSERT_EQ((std::set<std::string>{std::begin(kNames), std::end(kNames)}),
            files);
  ...
  for (const auto& name : kNames) {
    ...
    EXPECT_FALSE(reopened_file->IsEOF());
...
TEST_P(DirectoryTestCase, directory_size) {
  // write integer to file
  {
    auto file = _dir->create("test_file");
...
class FsDirectoryTest : public TestBase {
 public:
  FsDirectoryTest() : _name("directory") {}

  void SetUp() final {
    TestBase::SetUp();
    _path = test_case_dir() / _name;

    irs::file_utils::Mkdir(_path.c_str(), false);
    _dir = std::make_shared<FSDirectory>(_path);
  }

  void TearDown() final {
    _dir = nullptr;
    irs::file_utils::Remove(_path.c_str());
    TestBase::TearDown();
  }
 ...
 protected:
  ...
  std::shared_ptr<FSDirectory> _dir;
};

TEST_F(FsDirectoryTest, orphaned_lock) {
  // orhpaned lock file with orphaned pid, same hostname
  {
    ...
    // try to obtain lock
    {
      auto lock = _dir->make_lock("lock");
      ASSERT_FALSE(!lock);
      ASSERT_TRUE(lock->lock());
    }

    bool exists;
    ASSERT_TRUE(_dir->exists(exists, "lock") && !exists);
  }
