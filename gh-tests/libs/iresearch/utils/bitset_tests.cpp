...
TEST(bitset_tests, memset) {
  ...
  {
    const bitset::index_t words = 2;
    const bitset::index_t size = 2 * irs::BitsRequired<irs::bitset::word_t>();

    irs::bitset bs(size);
    ASSERT_NE(nullptr, bs.data());
    ASSERT_EQ(size, bs.size());
    ASSERT_EQ(words, bs.words());
    ASSERT_EQ(words * irs::BitsRequired<irs::bitset::word_t>(), bs.capacity());
    ASSERT_EQ(0, bs.count());
    ASSERT_TRUE(bs.none());
    ASSERT_FALSE(bs.any());
    ASSERT_FALSE(bs.all());
