...
class DirectoryUtilsTests : public ::testing::Test {
 protected:
  class DirectoryMock : public irs::Directory {
   public:
    using Directory::attributes;

    irs::DirectoryAttributes& attributes() noexcept final { return _attrs; }
    ...
    irs::IndexInput::ptr open(std::string_view,
   ...
   private:
    irs::DirectoryAttributes _attrs{};
  };

  struct CallbackDirectory : tests::DirectoryMock {
    ...
    irs::IndexInput::ptr open(std::string_view name,
      ...
      auto stream = tests::DirectoryMock::open(name, advice);
      after();
      return stream;
    }
...
TEST_F(DirectoryUtilsTests, test_reference) {
  // test add file
  {
...
TEST_F(DirectoryUtilsTests, test_ref_tracking_dir) {
  ...
  struct ErrorDirectory : public irs::Directory {
    ...
    irs::DirectoryAttributes& attributes() noexcept final { return attrs; }
    irs::IndexOutput::ptr create(std::string_view) noexcept final {
      return nullptr;
    }
    bool exists(bool&, std::string_view) const noexcept final { return false; }
  ...
  } error_dir{irs::ResourceManagementOptions::gDefault};
...
TEST_F(DirectoryUtilsTests, test_tracking_dir) {
  // test dereference and attributes
  {
    irs::MemoryDirectory dir;
