...
TEST(type_utils_tests, is_convertible) {
  ...
  ASSERT_FALSE((irs::IsConvertible<A>()));
  ASSERT_FALSE((irs::IsConvertible<A, void>()));
  ASSERT_TRUE((irs::IsConvertible<void*, A*>()));
  ASSERT_TRUE((irs::IsConvertible<A, B, C>()));
}
...
TEST(type_utils_tests, in_list) {
  ...
  ASSERT_FALSE((irs::InList<A>()));
  ASSERT_FALSE((irs::InList<A, B, C>()));
  ASSERT_TRUE((irs::InList<A, A, B, C>()));
}
...
TEST(type_utils_tests, template_traits) {
  ...
  // test align_max
  {
    ...
    {
      static const size_t kAlignments[]{std::alignment_of<int32_t>::value,
                                        std::alignment_of<bool>::value,
    ...
    {
      static const size_t kAlignments[]{
