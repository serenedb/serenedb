...
GeoTokenizer::GeoTokenizer(const S2RegionTermIndexer::Options& options)
  : _options{options} {
  *_coverer.mutable_options() = options;
}

bool GeoTokenizer::IsGeoTokenizer(const Tokenizer& tokens) noexcept {
  return tokens.type() == irs::Type<GeoPointTokenizer>::id() ||
         tokens.type() == irs::Type<GeoJsonTokenizer>::id();
}

GeoTokenizer& GeoTokenizer::Cast(Tokenizer& tokens) noexcept {
  if (tokens.type() == irs::Type<GeoPointTokenizer>::id()) {
    return irs::utils::downCast<GeoPointTokenizer>(tokens);
  }
  SDB_ASSERT(tokens.type() == irs::Type<GeoJsonTokenizer>::id());
  return irs::utils::downCast<GeoJsonTokenizer>(tokens);
}

template<TokenLayout Layout>
void GeoTokenizer::EmitTerms(TokenSink& sink) {
...
bool GeoTokenizer::DoFill(duckdb::string_t raw, TokenSink& sink) {
  if constexpr (Wkb) {
    if (!resetWKB(raw)) {
      return false;
    }
...
template class TypedTokenizer<GeoPointTokenizer>;
template class TypedTokenizer<GeoJsonTokenizer>;

irs::analysis::Tokenizer::ptr GeoPointTokenizer::Make(Options opts) {
  ...
  return std::make_unique<GeoPointTokenizer>(opts);
}

GeoPointTokenizer::GeoPointTokenizer(const Options& options)
  : GeoTokenizer{S2Options(options.options, true)},
...
bool GeoPointTokenizer::reset(simdjson::ondemand::value json) {
  S2LatLng point;
...
bool GeoPointTokenizer::resetWKB(duckdb::string_t wkb) {
  if (S2LatLng ll; ParseWkbPoint(wkb, ll)) {
    RestagePoint(ll.ToPoint());
    return true;
  }
  irs::geo::ShapeContainer shape;
  if (!irs::geo::ParseShapeWKB({wkb.GetData(), wkb.GetSize()}, shape)) {
    return false;
  }
  if (shape.type() != irs::geo::ShapeContainer::Type::S2Point) {
    return false;
  }
...
void GeoPointTokenizer::prepare(GeoFilterOptionsBase& options) const {
...
bool GeoPointTokenizer::FindDouble(simdjson::ondemand::object& object,
...
bool GeoPointTokenizer::ParsePoint(simdjson::ondemand::value json,
...
irs::analysis::Tokenizer::ptr GeoJsonTokenizer::Make(Options opts) {
  opts.options.Validate("geo_json");
  return std::make_unique<GeoJsonTokenizer>(opts);
}

GeoJsonTokenizer::GeoJsonTokenizer(const Options& options)
  : GeoTokenizer{S2Options(options.options, options.type != Type::Shape)},
    _type{options.type},
    _coding{options.coding} {
  if (_coding != Coding::Source) {
    _s2_coding = irs::geo::coding::Options{std::to_underlying(_coding)};
    _encoder.Ensure(30);
  }
}

bool GeoJsonTokenizer::reset(simdjson::ondemand::value json) {
  _encoder.clear();
...
bool GeoJsonTokenizer::resetWKB(duckdb::string_t wkb) {
  ...
  if (!irs::geo::ParseShapeWKB({wkb.GetData(), wkb.GetSize()}, _shape)) {
    return false;
  }
  if (_type == Type::Point &&
      _shape.type() != irs::geo::ShapeContainer::Type::S2Point) {
    return false;
  }
  if (SerializesShape()) {
