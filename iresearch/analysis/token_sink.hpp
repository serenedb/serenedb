...
class TokenSink final : util::Noncopyable {
 public:
  ...
  template<TokenLayout L, typename Gen>
  IRS_FORCE_INLINE void EmitRuns(size_t k, size_t max_run,
                                 const byte_type* base, const byte_type* limit,
                                 Gen gen) {
    SDB_ASSERT(max_run != 0);
    const auto* chars = reinterpret_cast<const char*>(base);
    const PaddedTail tail{chars, reinterpret_cast<const char*>(limit)};
    std::invoke_result_t<Gen&> run{};
    size_t pending = 0;
    size_t done = 0;
    while (done < k) {
      const auto slots = Next(k - done);
      auto* slot = slots.data();
      auto* const slot_end = slot + slots.size();
      if (pending != 0) {
        const size_t n = std::min<size_t>(pending, slot_end - slot);
        EmitRunGrams<L>(run, run.count - pending, n, slot, chars, tail);
        pending -= n;
        slot += n;
      }
      while (static_cast<size_t>(slot_end - slot) >= max_run) {
        run = gen();
        SDB_ASSERT(run.count != 0);
        SDB_ASSERT(run.count <= max_run);
        EmitRunGrams<L>(run, 0, run.count, slot, chars, tail);
        slot += run.count;
      }
      while (slot != slot_end) {
        run = gen();
        SDB_ASSERT(run.count != 0);
        const auto room = static_cast<size_t>(slot_end - slot);
        const size_t n = run.count <= room ? run.count : room;
        EmitRunGrams<L>(run, 0, n, slot, chars, tail);
        pending = run.count - n;
        slot += n;
      }
      done += slots.size();
 ...
 private:
  ...
  template<TokenLayout L, typename Run>
  IRS_FORCE_INLINE void EmitRunGrams(const Run& run, size_t from, size_t n,
                                     duckdb::string_t* slot, const char* chars,
                                     const PaddedTail& tail) {
    SDB_ASSERT(n != 0);
    const char* data = chars + run.begin;
    const auto bytes = tail.Load(data);
    const auto first = static_cast<uint32_t>(slot - _batch.terms);
    size_t i = 0;
    do {
      const uint32_t e = RunEnd(run, static_cast<uint32_t>(from + i));
      const uint32_t size = e - run.begin;
      if (size <= duckdb::string_t::INLINE_LENGTH) [[likely]] {
        StoreInlineTerm(slot + i, bytes, size);
      } else {
        slot[i] = duckdb::string_t{data, size};
      }
      PutSlotLanes<L>(first + static_cast<uint32_t>(i),
                      EmitKSlotPos{run.begin, e, run.pos}, Offs{run.begin, e});
    } while (++i != n);
  }

  static IRS_FORCE_INLINE uint32_t RunEnd(const EmitRun& run,
                                          uint32_t i) noexcept {
    return run.end + i;
  }

  static IRS_FORCE_INLINE uint32_t RunEnd(const EmitRunEnds& run,
                                          uint32_t i) noexcept {
    return run.ends[i];
  }
