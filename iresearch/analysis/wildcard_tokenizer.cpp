...
Tokenizer::ptr WildcardTokenizer::Make(Options opts,
                                       duckdb::SharedObjectCache& cache) {
  Tokenizer::ptr base;
  if (opts.base_analyzer) {
    base = CreateTokenizer(std::move(*opts.base_analyzer), cache);
  }
  return std::make_unique<WildcardTokenizer>(std::move(base), opts.ngram_size);
}
...
struct WildcardTokenizer::SubSink {
...
WildcardTokenizer::~WildcardTokenizer() = default;

std::tuple<bool> WildcardTokenizer::PrepareBatch(BlockTraits traits) {
  if (!_sub_sink) {
    ...
    [&](auto layout_tag, auto ascii_tag) IRS_FORCE_INLINE {
      ...
      for (const auto& term : tokens) {
        ...
        if constexpr (!ascii_tag()) {
          ascii = ascii && classify::IsAsciiValue(term.GetData(), term.GetSize());
        }
      }
...
void WildcardTokenizer::EmitTermGrams(TokenSink& sink, const byte_type* term,
                                      uint32_t size) {
...
WildcardTokenizer::WildcardTokenizer(Tokenizer::ptr base_analyzer,
                                     size_t ngram_size)
  : _analyzer{std::move(base_analyzer)},
    _ngram{NGramTokenizer::Options{
      ngram_size,
      ngram_size,
      false,
      NGramTokenizer::InputType::UTF8,
      {},
      {},
    }} {
  if (!_analyzer) {
...
template class TypedTokenizer<WildcardTokenizer>;
