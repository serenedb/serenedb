...
IRS_FORCE_INLINE void Put(uint32_t** blocks, uint64_t i,
                          uint32_t value) noexcept {
  blocks[i >> ScatterScratch::kBlockShift][i & ScatterScratch::kBlockMask] =
    value;
}
...
void ScatteredField::Reset(const FieldInverter& field) {
  ...
  const auto entries = field.Dictionary().Entries();
  SDB_ASSERT(field.InlineDocs().size() <= entries.size());
  _identity =
    _all_inline && absl::c_is_sorted(entries.first(field.InlineDocs().size()),
                                     std::less_equal<>{});
  if (_identity) {
    return;
  }
  ...
  if (field.UniqueTerms()) {
    FoldDuplicateTerms(entries);
  }
  if (_all_inline) {
    const auto docs = field.InlineDocs();
    const auto* ranked = _s->ranked.data();
    auto& out = _s->inline_docs;
    out.resize(docs.size());
    for (size_t r = 0; r < docs.size(); ++r) {
      out[r] = docs[ranked[r].id];
    }
    return;
  }
...
void ScatteredField::FoldDuplicateTerms(
  std::span<const duckdb::string_t> entries) {
...
void ScatteredField::RankLiveTerms(std::span<const duckdb::string_t> entries) {
  ...
  if (absl::c_is_sorted(ranked, less)) {
    return;
  }
  if (ranked.size() < kRadixThreshold) {
    absl::c_sort(ranked, less);
    return;
  }
  RadixSortByKey();
...
void ScatteredField::RekeyPastSharedPrefix(
  std::span<const duckdb::string_t> entries, uint64_t min_key,
  uint64_t max_key) {
  auto& ranked = _s->ranked;
  uint32_t skip = 0;
  while (skip < kRekeyMaxSkip && max_key != 0) {
    const uint64_t diff = min_key ^ max_key;
    const auto shared =
      diff ? static_cast<uint32_t>(std::countl_zero(diff)) / 8 : 8;
    if (shared < kRekeyMinShared) {
      break;
    }
    skip += shared;
...
uint32_t** ScatteredField::AssignBlocks(ManagedVector<uint32_t*>& col,
  ...
  for (auto& block : col) {
    if (next == pool.size()) {
      pool.push_back(_mem->allocator.Allocate(ScatterScratch::kBlockValues *
                                              sizeof(uint32_t)));
    }
...
void ScatteredField::Scatter(const Log& log, uint64_t nocc) {
  ...
  const size_t nblocks =
    (nocc + ScatterScratch::kBlockMask) >> ScatterScratch::kBlockShift;
  size_t next = 0;
  uint32_t** docs = AssignBlocks(_s->docs, nblocks, next);
  uint32_t** positions = nullptr;
  uint32_t** offs_start = nullptr;
  uint32_t** offs_end = nullptr;
  if constexpr (kPos) {
    positions = AssignBlocks(_s->pos, nblocks, next);
  }
  if constexpr (kOffs) {
    offs_start = AssignBlocks(_s->offs_start, nblocks, next);
    offs_end = AssignBlocks(_s->offs_end, nblocks, next);
  }
    ...
    for (uint32_t k = 0; k < run.ndocs; ++k, ++doc_idx) {
      ...
      const auto emit = [&](auto&& pos_of) {
        for (uint32_t j = 0; j < ntokens; ++j) {
          ...
          if constexpr (kOffs) {
            offs += delta_col.Read();
            Put(offs_start, c, offs);
            Put(offs_end, c, offs + len_col.Read());
          }
        }
      };
      if constexpr (kPos) {
