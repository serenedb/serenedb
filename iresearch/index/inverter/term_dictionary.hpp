...
class TermDictionary : util::Noncopyable {
 public:
  ...
  TermDictionary(duckdb::ArenaAllocator& arena, IResourceManager& rm)
    : _map{0, RefHashEq{&_entries}, RefHashEq{&_entries}},
      _entries{ManagedTypedAllocator<duckdb::string_t>{rm}},
      _arena{&arena} {}
  ...
  IRS_FORCE_INLINE void Insert(std::span<const duckdb::string_t> terms,
    ...
    for (size_t i = 0; SDB_LIKELY(i < n); ++i) {
  ...
  std::span<const duckdb::string_t> Entries() const noexcept {
  ...
  size_t Memory() const noexcept {
    return _entries.capacity() * sizeof(duckdb::string_t) +
 ...
 private:
  struct RefHashEq {
    ...
    const ManagedVector<duckdb::string_t>* entries;
  };
  ...
  IRS_FORCE_INLINE uint32_t InsertHashed(const HashedTerm& key) {
  ...
  IRS_FORCE_INLINE duckdb::string_t Intern(const duckdb::string_t& term) {
  ...
  irs::containers::FlatHashSet<Ref, RefHashEq, RefHashEq> _map;
  ManagedVector<duckdb::string_t> _entries;
