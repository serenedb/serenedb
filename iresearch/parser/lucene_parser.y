...
namespace irs { struct ParserContext; }
...
%parse-param { irs::ParserContext& ctx }
...
clause_list:
    mod_clause                      { ctx.AddClause(irs::Conjunction::Or); }
    | clause_list mod_clause        { ctx.AddClause(irs::Conjunction::Or); }
    | clause_list AND mod_clause    { ctx.AddClause(irs::Conjunction::And); }
    | clause_list OR mod_clause     { ctx.AddClause(irs::Conjunction::Or); }
    ;

mod_clause:
    term_expr                       { ctx.last_mod = irs::Modifier::None; }
    | PLUS term_expr                { ctx.last_mod = irs::Modifier::Required; }
    | MINUS term_expr               { ctx.last_mod = irs::Modifier::Not; }
    | NOT term_expr                 { ctx.last_mod = irs::Modifier::Not; }
    ;
