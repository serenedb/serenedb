...
struct ParserContext {
  ...
  irs::Filter& EndGroup(irs::Filter* parent_root) {
    ...
    _open.pop_back();
    current_root = &irs::utils::downCast<irs::BooleanFilter>(*parent_root);
    return node;
  }
  ...
  void SetSlop(irs::Filter* f, int slop) {
    irs::utils::downCast<irs::ByPhrase>(*f).mutable_options()->set_slop(
  ...
  void SetMinMatch(irs::Filter* f, int count) {
    auto& node = irs::utils::downCast<irs::BooleanFilter>(*f);
  ...
  irs::Filter& ApplyFuzzy(irs::Filter* built, bool has_value, float value) {
    ...
    auto& term = irs::utils::downCast<irs::ByTerm>(*built);
