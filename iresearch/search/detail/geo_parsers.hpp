...
struct SourceJsonParser {
  ...
  bool operator()(bytes_view value, irs::geo::ShapeContainer& shape) const {
    ...
    return irs::geo::ParseShape<irs::geo::Parsing::FromIndex>(
      json, shape, _cache, irs::geo::coding::Options::Invalid, nullptr);
  }
...
struct SourceWkbParser {
  bool operator()(bytes_view value, irs::geo::ShapeContainer& shape) const {
    ...
    const std::string_view bytes{reinterpret_cast<const char*>(value.data()),
                                 value.size()};
    shape = {};
    return irs::geo::ParseShapeWKB(bytes, shape);
  }
};
...
struct SourcePointParser {
  ...
  bool operator()(bytes_view value, irs::geo::ShapeContainer& shape) const {
    ...
    shape.reset(S2LatLng::FromDegrees(lat, lng).Normalized().ToPoint(),
                irs::geo::coding::Options::Invalid);
    return true;
  }
...
struct S2PointParser {
  bool operator()(bytes_view value, irs::geo::ShapeContainer& shape) const {
    SDB_ASSERT(!value.empty());
    SDB_ASSERT(shape.type() == irs::geo::ShapeContainer::Type::S2Point);
    Decoder decoder{value.data(), value.size()};
    S2Point point;
