...
inline IRS_FORCE_INLINE uint32_t FilterScores(doc_id_t* IRS_RESTRICT docs,
  ...
  for (; i + 8 <= len; i += 8) {
    ...
    const auto ids =
      _mm256_loadu_si256(reinterpret_cast<const __m256i*>(docs + i));
    const auto hits =
      _mm256_loadu_si256(reinterpret_cast<const __m256i*>(matches + i));
    _mm256_storeu_si256(reinterpret_cast<__m256i*>(docs + out),
                        _mm256_permutevar8x32_epi32(ids, control));
    _mm256_storeu_si256(reinterpret_cast<__m256i*>(matches + out),
                        _mm256_permutevar8x32_epi32(hits, control));
    _mm256_storeu_ps(scores + out, _mm256_permutevar8x32_ps(values, control));
    out += static_cast<uint32_t>(std::popcount(mask));
  }
#endif
...
inline IRS_FORCE_INLINE uint32_t FilterMatches(doc_id_t* IRS_RESTRICT docs,
                                               score_t* IRS_RESTRICT scores,
                                               uint32_t* IRS_RESTRICT matches,
                                               uint32_t len,
  ...
  for (uint32_t i = 0; i != len; ++i) {
    const auto hits = matches[i];
    docs[out] = docs[i];
    scores[out] = scores[i];
    matches[out] = hits;
    out += static_cast<uint32_t>(hits >= need);
  }
  return out;
}
