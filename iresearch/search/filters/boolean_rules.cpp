...
BooleanFilter* AsNested(Filter& child, const BooleanFilter& parent, Occur occur,
  ...
  auto& node = irs::utils::downCast<BooleanFilter>(child);
...
field_id AnyFieldOf(const Filter& node) {
  ...
  (void)((node.type() == Type<Ts>::id()
            ? (field = irs::utils::downCast<Ts>(node).field_id(), true)
            : false) ||
         ...);
  return field;
}
...
void CollectAnchors(const Filter& node,
                    irs::containers::FlatHashSet<field_id>& out);
...
void CollectBucketAnchors(const BooleanFilter& node,
  ...
  irs::containers::FlatHashSet<field_id> common;
  irs::containers::FlatHashSet<field_id> branch;
  bool first = true;
...
void CollectAnchors(const Filter& node,
  ...
  if (node.type() == Type<BooleanFilter>::id()) {
    CollectBucketAnchors(irs::utils::downCast<BooleanFilter>(node), out);
    return;
  }
...
bool BooleanNullMarkerRule::Apply(Filter::ptr& slot,
  ...
  irs::containers::FlatHashSet<field_id> anchors;
  bool ready = false;
...
bool BooleanFlattenRule::Apply(Filter::ptr& slot, const OptimizeContext& ctx) {
  ...
  for (auto& [occur, child] : nested) {
    auto& inner = irs::utils::downCast<BooleanFilter>(*child);
    PushDown(inner);
...
std::optional<OrAcceptorFusionRule::AcceptorInfo> OrAcceptorFusionRule::InfoOf(
  ...
  if (type == Type<ByTerm>::id()) {
    return info(irs::utils::downCast<ByTerm>(child));
  }
  if (type == Type<ByPrefix>::id()) {
    return info(irs::utils::downCast<ByPrefix>(child));
  }
  if (type == Type<ByWildcard>::id()) {
    return info(irs::utils::downCast<ByWildcard>(child));
  }
  if (type == Type<ByRegexp>::id()) {
    return info(irs::utils::downCast<ByRegexp>(child));
  }
  if (type == Type<AutomatonFilter>::id()) {
    return info(irs::utils::downCast<AutomatonFilter>(child));
  }
  return std::nullopt;
}
