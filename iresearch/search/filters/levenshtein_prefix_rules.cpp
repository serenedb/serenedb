...
bool LevenshteinPrefixFusionRule::Apply(Filter::ptr& slot,
  ...
  for (size_t i = 0; i < children.size(); ++i) {
    if (children[i]->type() == Type<ByPrefix>::id()) {
      auto& prefix = irs::utils::downCast<ByPrefix>(*children[i]);
  ...
  bool changed = false;
  irs::containers::FlatHashSet<const Filter*> remove_prefixes;
  for (auto& child : children) {
    if (child->type() != Type<ByEditDistance>::id()) {
      continue;
    }
    auto& filter = irs::utils::downCast<ByEditDistance>(*child);
