...
class ImmutableFastText : public FastText {
 public:
  ...
  std::vector<std::pair<real, std::string>> getNN(const std::string& word,
    ...
    SDB_ASSERT(wordVectors_);
    return getNN(*wordVectors_, query, k, {word});
  }

  std::vector<std::pair<real, std::string>> getNN(
    const DenseMatrix& word_vectors, const Vector& query_vec, int32_t k,
    const std::set<std::string>& ban_set) const {
    return FastText::getNN(word_vectors, query_vec, k, ban_set);
  }
};
