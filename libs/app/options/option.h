...
/// option flags. these can be bit-ORed to combine multiple flags
...
constexpr std::underlying_type_t<Flags> MakeFlags() {
  return std::to_underlying(Flags::None);
}

/// helper for building flags
template<typename... Args>
constexpr std::underlying_type_t<Flags> MakeFlags(Flags flag, Args... args) {
  return std::to_underlying(flag) | MakeFlags(args...);
}

template<typename... Args>
constexpr std::underlying_type_t<Flags> MakeDefaultFlags(Flags flag,
                                                         Args... args) {
  return std::to_underlying(Flags::Default) | std::to_underlying(flag) |
         MakeFlags(args...);
}
...
struct Option {
  ...
  Option(const std::string& value, const std::string& description,
  ...
  void toVPack(vpack::Builder& builder, bool detailed) const;
  ...
  std::string toVersionString(const std::vector<uint32_t>& version) const;

  // provide a detailed explanation of an option
  Option& setLongDescription(std::string_view long_desc) noexcept;
  ...
  // hidden
  void printHelp(const std::string& search, size_t tw, size_t ow, bool) const;
  ...
  // strip the "-" from a string
  static std::string stripShorthand(const std::string& name);

  // split an option name at the ".", if it exists
  static std::pair<std::string, std::string> splitName(std::string name);

  static std::vector<std::string> wordwrap(const std::string& value,
                                           size_t size);

  // right-pad a string
  static std::string pad(const std::string& value, size_t length);
  ...
  std::vector<uint32_t> introduced_in_versions;
  std::vector<uint32_t> deprecated_in_versions;
};
