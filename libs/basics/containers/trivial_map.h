...
#include "basics/containers/string_literal.h"
...
struct Found final {
  constexpr explicit Found(std::size_t value) noexcept {
    SDB_ASSERT(value == 0);
  }
...
class StringOrPayload final {
 public:
  constexpr explicit StringOrPayload(std::string_view string) noexcept
    : data_or_payload_(string.data() ? string.data() : ""),
      size_(string.size()) {
  ...
  constexpr const char* GetStringPointer() const noexcept {
    SDB_ASSERT(!HasPayload());
    SDB_ASSERT(data_or_payload_.data);
...
template<typename Value>
class SearchState<cstring_view, Value,
                  std::enable_if_t<kFitsInStringOrPayload<Value>>>
  final : public SearchState<std::string_view, Value> {};

template<typename Value>
class SearchState<StringLiteral, Value,
                  std::enable_if_t<kFitsInStringOrPayload<Value>>>
  final : public SearchState<std::string_view, Value> {};
...
class SearchState<Key, std::string_view,
 ...
 public:
  ...
  [[nodiscard]] constexpr std::optional<std::string_view> Extract() noexcept {
    return IsFound() ? std::optional{std::string_view{state_.GetStringPointer(),
                                                      state_.GetStringSize()}}
                     : std::nullopt;
  }
...
template<typename Key>
class SearchState<Key, cstring_view,
                  std::enable_if_t<kFitsInStringOrPayload<Key>>>
  final {
 public:
  constexpr explicit SearchState(Key key) noexcept : state_(key) {}
  ...
  constexpr void SetValue(cstring_view value) noexcept {
    state_ = StringOrPayload<Key>{value};
  }

  [[nodiscard]] constexpr std::optional<cstring_view> Extract() noexcept {
    return IsFound() ? std::optional{cstring_view::UnsafeMake(
                         state_.GetStringPointer(), state_.GetStringSize())}
                     : std::nullopt;
  }
...
template<typename Key>
class SearchState<Key, StringLiteral,
                  std::enable_if_t<kFitsInStringOrPayload<Key>>>
  final {
 public:
  constexpr explicit SearchState(Key key) noexcept : state_(key) {}
  ...
  constexpr void SetValue(StringLiteral value) noexcept {
    state_ = StringOrPayload<Key>{value};
  }

  [[nodiscard]] constexpr std::optional<StringLiteral> Extract() noexcept {
    return IsFound() ? std::optional{StringLiteral::UnsafeMake(
                         state_.GetStringPointer(), state_.GetStringSize())}
                     : std::nullopt;
  }
...
class SwitchByFirstICase final {
 public:
  ...
  constexpr SwitchByFirstICase& Case(std::string_view first,
    ...
    SDB_ASSERT(!impl::HasUppercaseAscii(first), "String literal '", first,
               "' in utils::Switch*::Case() should be in lower case");
...
class SwitchBySecondICase final {
 public:
  ...
  constexpr SwitchBySecondICase& Case(First first,
    ...
    SDB_ASSERT(!impl::HasUppercaseAscii(second), "String literal '", second,
               "' in utils::Switch*::Case() should be in lower case");
...
class SwitchTypesDetector final {
 public:
  ...
  using DetectType = std::conditional_t<
    std::is_convertible_v<T, StringLiteral>, StringLiteral,
    std::conditional_t<  // TODO: force StringLiteral
      std::is_same_v<T, cstring_view>, cstring_view,
      std::conditional_t<std::is_convertible_v<T, std::string_view>,
                         std::string_view, T>>>;
...
/// Decays utils::StringLiteral and utils::cstring_view to a more generic
/// std::string_view type.
template<class T>
using DecayToStringView = std::conditional_t<std::is_same_v<T, StringLiteral> ||
                                               std::is_same_v<T, cstring_view>,
                                             std::string_view, T>;
...
class TrivialBiMap final {
 ...
 public:
  ...
  template<class T>
  using MappedTypeFor =
    std::conditional_t<std::is_convertible_v<T, DecayToStringView<First>>,
                       Second, First>;
  ...
  constexpr TrivialBiMap(BuilderFunc&& func) noexcept : func_(std::move(func)) {
    ...
    static_assert(!std::is_void_v<Second>,
                  "If second type in Case is missing, use utils::TrivialSet "
                  "instead of utils::TrivialBiMap");
    static_assert(std::is_trivially_copyable_v<Second>,
                  "Second type in Case must be trivially copyable");
  }

  constexpr std::optional<Second> TryFindByFirst(
    DecayToStringView<First> value) const noexcept {
    return func_([value]() {
             return impl::SwitchByFirst<DecayToStringView<First>, Second>{
               value};
           })
      .Extract();
  }

  constexpr std::optional<First> TryFindBySecond(
    DecayToStringView<Second> value) const noexcept {
    return func_([value]() {
             return impl::SwitchBySecond<First, DecayToStringView<Second>>{
               value};
           })
      .Extract();
  }
  ...
  constexpr std::optional<MappedTypeFor<std::string_view>> TryFindICase(
    std::string_view value) const noexcept {
    if constexpr (std::is_convertible_v<std::string_view,
                                        DecayToStringView<First>>) {
      static_assert(
  ...
  constexpr ValueType GetValuesByIndex(std::size_t index) const {
    SDB_ASSERT(index < size(), "Index is out of bounds");
    auto result = func_(
...
class TrivialSet final {
 ...
 public:
  ...
  /// value.
  constexpr std::optional<std::size_t> GetIndex(
    DecayToStringView<First> value) const {
  ...
  constexpr std::optional<std::size_t> GetIndexICase(
...
std::string_view EnumToStringView(Enum value, TrivialBiMap<BuilderFunc> map) {
  ...
  SDB_ENSURE(string, ERROR_INTERNAL, "Invalid value of enum ",
             magic_enum::enum_name<Enum>(), ": ", std::to_underlying(value));
  return *string;
}
...
template<const auto& Pairs>
struct TrivialBiMapFirstToIndexDispatch {
  template<class Selector>
  constexpr auto operator()(Selector selector) const {
    return impl::TrivialBiMapFirstToIndexMultiCase(
      selector(), Pairs, std::make_index_sequence<std::size(Pairs)>{});
  }
};
...
/// @brief Builds a TrivialBiMap from a global `constexpr` array of pairs,
/// mapping each `entry.first` to its array index.
template<const auto& Pairs>
constexpr auto MakeTrivialBiMapFirstToIndex() {
  static_assert(std::size(Pairs) >= 1);
  return TrivialBiMap(impl::TrivialBiMapFirstToIndexDispatch<Pairs>{});
}
