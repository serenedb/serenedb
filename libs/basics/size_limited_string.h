...
class SizeLimitedString {
 ...
 public:
  SizeLimitedString(const SizeLimitedString&) = delete;
  SizeLimitedString& operator=(const SizeLimitedString&) = delete;

  SizeLimitedString() noexcept { clear(); }

  std::string_view view() const noexcept {
    SDB_ASSERT(_offset <= N);
  ...
  SizeLimitedString& appendUInt64(uint64_t value) noexcept {
    if (!_full) {
      if (basics::kMaxU64B10StringSize > remaining()) {
        _full = true;
      } else {
        _offset += basics::string_utils::Itoa(value, _buffer + _offset);
        recalculateState();
      }
    }
    return *this;
  ...
  template<typename T>
  SizeLimitedString& appendHexValue(T value,
                                    bool strip_leading_zeros) noexcept {
    ...
    // this case.
    constexpr char kChars[] = "0123456789abcdef";
    for (auto c : buffer) {
      if (!strip_leading_zeros || (c >> 4U) != 0) {
        push_back(kChars[c >> 4U]);
        strip_leading_zeros = false;
      }
      if (!strip_leading_zeros || (c & 0xfU) != 0) {
        push_back(kChars[c & 0xfU]);
        strip_leading_zeros = false;
      }
    }
