...
class EndpointList {
 public:
  ...
  bool add(const std::string& specification, int backlog_size,
           bool reuse_address);
  std::vector<std::string> all(Endpoint::TransportType transport) const;
  void apply(const std::function<void(const std::string&, Endpoint&)>& cb);
  bool hasSsl() const;
  void dump() const;

  auto begin() const { return _endpoints.begin(); }
  auto end() const { return _endpoints.end(); }
