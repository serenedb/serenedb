...
class Connection : public std::enable_shared_from_this<Connection> {
 ...
 public:
  ...
  std::unique_ptr<Response> sendRequest(const Request& r) {
 ...
 protected:
  Connection(const detail::ConnectionConfiguration& conf) : _config(conf) {}

  // Invoke the configured ConnectionFailureCallback (if any)
  void onFailure(Error error_code, const std::string& error_message) {
    if (_config.on_failure) {
      try {
        _config.on_failure(error_code, error_message);
      } catch (...) {
      }
    }
  }
...
class ConnectionBuilder {
 public:
  ...
  AuthenticationType authenticationType() const {
    return _conf.authentication_type;
  }
  ConnectionBuilder& authenticationType(AuthenticationType t) {
    _conf.authentication_type = t;
    return *this;
  }
  // Set the username of the connection
  std::string user() const { return _conf.user; }
  ConnectionBuilder& user(std::string_view u) {
    _conf.user = u;
    return *this;
  }
  // Set the password of the connection
  std::string password() const { return _conf.password; }
  ConnectionBuilder& password(std::string_view p) {
    _conf.password = p;
    return *this;
  }

  // Set the jwt token of the connection
  std::string jwtToken() const { return _conf.jwt_token; }
  ConnectionBuilder& jwtToken(std::string_view t) {
    _conf.jwt_token = t;
    return *this;
  }

  /// @brief tcp, ssl or unix
  SocketType socketType() const { return _conf.socket_type; }
  /// @brief protocol type
  ProtocolType protocolType() const { return _conf.protocol_type; }
  void protocolType(ProtocolType pt) { _conf.protocol_type = pt; }

  /// upgrade http1.1 to http2 connection (not necessary)
  ConnectionBuilder& upgradeHttp1ToHttp2(bool c) {
    _conf.upgrade_h1_to_h2 = c;
    return *this;
  }

  /// @brief should we verify the SSL host
  bool verifyHost() const { return _conf.verify_host; }
