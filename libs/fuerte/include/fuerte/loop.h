...
class EventLoopService {
 public:
  ...
  explicit EventLoopService(unsigned thread_count = 1, const char* name = "");
  virtual ~EventLoopService();

  // Prevent copying
  EventLoopService(const EventLoopService& other) = delete;
  EventLoopService& operator=(const EventLoopService& other) = delete;
 ...
 private:
  ...
  std::atomic<uint32_t> _last_used;

  /// protect ssl context creation
  absl::Mutex _ssl_context_mutex;
  /// global SSL context to use here
  std::unique_ptr<asio_ns::ssl::context> _ssl_context;

  /// io contexts
  std::vector<std::shared_ptr<asio_ns::io_context>> _io_contexts;
