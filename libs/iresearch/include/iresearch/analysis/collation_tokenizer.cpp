...
std::tuple<> CollationTokenizer::PrepareBatch() {
  if (!_collator) {
...
bool CollationTokenizer::DoFill(duckdb::string_t raw, TokenSink& sink) {
  ...
  const int32_t term_size =
    ...
    1;
  // ucol_getSortKey returns the zero-terminated length; 0 means failure or
  // overflow. The terminator is never stored in the dictionary.
  ...
  if (_options.force_utf8) {
    ...
    sink.Emit<Layout>(2 * size, [&](byte_type* mem) IRS_FORCE_INLINE {
      return EncodeSortKey(mem, raw_term_buf, size);
    });
  } else {
    sink.Emit<Layout>(size, [&](byte_type* mem) IRS_FORCE_INLINE {
      std::memcpy(mem, raw_term_buf, size);
      return static_cast<uint32_t>(term_size);
    });
  }
  return true;
}
