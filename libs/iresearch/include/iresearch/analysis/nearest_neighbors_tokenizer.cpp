...
Tokenizer::ptr NearestNeighborsTokenizer::Make(
  ...
  auto model = sdb::fast_text::GetOrBuildModel<fasttext::ImmutableFastText>(
...
bool NearestNeighborsTokenizer::DoFill(duckdb::string_t raw, TokenSink& sink) {
  ...
  for (const auto token_id : _line_token_ids) {
    ...
    for (const auto& [score, word] : neighbors) {
      sink.Emit<Layout>(word.data(), static_cast<uint32_t>(word.size()), pos);
    }
  }
  return true;
}
