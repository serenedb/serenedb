...
delim::Finder DetectSplit(const re2::RE2& pattern, int group) {
  ...
  return delim::FinderFor(set);
}

}  // namespace

delim::Finder PatternTokenizer::Detect(std::string_view pattern, int group) {
...
Tokenizer::ptr PatternTokenizer::Make(Options opts) {
  ...
  if (auto split =
        delim::Make<PatternTokenizer>(DetectSplit(*regex, opts.group))) {
    return split;
  }
  return std::make_unique<PatternRegexTokenizer>(std::move(regex), opts.group);
}
