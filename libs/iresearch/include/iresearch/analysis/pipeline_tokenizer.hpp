...
class PipelineTokenizer final : public TypedTokenizer<PipelineTokenizer>,
 ...
 private:
  ...
  struct SubAnalyzer {
    explicit SubAnalyzer(Tokenizer::ptr a, bool track_offset);

    bool Tokenize(uint32_t start, uint32_t end, duckdb::string_t data,
                  SubSink& scratch, bool data_durable = false) {
      _data_size = data.GetSize();
      _data_start = start;
      _data_end = end;
      ...
      _offs_start.clear();
      _offs_end.clear();
      // A producer may emit views into `data`; retain them zero-copy only when
      // `data` is the pipeline's original input (durable for the whole fill).
      // Inner stages tokenize a transient upstream term, so they copy.
      scratch.accumulator.Bind(_terms, _pos, !_analyzer->Traits().explicit_pos,
                               _track_offset ? &_offs_start : nullptr,
                               _track_offset ? &_offs_end : nullptr,
                               data_durable ? data : duckdb::string_t{});
      if (!_analyzer->Fill(data, doc_limits::min(), scratch.writer,
    ...
    const duckdb::string_t& Term() const noexcept { return _term; }
  ...
  using pipeline_t = std::vector<SubAnalyzer>;
  ...
  class ChainSink final : public TokenConsumer {
   public:
    ChainSink() { writer.Bind(*this, nullptr); }

    void Bind(TokenSink& out, TokenLayout out_layout,
              std::span<const ChainStage> stages,
              const SolrSynonymsTokenizer* solr,
              const WordnetSynonymsTokenizer* wordnet, bool producer_dense,
              bool track_offset) {
      _out = &out;
      ...
      _solr = solr;
      _wordnet = wordnet;
      _producer_dense = producer_dense;
   ...
   private:
    ...
    bool ApplyStages(const duckdb::string_t*& cur, duckdb::string_t& slot);
  ...
  template<TokenLayout Layout>
  void FillValue(TokenSink& sink, duckdb::string_t raw);
  template<TokenLayout Layout>
  bool FillFast(duckdb::string_t value, TokenSink& sink);

  bool RewriteGatesPass(const duckdb::string_t& value) const noexcept;
