...
void SparseNGramTokenizer::FillHashes(FillCtx& ctx) {
...
bool SparseNGramTokenizer::Advance(FillCtx& ctx) {
  ...
  while (ctx.pending_out == _pending.data()) {
    if (ctx.pos < pos_end) {
      ...
      // the drain leaves at most one entry, so it cannot re-trigger
...
void SparseNGramTokenizer::StepAll(FillCtx& ctx, size_t i, uint32_t hash) {
...
void SparseNGramTokenizer::StepCovering(FillCtx& ctx, size_t i,
                                        uint32_t hash) {
  const HashAndPos p{hash, i};
  if (StackSize(ctx) > 1 &&
...
bool SparseNGramTokenizer::DoFill(duckdb::string_t raw, TokenSink& sink) {
  ...
  while (Advance(ctx)) {
    sink.EmitK<Layout>(ctx.pending_size, ctx.data.data(), [&](size_t j) {
      const uint64_t entry = _pending[j];
      return Offs{static_cast<uint32_t>(entry),
