...
Tokenizer::ptr Make(Finder&& finder) {
  return std::visit(
    []<typename F>(F&& f) -> Tokenizer::ptr {
      if constexpr (std::is_same_v<std::remove_cvref_t<F>, std::monostate>) {
        return nullptr;
      } else {
        return std::make_unique<SplitTokenizer<Owner, std::remove_cvref_t<F>>>(
          std::move(f));
      }
    },
    std::move(finder));
