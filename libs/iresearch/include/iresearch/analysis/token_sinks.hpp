...
class ValueTokens final : public TokenConsumer, public StoreSink {
 ...
 private:
  struct Absent {};
  using PosLane =
    std::conditional_t<L != TokenLayout::Terms, std::vector<uint32_t>, Absent>;
  using OffsLane = std::conditional_t<L == TokenLayout::TermsPosOffs,
  ...
  [[no_unique_address]] PosLane _pos;
  [[no_unique_address]] OffsLane _offs_start;
  [[no_unique_address]] OffsLane _offs_end;
  bstring _store;
