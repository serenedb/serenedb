...
NormColumnReader::NormColumnReader(field_id id, std::string name,
  ...
  for (size_t rg = 0; rg < _pointers.size(); ++rg) {
    ...
    // ReadData returns a stable pointer when the backend exposes one
    // (mmap); fall back to a heap copy via ReadBytes otherwise.
    const auto* ptr = in.ReadData(p.file_offset, byte_count);
    if (ptr) {
      _spans[rg] = std::span<const byte_type>{ptr, byte_count};
    } else {
      _owned[rg].resize(byte_count);
      in.ReadBytes(p.file_offset, _owned[rg].data(), byte_count);
      _spans[rg] = std::span<const byte_type>{_owned[rg].data(), byte_count};
    }
  }
}
