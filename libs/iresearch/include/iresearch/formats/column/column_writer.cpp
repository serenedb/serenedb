...
void CaptureSegment(duckdb::ColumnSegment& segment, duckdb::idx_t segment_size,
  ...
  m.compression_type = segment.GetCompressionFunction().type;
...
void SliceChunks(std::vector<WriteChunk>& out, duckdb::Vector& vec,
  ...
  while (off < count) {
    ...
    if (off == 0) {
      ...
      duckdb::FlatVector::SetSize(owned, take);
      out.push_back(WriteChunk{std::move(owned), take});
    }
    off += take;
  }
}
...
void EmitEmptyValidity(const duckdb::LogicalType& validity_type,
                       uint64_t row_count, std::vector<ColumnBlockMeta>& sink) {
  ColumnBlockMeta m{duckdb::BaseStatistics::CreateEmpty(validity_type)};
  m.tuple_count = row_count;
  m.compression_type = duckdb::CompressionType::COMPRESSION_EMPTY;
  sink.push_back(std::move(m));
}
...
void ColumnWriter::SealValidity(std::span<WriteChunk> chunks,
  ...
  if (valid == row_count) {
    EmitEmptyValidity(validity_type, row_count, sink);
    return;
  }
  duckdb::unique_ptr<duckdb::AnalyzeState> state;
...
void ColumnWriter::SealArray(const duckdb::LogicalType& type,
  ...
  for (auto& c : chunks) {
    ...
    SliceChunks(elem_chunks, child, c.count * array_size);
  }
...
void ColumnWriter::SealList(const duckdb::LogicalType& type,
  ...
  for (auto& c : chunks) {
    ...
    SliceChunks(elem_chunks, child, static_cast<duckdb::idx_t>(chunk_elems));
  }
  seal_offsets();
...
void ColumnWriter::SealVariant(const duckdb::LogicalType& type,
  ...
  if (!should_shred) {
    layout.shred_state = VariantShredState::Unshredded;
  ...
  layout.shred_state = VariantShredState::Partial;
...
void ColumnWriter::SealColumn(const duckdb::LogicalType& type,
  ...
  if (nulls_covered_by_data) {
    EmitEmptyValidity(duckdb::LogicalType{duckdb::LogicalTypeId::VALIDITY},
                      row_count, meta.validity);
    return;
  }
  SealValidity(chunks, row_count, meta.validity);
}
...
ColumnWriter::ColumnWriter(ColWriter& owner, field_id id,
  ...
  : _owner{&owner},
    ...
    _forced{forced},
    _null_pad{_type, STANDARD_VECTOR_SIZE} {
  ...
  if (!_is_nested) {
    ...
    duckdb::FlatVector::ValidityMutable(_null_pad).SetAllInvalid(
