...
struct TermReader : public AttributeProvider {
  ...
  // The terms this acceptor accepts, with the acceptor driven *inside* the
  // dictionary walk: a sub-block whose prefix it cannot extend is skipped
  // without being read, and a block's prefix is stepped once for all its keys.
  // One overload per concrete acceptor, so the implementation templates over it
  // and a term still costs a single virtual call -- `SeekTermIterator::next()`.
  // Filtering from outside instead costs a full walk plus a re-step of every
  // key, which is why the overloads exist rather than a predicate on top.
  virtual SeekTermIterator::ptr iterator(const LevenshteinAcceptor&) const = 0;
  virtual SeekTermIterator::ptr iterator(const RegexpAcceptor&) const = 0;
