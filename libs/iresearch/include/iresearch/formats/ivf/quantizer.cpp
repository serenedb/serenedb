...
class PanoramaQuantizerReader final : public QuantizerReader {
 ...
 private:
  ...
  void ScoreBatch(const byte_type* p, size_t len, score_t score, score_t* out) {
    ...
    const float threshold = PanoramaThreshold(score, M);
...
class TurboQuantizerWriter final : public QuantizerWriter {
 public:
  ...
  PayloadBlockSetting BlockSetting() const noexcept final {
    return {.group_size = kFastScanBbs, .record_size = _lay.record_size};
  }
  ...
  uint32_t ScanCostBytes() const noexcept final {
    return _lay.record_size + (_lay.nsq1 + _lay.nsq2) *
                                static_cast<uint32_t>(kFastScanKsub) /
                                static_cast<uint32_t>(kFastScanBbs);
  }
...
class TurboQuantizerReader final : public QuantizerReader {
 ...
 private:
  ...
  void ScoreGroup(const byte_type* codes, score_t threshold, score_t* out) {
    ...
    Accumulate(codes, _cb->Lut1(), _cb->Chunks1(), _cb->A1(), _cb->B1());
    for (size_t i = 0; i < kFastScanBbs; ++i) {
    ...
    const auto* gammas =
      reinterpret_cast<const float*>(codes + _lay.GammaOffset());
    ...
    Accumulate(codes + _lay.group1_bytes, _cb->Lut2(), _cb->Chunks2(),
               _cb->A2(), _cb->B2());
...
std::unique_ptr<QuantizerWriter> MakeQuantizerWriter(
  ...
  switch (quant) {
    ...
    case VectorQuantization::TQMse: {
      ...
      SDB_ASSERT(qtype);
      return MakeWriterWithMetric<TurboQuantizerWriter>(metric, d, full,
                                                        nb_bits, *qtype);
    }
  ...
  switch (quant) {
    ...
    case VectorQuantization::TQMse:
      return MakeStatsWithMetric<TurboQuantizerStats>(
        metric, d, quant == VectorQuantization::TQ, stats);
    case VectorQuantization::RaBitQ:
