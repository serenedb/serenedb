...
class PostingsWriterBase : public PostingsWriter {
 ...
 protected:
  ...
  // Lanes the current field's payload stream has handed out: where the next
  // term's first document sits.
  uint64_t _pay_lanes{};
...
void PostingsWriterImpl<FormatTraits>::Write(DocIterator& docs,
  ...
  while (true) {
    ...
    if (doc_limits::valid(_doc.last) && _doc.Empty()) {
      // The block boundary: record a skip entry before the document that
  ...
  // codes) into the field's ".pay" stream. That stream is continuous across
  // terms -- its fast-scan packs straddle term boundaries -- so a term is
  // located by the stream ordinal ("lane") of its first document rather than
  // by a byte offset.
  if (has_payload) {
    SDB_ASSERT(_pay_out && _term_pay);
    meta.pay_start = _pay_lanes;
    _pay_lanes += _term_docs.size();
