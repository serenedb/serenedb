...
class ScatteredField : util::Noncopyable {
 public:
  ...
  bytes_view TermAt(size_t rank) const noexcept {
    const auto& term =
      _field->Dictionary().Entries()[_s->ranked[RankAt(rank)].id].term;
    return AsBytesView(term);
  }
  ...
  doc_id_t InlineDocAt(uint64_t k) const noexcept {
    SDB_ASSERT(_all_inline);
    return _field->InlineDocs()[_s->ranked[k].id];
  }

  doc_id_t DocAt(uint64_t k) const noexcept {
    return _all_inline ? InlineDocAt(k) : ScatterView{_s->docs.data()}[k];
  }

  ScatterView Docs() const noexcept {
    SDB_ASSERT(!_all_inline);
    return ScatterView{_s->docs.data()};
  }
...
class ColumnarDocIterator final : public DocIterator {
 public:
  void ResetField(const ScatteredField& scattered) {
    ...
    _all_inline = scattered.AllInline();
    _freq.value = 0;
  ...
  doc_id_t advance() final {
    ...
    doc_id_t doc;
    if (_all_inline) {
      doc = _scattered->InlineDocAt(_cur);
      do {
        ++_cur;
        ++freq;
      } while (_cur != _end && _scattered->InlineDocAt(_cur) == doc);
    } else {
