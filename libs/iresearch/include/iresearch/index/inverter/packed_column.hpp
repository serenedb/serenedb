...
class PackedU32Column : util::Noncopyable {
 ...
 public:
  ...
  uint32_t PushNDelta(const uint32_t* values, size_t count, uint32_t base,
    ...
    PushNImpl(count, [&](uint32_t* dst, size_t n) {
      for (size_t i = 0; i < n; ++i) {
        const uint32_t abs = base + values[i];
        dst[i] = abs - prev;
        prev = abs;
      }
      values += n;
  ...
  size_t Memory() const noexcept {
    return _packed.capacity() * sizeof(PackedBlock);
  }
  ...
  struct PackStats {
    ...
    std::array<uint32_t, 33> bits_hist{};
  };

  PackStats Stats() const noexcept {
    PackStats s;
    s.values = Size();
    s.sealed_blocks = _packed.size();
    for (const auto& blk : _packed) {
      ++s.bits_hist[blk.bits];
      if (blk.bits) {
        s.packed_bytes += packed::BytesRequired32(kBlockValues, blk.bits);
      }
    }
    return s;
  }
 ...
 private:
  ...
  ManagedVector<PackedBlock> _packed;
