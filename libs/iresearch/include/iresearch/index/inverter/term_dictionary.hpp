...
// A string_t viewed as its raw bytes (no copy), for the bytes_view-keyed
// surfaces of the inverter (term hashing, reader TermAt). Takes const& BY
// NECESSITY: the result aliases the argument's bytes, and an inline
// (<= 12B) handle carries them in the object itself -- a by-value param
// would return a view into a dead stack slot.
...
class TermDictionary : util::Noncopyable {
 public:
  ...
  struct Entry : HashedTerm {
    Entry(duckdb::string_t term, size_t hash) noexcept
      : HashedTerm{term, hash} {}
  };
  ...
  IRS_FORCE_INLINE void Insert(std::span<const duckdb::string_t> terms,
                               std::span<uint64_t, N> hash_scratch,
                               std::span<uint32_t, N> out_ids) {
    SDB_ASSERT(terms.size() <= hash_scratch.size());
    SDB_ASSERT(terms.size() <= out_ids.size());
    if (Size() < kFusedProbeThreshold) {
      for (size_t j = 0; j < terms.size(); ++j) {
        out_ids[j] = Insert(terms[j]);
      }
    } else {
      InsertPipelined(terms, hash_scratch.data(), out_ids.data());
    }
  }
 ...
 private:
  ...
  // Dictionaries at or below this many distinct terms stay L2-resident, so
  ...
  IRS_FORCE_INLINE void InsertPipelined(std::span<const duckdb::string_t> terms,
    ...
    InsertHashed(terms, {hash_scratch, n}, out_ids);
  }
