...
struct DocIterator : AttributeProvider {
  ...
  virtual uint32_t Collect(const ScoreFunction& scorer,
                           ColumnCollector& columns, size_t offset,
                           std::span<std::pair<doc_id_t, score_t>> docs) {
    SDB_ASSERT(kScoreBlock <= docs.size());
    return Collect(*this, scorer, columns, offset, docs);
  }
 ...
 protected:
  ...
  static uint32_t Collect(Iterator& it, const ScoreFunction& scorer,
                          ColumnCollector& columns, size_t offset,
                          std::span<std::pair<doc_id_t, score_t>, N> docs) {
    SDB_ASSERT(kScoreBlock <= docs.size());
    ...
    columns.Collect(docs.subspan(i));
    scorer.Score2(docs.data(), i);
    return static_cast<uint32_t>(i);
  }
