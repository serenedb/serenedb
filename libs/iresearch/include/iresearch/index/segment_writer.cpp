...
[[nodiscard]] DocMap SegmentWriter::flush(IndexSegment& segment,
  ...
  // reads in FlushFields go through `PersistedNormReader` over the
  // disk-backed payload. SegmentWriter::norms(field_id) consults
...
void SegmentWriter::reset(const SegmentMeta& meta) {
  ...
  // a Norm feature open a NormColumnWriter on the columnstore Writer.
  _fields.SetColumnstore(_columnstore.get());
