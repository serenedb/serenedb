...
class BlockDisjunction : public DocIterator, private ScoreCtx {
 ...
 private:
  ...
  bool Refill() {
    ...
    [[maybe_unused]] auto min_match_count = 0;
    [[maybe_unused]] uint32_t* matches = nullptr;
    [[maybe_unused]] score_t* score_window = nullptr;
    if constexpr (Traits::kMinMatch) {
      matches = _match_buf.data();
      min_match_count = _match_buf.min_match_count();
    }
    if constexpr (kHasScore) {
    ...
    do {
      ...
      VisitAndPurge([&](auto& it) mutable {
        ...
        const auto [doc, has_hits] =
          it.CollectBlock(_doc_base, _max, MergeType, &it.score(), _mask,
                          score_window, matches, min_match_count);
