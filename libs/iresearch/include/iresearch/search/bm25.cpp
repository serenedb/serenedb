...
void Bm1Boost(Iterator res, size_t n, const score_t* IRS_RESTRICT boost,
...
template<bool HasBoost, typename Iterator>
void Bm15(Iterator res, size_t n, const uint32_t* IRS_RESTRICT freq,
...
template<bool HasBoost, typename Iterator>
void Bm25(Iterator res, size_t n, const uint32_t* IRS_RESTRICT freq,
    ...
    if constexpr (HasFilterBoost) {
      return ScoreFunction::Make<Ctx>(
        [](irs::ScoreCtx* ctx, irs::score_t* res, size_t n) noexcept {
          ...
          Bm1Boost<score_t * IRS_RESTRICT>(res, n, state.filter_boost,
                                           state.num);
        },
  ...
  static auto Make(Args&&... args) {
    return ScoreFunction::Make<Ctx>(
      [](irs::ScoreCtx* ctx, irs::score_t* res, size_t n) noexcept {
        ...
        SDB_ASSERT(ctx);
        auto& state = *static_cast<Ctx*>(ctx);
        Bm15<HasFilterBoost, score_t * IRS_RESTRICT>(
          res, n, state.freq, state.filter_boost, state.num, state.norm_const);
      },
      ScoreFunction::NoopMin, std::forward<Args>(args)...);
  }
};
  ...
  static auto Make(Args&&... args) {
    return ScoreFunction::Make<Ctx>(
      [](ScoreCtx* ctx, score_t* res, size_t n) noexcept {
        ...
        SDB_ASSERT(ctx);
        auto& state = *static_cast<Ctx*>(ctx);
        Bm25<HasFilterBoost, score_t * IRS_RESTRICT>(
          res, Single ? 1 : n, state.freq, state.norm, state.filter_boost,
