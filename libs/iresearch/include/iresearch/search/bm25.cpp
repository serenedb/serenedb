...
struct BM1Context : public irs::ScoreCtx {
  BM1Context(float_t k, score_t boost, const BM25Stats& stats,
             uint16_t score_block, const score_t* fb = nullptr) noexcept
    : filter_boost{fb},
...
struct BM15Context : public BM1Context {
  BM15Context(float_t k, score_t boost, const BM25Stats& stats,
              uint16_t score_block, const uint32_t* freq,
              const score_t* fb = nullptr) noexcept
    : BM1Context{k, boost, stats, score_block, fb},
...
struct BM25Context : public BM15Context {
  BM25Context(float_t k, score_t boost, const BM25Stats& stats,
              uint16_t score_block, const uint32_t* freq, const uint32_t* norm,
              const score_t* filter_boost = nullptr) noexcept
    : BM15Context{k, boost, stats, score_block, freq, filter_boost},
      norm{norm},
...
template<bool HasBoost>
void Bm25(score_t* IRS_RESTRICT res, size_t n,
          const uint32_t* IRS_RESTRICT freq, const uint32_t* IRS_RESTRICT norm,
          [[maybe_unused]] const score_t* IRS_RESTRICT boost, float_t num,
  ...
  static auto Make(Args&&... args) {
    return ScoreFunction::Make<Ctx>(
      [](irs::ScoreCtx* ctx, irs::score_t* res, size_t n) noexcept {
        ...
        SDB_ASSERT(ctx);
        auto& state = *static_cast<Ctx*>(ctx);
        Bm15<HasFilterBoost>(res, n, state.freq, state.filter_boost, state.num,
                             state.norm_const);
      },
      ScoreFunction::NoopMin, std::forward<Args>(args)...);
  }
  ...
  static auto Make(Args&&... args) {
    return ScoreFunction::Make<Ctx>(
      [](ScoreCtx* ctx, score_t* res, size_t n) noexcept {
        ...
        SDB_ASSERT(ctx);
        auto& state = *static_cast<Ctx*>(ctx);
        Bm25<HasFilterBoost>(res, n, state.freq, state.norm, state.filter_boost,
                             state.num, state.norm_length, state.norm_const);
      },
      ScoreFunction::NoopMin, std::forward<Args>(args)...);
  }
};
...
ScoreFunction BM25::PrepareScorer(const ScoreContext& ctx) const {
  ...
  if (IsBM15()) {
    return MakeScoreFunction<BM15Context>(filter_boost, _k, ctx.boost, *stats,
                                          ctx.score_block, freq->value);
  }

  // TODO(gnusi): do we really need this case when norm is in place?
  auto* norm = [&] {
  ...
  return MakeScoreFunction<BM25Context>(filter_boost, _k, ctx.boost, *stats,
                                        ctx.score_block, freq->value, norm);
}
