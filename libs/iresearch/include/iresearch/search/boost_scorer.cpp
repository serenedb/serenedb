...
struct VolatileBoostScoreCtx final : ScoreCtx {
  VolatileBoostScoreCtx(uint16_t score_block, const score_t* volatile_boost,
  ...
  const uint16_t score_block;
};

void Impl(score_t* IRS_RESTRICT res, size_t n,
...
ScoreFunction BoostScore::PrepareScorer(const ScoreContext& ctx) const {
  ...
  return ScoreFunction::Make<VolatileBoostScoreCtx>(
    ...
    ScoreFunction::NoopMin, ctx.score_block, volatile_boost->value, ctx.boost);
}
