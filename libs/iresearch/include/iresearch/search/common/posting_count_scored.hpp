...
class PostingCountScored : public PostingLeaf<InputType, kWindowScoredShape> {
  static_assert(
  ...
  static constexpr bool kCounts = true;

 public:
  PostingCountScored() = default;
    ...
    if (_left_in_leaf != 0) {
      ...
      if (begin == end) {
        ...
        const auto* const it = this->template AddUntil<MergeType, kCounts>(
          begin, end, min, max, counts, mask, window);
        _left_in_leaf = static_cast<uint32_t>(end - it);
        return _doc = *it;
      } else {
        this->template AddWhole<MergeType, kCounts>(begin, end, min, counts,
                                                    mask, window);
        _left_in_leaf = 0;
      }
    }
