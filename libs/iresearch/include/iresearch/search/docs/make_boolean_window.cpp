...
Root::ptr MakeWindowNegation(
  ...
  return MakeWindow<fill::AllDocs, utils::Empty, utils::Empty, Excludes>(
...
Root::ptr MakeWindowExclusion(
  ...
  if (terms.empty() && filters.empty()) {
    return MakeWindowNegation(exclude_terms, exclude_filters, segment, ctx);
  }
  ...
  if (min_match > search::kBitplaneMaxMatch && rest.empty()) {
    ...
    return ResolveInput(in, [&]<typename Input> -> Root::ptr {
      ...
      const auto init = [&](Leaf& leaf, size_t i) {
        const auto& own = search::FieldOf(terms[i], nullptr);
