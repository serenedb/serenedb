...
Scorer::ptr MakeJson(std::string_view /*args*/) {
  return std::make_unique<DocumentLength>();
}
...
struct DocumentLengthScore : public ScoreOperator {
  DocumentLengthScore(score_t boost, const uint32_t* norm,
...
ScoreFunction DocumentLength::PrepareScorer(const ScoreContext& ctx) const {
  ...
  return ResolveBool(filter_boost != nullptr, [&]<bool HasBoost>() {
    return ScoreFunction::Make<DocumentLengthScore<HasBoost>>(ctx.boost, norm,
                                                              filter_boost);
  });
}

void DocumentLength::init() {
  REGISTER_SCORER_JSON(DocumentLength, MakeJson);
  REGISTER_SCORER_VPACK(DocumentLength, MakeVPack);
}
