  ...
  class FilterRule { // think about managed_ptr like in Filter::Query
   public:
    ...
    struct FilterRuleOptions {
      bool has_other_filters = false; // for cases when current filter has filters which differ from desired_filter_types
    };

    struct FilterRuleResult {
      Filter::ptr filter = nullptr; // return filter value - could be the same or new
    ...
    inline virtual FilterRuleResult ApplyBoolean(And& current_filter, std::unordered_map<TypeInfo::type_id, std::vector<Filter::ptr>>&& sub_filters, FilterRuleOptions options) const {
      return FilterRuleResult{};
    };

    inline virtual FilterRuleResult ApplyBoolean(Or& current_filter, std::unordered_map<TypeInfo::type_id, std::vector<Filter::ptr>>&& sub_filters, FilterRuleOptions options) const {
      return FilterRuleResult{};
    };

    inline virtual FilterRuleResult ApplyNot(Not& current_filter, Filter::ptr sub_filter, FilterRuleOptions options) const {
      return FilterRuleResult{};
    };
   ...
   private:
    ...
    bool _works_with_empty_filters = false; // think about that, maybe want to have rule with zero suitable subfilters
  };
...
class NotFilterRule : public FilterRulesConstructor::FilterRule {
 public:
  ...
  virtual FilterRuleResult ApplyNot(Not& current_filter, Filter::ptr sub_filter, FilterRuleOptions options) const override {
    ...
    return FilterRuleResult{.filter = std::move(sub_sub_filter), .is_new_filter = true};
  }
 private:
};
