...
class GeoIterator : public irs::DocIterator {
 ...
 public:
  GeoIterator(DocIterator::ptr&& approx, DocIterator::ptr&& column_it,
              ...
              ColumnCollector* collector, const irs::TermReader& field,
    ...
    if (!order.empty()) {
      ...
      CompileScore(score, order.buckets(), reader, collector, field,
...
irs::DocIterator::ptr MakeIterator(
  ...
  const irs::SubReader& reader, irs::ColumnCollector* collector,
  const irs::TermReader& field, const irs::byte_type* query_stats,
  const irs::Scorers& order, irs::score_t boost, Parser& parser,
  Acceptor& acceptor) {
  ...
  return irs::memory::make_managed<GeoIterator<Parser, Acceptor>>(
    ...
    std::move(column_it), parser, acceptor, reader, collector, field,
    query_stats, order, boost);
}
 ...
 public:
  ...
  irs::DocIterator::ptr execute(const irs::ExecutionContext& ctx) const final {
    ...
    return MakeIterator(std::move(itrs), std::move(column_it), segment,
                        ctx.collector, *state->reader, _stats.c_str(),
                        ctx.scorers, Boost(), _parser, _acceptor);
  }
