...
Root::ptr MakeFixedPhraseSlop(const FixedPhraseQuery& query,
  ...
  if (const auto value =
    ...
    if (ctx.table != nullptr) {
      return irs::detail::MakeFixedPhraseOf<irs::detail::PhraseMatch::Slop,
  ...
  if (ctx.table != nullptr) {
    return irs::detail::MakeFixedPhraseOf<irs::detail::PhraseMatch::Slop, FilteredWalk,
                                     Root::ptr, true, lead::TwoPhaseScored>(
      query, ctx.table, ctx.fetcher, query.Segment(), *query.state.reader,
      args);
  }
  return irs::detail::MakeFixedPhraseOf<irs::detail::PhraseMatch::Slop, PlainWalk,
                                   Root::ptr, true, lead::TwoPhaseScored>(
