...
class MaxScoreIterator : public DocIterator {
 ...
 private:
  ...
  // A view of the essential's decoded block, shaped like the containers the
  // non-essential pass compacts. Both halves share one size so a compaction
  // through either keeps them in step.
  template<typename T>
  struct BlockView {
    T* base;
    size_t count;
