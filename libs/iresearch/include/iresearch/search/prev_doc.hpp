...
class PrevDocAttr : public Attribute {
 public:
  using prev_doc_f = doc_id_t (*)(const void*);  // NOLINT

  static constexpr std::string_view type_name() noexcept { return "prev_doc"; }

  constexpr explicit operator bool() const noexcept { return nullptr != _func; }
  ...
  doc_id_t operator()() const {
    SDB_ASSERT(static_cast<bool>(*this));
    return _func(_ctx);
  }
