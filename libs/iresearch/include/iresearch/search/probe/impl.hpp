...
class Impl : public Node {
 public:
  ...
  IRS_FORCE_INLINE doc_id_t Probe(doc_id_t target) final {
    return _leaf.Probe(target);
  }

  IRS_FORCE_INLINE void FetchScoreArgs(uint32_t slot) final {
    if constexpr (ScoredType<L>) {
      _leaf.FetchScoreArgs(slot);
    }
  }

  IRS_FORCE_INLINE ScoreFunction PrepareScore() final {
    if constexpr (ScoredType<L>) {
...
class Erased {
 public:
  ...
  IRS_FORCE_INLINE ScoreFunction PrepareScore() {
    return _node->PrepareScore();
  }
