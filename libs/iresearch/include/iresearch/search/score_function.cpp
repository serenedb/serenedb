...
void ConstantScore2(ScoreCtx* ctx, std::pair<doc_id_t, score_t>* res,
                    size_t n) noexcept {
  SDB_ASSERT(res);
  auto state = std::bit_cast<ConstantCtx>(ctx);
...
void ScoreFunction::DefaultScore2(ScoreCtx* ctx,
                                  std::pair<doc_id_t, score_t>* res,
                                  size_t n) noexcept {
  SDB_ASSERT(res != nullptr);
  for (auto& [_, score] : std::span{res, n}) {
    score = 0;
...
ScoreFunction ScoreFunction::Constant(score_t value) noexcept {
  ...
  return ScoreFunction{std::bit_cast<ScoreCtx*>(ctx), ConstantScore,
                       ConstantScore2, NoopMin};
}

ScoreFunction ScoreFunction::Default() {
  return ScoreFunction{nullptr, DefaultScore, DefaultScore2, NoopMin};
}
