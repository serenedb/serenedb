...
Root::ptr MakeVariadicPhraseSlop(const VariadicPhraseQuery& query,
  ...
  if (const auto value =
    ...
    if (ctx.table != nullptr) {
      return search::MakeVariadicPhraseOf<search::PhraseMatch::Slop,
  ...
  if (ctx.table != nullptr) {
    return search::MakeVariadicPhraseOf<search::PhraseMatch::Slop, FilteredWalk,
                                        Root::ptr, true, lead::TwoPhaseScored>(
      query, ctx.table, ctx.fetcher, query.Segment(), *query.state.reader,
      args);
  }
  return search::MakeVariadicPhraseOf<search::PhraseMatch::Slop, PlainWalk,
                                      Root::ptr, true, lead::TwoPhaseScored>(
