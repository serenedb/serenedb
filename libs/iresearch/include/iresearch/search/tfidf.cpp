...
IRS_FORCE_INLINE float_t Tfidf(uint32_t freq, float_t idf) noexcept {
  ...
  return std::sqrt(freq) * idf;
}

struct TFIDFContext : public ScoreCtx {
  TFIDFContext(uint16_t score_block, const uint32_t* norm, score_t boost,
               TFIDFStats idf, const uint32_t* freq,
...
template<bool HasNorm, bool HasBoost>
...
struct MakeScoreFunctionImpl<TFIDFContextImpl<HasNorm>> {
  ...
  template<bool HasBoost, typename... Args>
  static auto Make(Args&&... args) {
    return ScoreFunction::Make<Ctx>(
      [](ScoreCtx* ctx, score_t* res, size_t n) noexcept {
        ...
        TfIdf<HasNorm, HasBoost>(res, n, state.freq, state.norm,
                                 state.filter_boost, state.idf);
      },
...
ScoreFunction TFIDF::PrepareScorer(const ScoreContext& ctx) const {
  ...
  if (_normalize) {
    // TODO(gnusi): do we really need this case when norms are in place?
  ...
  return ResolveBool(norm != nullptr, [&]<bool HasNorms>() {
    ...
    return MakeScoreFunction<TFIDFContextImpl<HasNorms>>(
      filter_boost, ctx.score_block, norm, ctx.boost, *stats, freq->value);
  });
}
