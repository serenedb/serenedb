...
/// true disables normalizing resulting similarity by length of longest string
/// and result is evaluated based strictly on target length (search-like
/// semantics). If search_semantics is false there is no difference which string
/// pass as target. First characters affixing currently is not implemented
...
float_t ngram_similarity(const T* target, size_t target_size, const T* src,
                         size_t src_size, size_t ngram_size) {
  if (ngram_size == 0) {
    return 0.f;
  }
  ...
  const T* t_ngram_start_end = target + target_size - ngram_size +
                               1;  // end() analog for target ngram start
  ...
  for (; t_ngram_start != t_ngram_start_end; ++t_ngram_start) {
    ...
    for (; s_ngram_start != s_ngram_start_end; ++s_ngram_start, ++s_ngram_idx) {
      ...
      for (const T *l = t_ngram_start, *r = s_ngram_start;
           l != t_ngram_end && r != rhs_ngram_end; ++l, ++r) {
        if constexpr (search_semantics) {
          if (*l != *r) {
            similarity = 0;
            break;
