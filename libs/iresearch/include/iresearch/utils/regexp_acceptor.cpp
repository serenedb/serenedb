...
// model widens `.` in exactly that way only when the class is *full*
// (`MakeAny`); a partial class expands strictly, range by range. Splitting the
// range below Runemax into two class nodes is what keeps RE2 off the wide path,
// and it is a narrowing: RE2 then rejects the ill-formed sequences it accepts
// today.
...
// iresearch's UTF-8 model byte by byte -- the loose one, matching `MakeAny` /
// `MakeAll`, overlongs and surrogates included. A wildcard pattern is arbitrary
// bytes, which is why it cannot go through a regexp source string at all.
...
void RegexpAcceptor::Build(bytes_view pattern, RegexpSyntax syntax,
  ...
  _live.assign(kLiveWords, 0);
  _loop.assign(kLiveWords, 0);
  _accept.assign(1, 0);
  ...
  _live.assign(size_t{states + 1} * kLiveWords, 0);
  _loop.assign(size_t{states + 1} * kLiveWords, 0);
  ...
  for (uint32_t s = 0; s != states; ++s) {
    ...
    auto* live = _live.data() + size_t{s + 1} * kLiveWords;
    auto* loop = _loop.data() + size_t{s + 1} * kLiveWords;
    for (size_t label = 0; label != _bytemap.size(); ++label) {
      const State target = _next[base + _bytemap[label]];
      if (target != kDead) {
        live[bitset::word(label)] |= bitset::word_t{1} << bitset::bit(label);
        if (target == base) {
