...
```
<signal>/<case>.json            an Export<Signal>ServiceRequest
<signal>/<case>.pb              the same request, binary
<signal>/<case>.expected.jsonl  the rows it should produce
```

`.pb` is generated from `.json` by `scripts/otel/fixtures.py`, using
...
`upstream.json` are vendored from
...
Consumed by `tests/drivers/python/test_otel_api.py` and
