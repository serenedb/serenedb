...
def dedup_blocks(props):
    padded = bytes(props) + bytes(STAGE1_CODEPOINTS - len(props))
    stage1 = []
    stage2 = []
    seen = {}
    for block_index in range(STAGE1_CODEPOINTS >> 8):
        block = padded[block_index << 8:(block_index + 1) << 8]
...
def emit_word_break_tables(path, stage1, stage2, ucd_version):
    ...
    out.append(
        "inline constexpr uint32_t kWbMaxCp = kWbStage1.size() * 256 - 1;\n")
    out.append(
        "IRS_FORCE_INLINE constexpr uint8_t WbLookup(uint32_t cp) noexcept {\n"
        "  SDB_ASSERT(cp <= kWbMaxCp);\n"
        "  return kWbStage2[size_t{kWbStage1[cp >> 8]} * 256 + (cp & 0xFF)];\n"
        "}\n")
    out.append("static_assert(WbLookup(kWbMaxCp) == kOther);\n")
    out.append("}  // namespace irs::analysis::words")
