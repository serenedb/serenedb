...
if [ -d cluster-init ]; then
	...
	mkdir -p cluster || {
		echo "failed to create cluster directory"
		exit 1
	}
...
if [ -z "$JWT_SECRET" ]; then
	...
	export AUTHORIZATION_HEADER="Authorization: bearer $(jwtgen -a HS256 -s $JWT_SECRET -c 'iss=serenedb' -c 'server_id=setup')"
fi
...
if [ ! -z "$INTERACTIVE_MODE" ]; then
	if [ "$INTERACTIVE_MODE" == "C" ]; then
		SERENED="${BUILD}/bin/serened "
		CO_SERENED="$XTERM $XTERMOPTIONS ${BUILD}/bin/serened --console"
		echo "Starting one coordinator in terminal with --console"
	elif [ "$INTERACTIVE_MODE" == "R" ]; then
		SERENED="rr ${BUILD}/bin/serened"
		CO_SERENED="$SERENED"
...
for p in $(seq $CO_BASE $PORTTOPCO); do
	if [ -z "$JWT_SECRET" ]; then
		echo "   ${BUILD}/bin/serenesh --server.endpoint $TRANSPORT://$ADDRESS:$p"
	else
		echo "   ${BUILD}/bin/serenesh --server.endpoint $TRANSPORT://$ADDRESS:$p --server.jwt-secret-keyfile cluster/jwt.secret"
	fi
done
