...
run_one() {
	...
	pgbench "${conn[@]}" -n -f "${sqlfile}" -M "${mode}" \
		...
		>"${pgb_log}" 2>&1 &
	local pgb_pid=$!
	# Sample the server for slightly less than the pgbench window so perf
	# stops while load is still flowing (clean steady-state counters).
...
echo
echo "=== benchmarking (each server isolated: start -> bench -> kill) ==="
...
awk -F'\t' '
{
  key=$1; sub(/^old_/,"",key); sub(/^new_/,"",key);
  side=($1 ~ /^old_/) ? "old" : "new";
