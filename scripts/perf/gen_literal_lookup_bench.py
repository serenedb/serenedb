...
def build():
    ...
    for size in SIZES:
        for dist in DISTS:
            ...
            parts.append(
                f"#ifdef SDB_ENABLE_TRIVIAL_BIMAP\n"
                f"constexpr auto kTriv_{tag} = duckdb::MakeTrivialSet<kLit_{tag}>();\n"
                f"#endif  // SDB_ENABLE_TRIVIAL_BIMAP")
    ...
    # ---- emit cells ----
    def cell(fn, helper, struct_expr, q, guard=None):
        nm = f"{fn}"
        body = (
    ...
    for size in SIZES:
        for dist in DISTS:
            ...
            cell(f"cs_trivial_{tag}", "RunTrivCs", f"kTriv_{tag}", qcs,
                 guard="SDB_ENABLE_TRIVIAL_BIMAP")
            # --- CI ---
