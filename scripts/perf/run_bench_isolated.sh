...
# PERF_DB selects which comparison pg-wire engines to include as columns (comma
# list of old/pg/crdb/cedar/risingwave/clickhouse, or "all"; "old" is the last
...
ALL_ENGINES=(old pg crdb cedar risingwave)
BASELINES="${ROOT}/scripts/perf/results/baselines"

# Resolve PERF_DB into a list of engine names (uniform; no per-engine flags).
DB_RAW="${PERF_DB:-}"
SELECTED=()
...
# remeasure check for one engine: PERF_REMEASURE_DB list (or "all"). Returns 0
# when the engine should be refreshed.
remeasure_wanted() {
	local eng="$1" item
...
cleanup() {
	...
	if [[ -d "$CG" ]]; then
		# Demote the partition AND release the exclusive cpu claim before rmdir.
		# If cpuset.cpus.exclusive is left set, rmdir fails and the empty dir
		# keeps those cores exclusive -- the next run's cpuset.mems write then
		# fails with EINVAL ("cat: write error: Invalid argument").
