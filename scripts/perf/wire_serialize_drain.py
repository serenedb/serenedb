...
# Per-recv timeout so a server that stalls mid-response (e.g. an older release
# that mis-handles a format it doesn't fully support) becomes a recorded-0 cell
# instead of hanging the whole benchmark. Generous: a healthy drain receives a
# chunk every few ms; only a true stall waits this long.
TIMEOUT = float(os.environ.get("PERF_SER_TIMEOUT", "120"))

# Engine user/db/password from the bench env (PGPASSWORD for password-required
# engines like CedarDB); wire_auth.startup satisfies trust/cleartext/md5/SCRAM.
...
def connect():
    ...
    wire_auth.startup(sock, USER, DB, PASSWORD)
    if SETUP:
        q = SETUP.encode()
...
try:
    for rep in range(REPS):
        ...
        if err:
            break
        timings.append(secs)
except Exception as exc:  # timeout / eof / connect-refused / protocol error
    # Any failure (incl. a stalled server hitting the recv timeout) is a
    # recorded-0 cell, never a hang or a crash that aborts the bench.
    err = f"{type(exc).__name__}: {exc}"

if err or not timings:
    msg = str(err or "no timings").replace("\t", " ").replace("\n", " ").replace("\r", " ")
    print(f"{LABEL}\t0\t0\t0\tERROR: {msg}")
else:
