...
bool RoleClosure::Can(duckdb::CatalogType type,
  ...
  if (Owns(catalog::OwnerOf(perm))) {
    return true;
  }
  return AclCheckSorted(perm.acl, type, catalog::OwnerOf(perm), closure, need,
                        PrivMatch::All);
}
...
bool RoleClosure::CanColumns(const catalog::Permissions& perm,
  ...
  return !acls.empty() && std::ranges::all_of(acls, [&](catalog::AclView acl) {
    return ColumnGrants(acl, catalog::OwnerOf(perm), closure, need);
  });
}
