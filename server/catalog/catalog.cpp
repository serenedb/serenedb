...
// Walks `path` into nested STRUCTs starting at `depth` (where `path[depth]`
// names `type`) and applies the field op, returning the rewritten type in
// `out`. Sets `noop` when an `if_exists` short-circuit applies (Add of an
// existing field, or Drop/Rename of a missing field).
Result ApplyStructFieldOp(duckdb::LogicalType& out,
                          const duckdb::LogicalType& type,
                          Catalog::StructFieldOp op,
                          const std::vector<std::string>& path, size_t depth,
                          const std::string& field_name,
                          const duckdb::LogicalType& field_type, bool if_exists,
                          bool& noop) {
  if (type.id() != duckdb::LogicalTypeId::STRUCT) {
    return Result{ERROR_BAD_PARAMETER, "field \"", path[depth],
                  "\" is not a struct; ALTER ... FIELD requires a STRUCT"};
  }
  const auto& children = duckdb::StructType::GetChildTypes(type);
  duckdb::child_list_t<duckdb::LogicalType> next;
  next.reserve(children.size() + 1);

  const bool add = op == Catalog::StructFieldOp::Add;
  // For Add, `path` ends at the parent struct; for Drop/Rename the last path
  // element is the target field one level below the parent struct.
  const bool at_parent =
    add ? depth + 1 == path.size() : depth + 2 == path.size();

  if (at_parent) {
    if (add) {
      for (const auto& [n, t] : children) {
        if (duckdb::StringUtil::CIEquals(n, field_name)) {
          noop = if_exists;
          if (if_exists) {
            out = type;
            return {};
          }
          return Result{ERROR_SERVER_DUPLICATE_NAME, field_name};
        }
        next.emplace_back(n, t);
      }
      next.emplace_back(field_name, field_type);
      out = duckdb::LogicalType::STRUCT(std::move(next));
      return {};
    }
    const auto& target = path[depth + 1];
    bool found = false;
    for (const auto& [n, t] : children) {
      if (duckdb::StringUtil::CIEquals(n, target)) {
        found = true;
        if (op == Catalog::StructFieldOp::Drop) {
          continue;  // skipped -> removed
        }
        next.emplace_back(field_name, t);  // rename
      } else {
        next.emplace_back(n, t);
      }
    }
    if (!found) {
      ...
      return Result{ERROR_SERVER_ILLEGAL_NAME, target};
    }
    out = duckdb::LogicalType::STRUCT(std::move(next));
    return {};
  }

  const auto& descend = path[depth + 1];
  bool found = false;
  for (const auto& [n, t] : children) {
    if (!found && duckdb::StringUtil::CIEquals(n, descend)) {
      duckdb::LogicalType child_out;
      if (auto r = ApplyStructFieldOp(child_out, t, op, path, depth + 1,
                                      field_name, field_type, if_exists, noop);
          !r.ok()) {
        return r;
      }
      next.emplace_back(n, std::move(child_out));
      found = true;
    } else {
      next.emplace_back(n, t);
    }
  ...
  if (!found) {
    return Result{ERROR_SERVER_ILLEGAL_NAME, descend};
  }
  out = duckdb::LogicalType::STRUCT(std::move(next));
  return {};
}

// Reconstructs the dotted ALTER clause forwarded to the store engine, which
// re-encodes the data (e.g. `ADD COLUMN "s"."k" INTEGER`).
std::string BuildStructFieldClause(Catalog::StructFieldOp op,
                                   const std::vector<std::string>& path,
                                   const std::string& field_name,
                                   const duckdb::LogicalType& field_type) {
  std::string dotted;
  for (size_t i = 0; i < path.size(); ++i) {
    if (i != 0) {
      dotted += '.';
    }
    dotted += duckdb::KeywordHelper::WriteQuoted(path[i], '"');
  }
  switch (op) {
    case Catalog::StructFieldOp::Add:
      return absl::StrCat("ADD COLUMN ", dotted, ".",
                          duckdb::KeywordHelper::WriteQuoted(field_name, '"'),
                          " ", field_type.ToString());
    case Catalog::StructFieldOp::Drop:
      return absl::StrCat("DROP COLUMN ", dotted);
    case Catalog::StructFieldOp::Rename:
      return absl::StrCat("RENAME COLUMN ", dotted, " TO ",
                          duckdb::KeywordHelper::WriteQuoted(field_name, '"'));
  }
  return {};
}
...
Result Catalog::ChangeStructField(ObjectId database_id, std::string_view schema,
  ...
  auto table_ptr = basics::downCast<Table>(std::move(object));
  const auto& column = column_path[0];
  auto col_it = absl::c_find_if(table_ptr->Columns(), [&](const Column& c) {
    return c.GetName() == column;
  });
  if (col_it == table_ptr->Columns().end()) {
  ...
  // would leave them inconsistent. GetReferencedColumnIds covers columns
  // reached through an indexed expression too (e.g. inverted
  // ((s['id']::INTEGER))), so a sub-field index is caught. Reject and let the
  // user drop the index.
  ...
  bool noop = false;
  if (auto r = ApplyStructFieldOp(new_type, col_it->type, op, column_path,
                                  /*depth=*/0, field_name, field_type,
                                  if_exists, noop);
      !r.ok()) {
    return r;
  }
...
Result Catalog::AddPrimaryKey(ObjectId database_id, std::string_view schema,
  ...
  if (auto td = _snapshot->GetDependency<TableDependency>(*table_id);
    ...
    return Result{ERROR_BAD_PARAMETER,
                  "cannot add a primary key to table \"",
                  std::string{table},
                  "\" because index \"",
                  idx_name,
                  "\" depends on it; drop the index first"};
  ...
  for (const auto& cname : column_names) {
    ...
    const bool has_not_null = absl::c_any_of(
      table_ptr->CheckConstraints(), [&](const CheckConstraint& c) {
  ...
  for (size_t i = 0; i < column_names.size(); ++i) {
    ...
    clause +=
      duckdb::KeywordHelper::WriteOptionallyQuoted(column_names[i], '"');
  }
  clause += ")";
