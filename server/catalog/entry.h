...
// AclMode is a privilege bitmask: the reflection serializer would reject
// combined values (e.g. Select|Insert) as non-enumerators, so serialize the
// raw underlying integer. In duckdb's namespace for ADL.
...
// Owner, grants and privilege bitmask are duckdb's storage
// (CatalogEntry::permissions), so every entry version carries the ACL its
// committing transaction saw without a second MVCC; what the bits mean is here.
...
// The same identity, off the entry the info built: duckdb's oid, set from the
// definition, carried across versions by CatalogSet and rebuilt at boot from
// the record.
...
// The property a user type's stable id rides in its extension info, so a
// LogicalType read back from a column, a parameter or a wire description
// still names the catalog row it came from.
...
// The stamped ids of every user type `type` is built from, nested types
// included. Reads what CREATE TYPE wrote; resolves nothing.
...
// duckdb's own object-id allocator is the identity space; the durable horizon
// that keeps ids from being reissued after a crash lives beside the counter.
duckdb::DatabaseManager& IdAllocator();

// Plugs the catalog-log entry decoder into duckdb's
// foreign_create_info_deserializer hook; boot calls it before any replay.
