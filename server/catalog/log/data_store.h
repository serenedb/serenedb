...
class DataStore {
 public:
  // Gate for background work that touches a database attachment: false until
  // the boot attach loop has run.
 ...
 private:
  ...
  // Private: callers go through WithStoreBindContext, which gates on the store
  // being up.
  ...
  // between databases; null outside one. Held rather than pointed at: a DROP
  // DATABASE in another session detaches while the batch is still running, and
  // the close waits for the last reference.
  duckdb::shared_ptr<duckdb::AttachedDatabase> _target;
  // Serializes the batches, which is what makes the routing above one batch's
  // rather than whichever statement wrote it last.
  absl::Mutex _ops_mutex;
  // Kept apart from _conn: the connection that issues store DDL must stay
  // plain, or the statements resolve differently.
  // Reached from two directions -- a store batch resolving the connection
