...
bool ReadFrame(duckdb::BufferedFileReader& reader,
  ...
  // Bounds are what keep a corrupt size from becoming a huge allocation. The
  // header check is stated as an addition because the offset is bounded by the
  // file; the payload check is not, because `size` is arbitrary bytes from a
  // torn tail and `offset + size` wraps for a large one -- passing the guard
  // and reaching resize() with it, which is an out-of-memory abort at boot
  // before the checksum is ever consulted.
  const auto file_size = reader.FileSize();
  if (reader.CurrentOffset() + 2 * sizeof(uint64_t) > file_size) {
    return false;
  }
  auto size = reader.Read<uint64_t>();
  auto checksum = reader.Read<uint64_t>();
  if (size > file_size - reader.CurrentOffset()) {
    return false;
  }
  payload.resize(size);
...
void CatalogWal::Open(std::string_view directory, FrameVisitor replay) {
  ...
  _writable.store(true, std::memory_order_release);
...
void CatalogWal::Close() {
  absl::MutexLock lock{&_mutex};
  _writable.store(false, std::memory_order_release);
  if (!_writer) {
    return;
  }
  _mutex.Await(absl::Condition(
