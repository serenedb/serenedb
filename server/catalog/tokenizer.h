...
// Owns the reuse pool of constructed analyzer instances. A lease
// (GetTokenizer) hands out an exclusive instance Bind()-ed to the leasing
// caller's context; the pool return (PushTokenizer) Unbind()s it, so a
// parked instance holds no caller state and a fresh lease always starts
// clean.
class Tokenizer : public Object {
 public:
  ...
  TokenizerWrapper GetTokenizer(duckdb::ClientContext& ctx);
