...
class SereneDBClientState final : public duckdb::ClientContextState {
 public:
  ...
  // The relation a CREATE TABLE AS is loading into, handed to the load rather
  // than looked up by it. Cleared at the load's commit point and again at
  // rollback: a load that threw would otherwise leave it standing, and the next
  // CREATE TABLE on this connection would be answered with it.
  duckdb::optional_ptr<duckdb::CatalogEntry> ctas_target;
