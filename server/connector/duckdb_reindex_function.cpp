...
class ReindexSession {
 public:
  ...
  ReindexSession(duckdb::DatabaseInstance& db, const std::string& user,
                 ObjectId role_id, const std::string& database,
                 ObjectId database_id, int32_t backend_pid, NoticeSink sink)
    : _conn{db},
...
ReindexOutcome RunManualReindex(duckdb::ClientContext& context,
  ...
  if (auto client_state = context.registered_state->Get<SereneDBClientState>(
    ...
    pg::ProgressMetrics::Set(
      progress->relid,
      static_cast<int64_t>(target.index->GetRelationId().id()));
    pg::ProgressMetrics::Set(progress->current_relid,
                             static_cast<int64_t>(target.index->GetId().id()));
  }
  ...
  for (uint64_t attempt = 1;; ++attempt) {
    ...
    if (current.index->GetId() != target.index->GetId()) {
      THROW_SQL_ERROR(ERR_CODE(ERRCODE_UNDEFINED_OBJECT),
                      ERR_MSG("index \"", name, "\" does not exist"));
    }
    try {
...
absl::StatusOr<bool> RunReindexTick(duckdb::DatabaseInstance& db,
  ...
  try {
    ...
    const auto storage = catalog::InvertedStorageOf(database_id, index_id);
    if (!storage) {
      return false;
    }
    const auto claim =
    ...
    if (target.index->GetId() != index_id) {
      return false;
    }
    const auto outcome =
    ...
    SDB_DEBUG(SEARCH, "periodic reindex of Search index '", index_id.id(),
              "' retried: ", ex.what());
    return true;
