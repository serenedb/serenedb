...
// Fallback: delegate entirely to SKFullScan until proper range iteration lands.

duckdb::unique_ptr<duckdb::GlobalTableFunctionState> SKRangeScanInitGlobal(
  duckdb::ClientContext& context, duckdb::TableFunctionInitInput& input) {
  return SKFullScanInitGlobal(context, input);
}

void SKRangeScanFunction(duckdb::ClientContext& context,
                         duckdb::TableFunctionInput& data,
                         duckdb::DataChunk& output) {
  SKFullScanFunction(context, data, output);
}
