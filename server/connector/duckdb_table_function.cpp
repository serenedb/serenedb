...
duckdb::unique_ptr<duckdb::FunctionData> SereneDBScanBind(
  ...
  // An index over a search table keeps no store of its own, so the snapshot is
  // only opened for the indexes that have one.
  auto index_snapshot = [&] {
    return GetSereneDBContext(context).EnsureSearchSnapshot(
  ...
  if (table->IsSearchTable()) {
    // The index keeps no store of its own here -- it shares the table's -- so
    // this scan reads that store the way the table's own scan does.
    auto& conn_ctx = GetSereneDBContext(context);
    auto reader = conn_ctx.SearchTxn().EnsureSearchTableReader(
      index->GetRelationId(),
      [&] { return table->GetSearchData()->GetDirectoryReader(); });
...
SereneDBScanToValue(duckdb::TableFunctionToStringInput& input) {
  ...
  // A scan that reads an index names it, even where the store it reads is the
  // indexed table's own.
  if (bind.inverted_index && bind.entry_kind != ScanEntryKind::BaseTable) {
