...
struct FeedPool {
  ...
  const catalog::CreateInvertedIndexInfo& Info() const noexcept {
  ...
  irs::IndexWriter::Transaction NewTransaction() {
    ...
    trx.SetFieldOptions(catalog::InvertedInfoRef(index));
    return trx;
  }
  ...
  FeedPool(std::shared_ptr<search::InvertedIndexStorage> storage_in,
           catalog::TokenizerMap dicts_in, catalog::IndexInfoRef index_in,
           catalog::TableInfoRef table_in,
...
// Serializes one transaction's segment. Used for the transactions the
// committing thread fed itself (chunk slices, the inline leg): a worker task
// already flushes its own, but these are still buffered at commit time and
// would otherwise be written one after another under the writer's lock.
struct FlushTask final : duckdb::BaseExecutorTask {
  FlushTask(duckdb::TaskExecutor& executor_in,
            irs::IndexWriter::Transaction& trx_in)
    : BaseExecutorTask{executor_in}, trx{trx_in} {}

  void ExecuteTask() override { trx.Flush(); }
  std::string TaskType() const override { return "InvertedFlush"; }
...
struct InvertedFeedSession {
  InvertedFeedSession(std::shared_ptr<search::InvertedIndexStorage> storage,
                      catalog::TokenizerMap dicts, catalog::IndexInfoRef index,
                      catalog::TableInfoRef table,
                      duckdb::AttachedDatabase& attached,
                      const InvertedStoreIndex& owner)
    : pool{std::move(storage), std::move(dicts), std::move(index),
           std::move(table),   attached,         owner},
      live{pool} {}
...
InvertedStoreIndex::InvertedStoreIndex(
  ...
  duckdb::AttachedDatabase& db, catalog::TableInfoRef attached_table,
  catalog::IndexInfoRef attached_index,
...
const std::shared_ptr<InvertedFeedSession>&
InvertedStoreIndex::EnsureInvertedFeedSession() {
  ...
  if (_feed) {
    ...
    const bool stale =
      _feed->pool.index != inverted || _feed->pool.table != table;
    if (!stale || !_feed->live.Idle() || _feed->HasReplay()) {
      return _feed;
    }
    _feed.reset();
  }
  ...
  _feed = std::make_shared<InvertedFeedSession>(
    std::move(storage), std::move(dicts), std::move(inverted), std::move(table),
    db, *this);
...
duckdb::unique_ptr<InvertedStoreIndex> MakeInjectedInvertedIndex(
  ...
  duckdb::DataTable& storage, catalog::TableInfoRef table,
  catalog::IndexInfoRef inverted) {
  ...
  return duckdb::make_uniq<InvertedStoreIndex>(
    ...
    column_ids, exprs, storage.db, table, inverted, std::move(expr_fields),
    has_predicate, std::move(ref_columns));
}
