...
// pooled TokenSink staging them. Bind() per column, run fills against the
...
class InverterSink final : public irs::TokenConsumer {
 public:
  irs::TokenSink& Bind(const irs::IndexWriter::Document& writer,
                         irs::FieldInverter& slot, bool unique, bool dense_pos,
                         irs::TokenConsumer* store = nullptr) {
    _document = &writer;
    _slot = &slot;
    slot.SetOneToOne(unique);
    slot.SetDensePos(dense_pos);
    if (!_writer) {
      _writer = std::make_unique<irs::TokenSink>(writer.Allocator());
    }
 ...
 private:
  std::unique_ptr<irs::TokenSink> _writer;
