...
struct DeferredClaimBuilder {
  connector::SereneDBScanBindData::DeferredClaim claim;
  std::map<std::pair<duckdb::idx_t, duckdb::idx_t>,
           connector::SereneDBScanBindData::DeferredColumn>
    columns;
...
void DecidePlanCache(
  connector::SereneDBScanBindData& scan,
...
bool ClaimSearchConjuncts(
  ...
  DeferredClaimBuilder* const deferred_ptr =
    scan.vector_scorer && !scan.text_scorer ? &deferred : nullptr;
  ...
  if (!deferred.claim.conjuncts.empty()) {
    ...
    scan.deferred_claim = std::move(deferred.claim);
  }
...
void DecidePlanCache(
  connector::SereneDBScanBindData& scan,
  const duckdb::vector<duckdb::unique_ptr<duckdb::Expression>>& residual) {
  if (!scan.vector_scorer || scan.text_scorer || scan.TsDictMode() ||
      !scan.offsets.empty() || scan.declined_parameter ||
      !scan.reacquire_snapshot) {
    scan.cache_plan = false;
    return;
  }
  const bool residual_parameter =
    absl::c_any_of(residual, [](const auto& e) { return e->HasParameter(); });
  scan.cache_plan = !residual_parameter;
}
...
std::optional<connector::SearchColumnInfo> ResolveSearchColumnById(
  ...
  const bool table_backed =
    scan.GetKind() == connector::SereneDBScanBindData::Kind::Table;
  ...
  for (const auto* index : scan.InvertedIndexes()) {
    const auto* info = index->FindColumnInfo(col_id);
  ...
  if (scan.IsSearchTableEntry() && table_backed) {
    const auto& table_bd = scan.As<connector::TableScanBindData>();
    const auto* entry = dynamic_cast<const catalog::SereneDBTableEntry*>(
      table_bd.table_entry.get());
    if (entry != nullptr && entry->GetSearchData()) {
      const auto& shard = *entry->GetSearchData();
