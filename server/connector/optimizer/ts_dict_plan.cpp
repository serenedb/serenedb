...
class TsDictFacetPushdown {
 ...
 private:
  ...
  FoundScan _found{};
  const catalog::CreateInvertedIndexInfo* _index = nullptr;
 ...
 private:
      ...
      if (col_id == catalog::kInvertedIndexTermId) {
        if (const auto term_ref = ClassifyTsDictGetCol(
          ...
          multi_tok |= !_index.IsKeywordField(
            _context, _ss.ts_dicts[term_ref->req_index].field_id);
        }
        return true;
      }
  ...
  std::optional<connector::SearchColumnInfo> RawTerms(
    ...
    if (info && Enumerated(info->field_id) && info->tokenizer.analyzer &&
      ...
      info->tokenizer.analyzer = catalog::CreateTokenizerInfo::TokenizerWrapper{
  ...
  std::optional<connector::SearchColumnInfo> TermVirtualInfo(
    ...
    auto info = MakeSearchColumnInfo(
      ...
      {.analyzer = catalog::CreateTokenizerInfo::TokenizerWrapper{
         new irs::StringTokenizer(), {}}});
  ...
  const catalog::CreateInvertedIndexInfo& _index;
  duckdb::ClientContext& _context;
