...
duckdb::unique_ptr<duckdb::GlobalTableFunctionState> IResearchScanInitGlobal(
  ...
  if (state->shape == ScanShape::TsDict) {
    ...
    // A request that only wants the least or greatest term stops at the one
    // term it seeks, so splitting the dictionary walk across ranges would turn
    // a seek into a scan.
    ...
    // Counts that come from term metadata are already answered; only a scan
    // that walks postings per term has work to split across doc ranges.
    state->splittable =
      !seeks_one_term &&
