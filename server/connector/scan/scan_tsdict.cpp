...
struct TsDictLocalState : public ScanLocalState {
 ...
 public:
  // Counts come from the claimed range only, so a split segment sums them per
  // term across its workers before anything is emitted.
...
irs::detail::LazyBitset& TsDictLocalState::Live() {
  if (!_live) {
    ...
    auto node = query.PlanFill({.range = _range}, irs::ScoreMergeType::Noop);
    EnsurePlanned(node != nullptr);
    const auto* removals = _seg->docs_mask();
    if (auto* folded = node->Folded(); folded != nullptr) {
      // The plan may fold a set narrower than the claimed range; postings are
      // read against Range(), which therefore cannot outrun the storage.
      auto range = _range;
      if (range.Bounded()) {
        range.begin = std::max(range.begin, folded->Min());
        range.end = std::min(range.end, folded->End());
      }
      _live = std::make_unique<irs::detail::LazyBitset>(std::move(*folded),
                                                        range, removals);
    } else {
      _live = std::make_unique<irs::detail::LazyBitset>(
        std::move(node), _range, static_cast<irs::doc_id_t>(_seg->docs_count()),
        removals);
    }
  }
  return *_live;
...
uint32_t TsDictLocalState::NullDocs(const irs::TermReader& reader,
  ...
  if (_col_verify.Empty() && irs::detail::DocOf(reader) != nullptr) {
    if (auto counts =
      ...
      return count_all ? static_cast<uint32_t>(counts->Count(term))
                       : static_cast<uint32_t>(counts->Any(term));
    }
  }
  ...
  while (_next_field) {
    ...
    if (const auto* reader = _seg->field(field.field_id);
      ...
      if (emitting || counting) {
        // Both passes walk the dictionary the same way, so a term keeps the
        // position its count was added at.
