...
ResultOr<std::pair<irs::GeoDistanceFilter*, double>> PrepareGeoDistanceFilter(
  ...
  const auto* column_info =
    TryFindColumnInfo(ctx, PeelSameTypeIdCast(*geo_call.children[0]));
  if (!column_info) {
...
Result FromGeoInRange(irs::BooleanFilter& filter, const FilterContext& ctx,
  ...
  const auto* column_info =
    TryFindColumnInfo(ctx, PeelSameTypeIdCast(*func.children[0]));
  if (!column_info) {
    return {ERROR_BAD_PARAMETER,
...
Result FromGeoFilter(irs::BooleanFilter& filter, const FilterContext& ctx,
  ...
  const auto* column_info =
    TryFindColumnInfo(ctx, PeelSameTypeIdCast(*func.children[0]));
  if (!column_info) {
    column_info = TryFindColumnInfo(ctx, PeelSameTypeIdCast(*func.children[1]));
    if (!column_info) {
