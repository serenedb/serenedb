...
absl::Status SetupTermFilter(irs::ByTerm& filter,
  ...
  if (type_id == duckdb::LogicalTypeId::BOOLEAN &&
      !HasBoolFieldIds(column_info)) {
    return absl::UnimplementedError(
      "Boolean column without the always-<X> value fields");
  }
  ...
  // The dispatch that makes the boolean split real: the FIELD comes from the
  // value, at execution time, not from the type at plan time.
...
// false sorts before true, matching BooleanTokenizer's \x00 / \xFF terms.
...
// `json_type(<indexed json key>) = 'NULL'`: the JSON `null` leaf, which is a
// different fact from the key being SQL NULL and lives in its own field. Any
// other type name is declined -- the leaf fields answer values, not the
// existence of a type at the path.
...
absl::Status FromComparison(irs::BooleanFilter& filter,
  ...
  if (type_id == duckdb::LogicalTypeId::VARCHAR) {
    ...
    // Two values, two fields: the range is its accepted set. `ctx.negated` is
    // already folded into `op` above, so the set is added positively.
