...
void SearchSinkInsertBaseImpl::WriteJsonBatch(const duckdb::Vector& vec,
  ...
  for (duckdb::idx_t i = 0; i < count; ++i) {
    ...
    if (is_null) {
      // A SQL-NULL cell (a NULL row, or a path the document does not carry)
      // is what `IS NULL` asks about; a `null` leaf is not.
      jpf.sql_null_field.SetNullValue();
      EmitField(&jpf.sql_null_field);
    } else {
        ...
        if (doc.type().get(t) == simdjson::SUCCESS) {
          ...
          switch (t) {
            ...
            case simdjson::ondemand::json_type::null: {
              jpf.json_null_field.SetNullValue();
              insert_field(jpf.json_null_field);
            } break;
...
void SearchSinkInsertBaseImpl::JsonExpressionFields::InitForExpression(
  ...
  bool_field.PrepareForBooleanValue(entry->true_field_id,
                                    entry->false_field_id);
  json_null_field.PrepareForNullValue();
  json_null_field.id = entry->json_null_field_id;
  sql_null_field.PrepareForNullValue();
  sql_null_field.id = entry->null_field_id;
}
