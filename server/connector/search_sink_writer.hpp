...
inline TokenizerProvider MakeTokenizerProvider(
  duckdb::ClientContext& ctx, std::shared_ptr<const catalog::Snapshot> snapshot,
  const catalog::InvertedIndex& index) {
  return [&ctx, snapshot = std::move(snapshot),
          &index](irs::field_id field_id) -> catalog::ColumnTokenizer {
    return index.GetTokenizer(ctx, snapshot, field_id);
  };
}
...
class SearchSinkInsertBaseImpl {
 ...
 protected:
  ...
  void EmitPkTerms(std::span<const duckdb::string_t> keys);
  ...
  class StoreAppender final : public irs::TokenConsumer {
   public:
    ...
    void Consume(irs::TokenBatch&, irs::DocRuns) final {
      SDB_ASSERT(false);
    }
