...
SearchWriteTarget ResolveSearchWriteTarget(
  ...
  // A table that declares no key is identified by the synthetic one. Its
  // counter is shared side state -- every version of the table hands out from
  // the same cell -- so it is held off the entry, never resolved by name.
  if (pk_indexes.empty()) {
