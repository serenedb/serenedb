...
class H2CommTask final : public GeneralCommTask<T> {
 public:
  ...
  void Start() final;
  /// upgrade from  H1 connection, must not call start
 ...
 private:
  ...
  struct Stream final {
    ...
    bool must_send_auth_header = true;

    size_t header_buff_size = 0;  // total header size
    size_t response_offset = 0;   // current offset in response body
  };

  /// init h2 session
  void InitNgHttp2Session();

  /// handle stream request in serenedb
  ...
  std::atomic<unsigned> _num_processing{0};
