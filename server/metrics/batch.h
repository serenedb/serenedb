...
class Batch final : public IBatch {
 public:
  ...
  void toVPack(vpack::Builder& builder, SerenedServer& server) const final {
    for (auto& [labels, metric] : _metrics) {
      if (T::skip(server, labels)) {
        continue;
      }
      const auto coordinator_labels = T::coordinatorLabels(labels);
      const auto data = metric.load();
      // TODO(mbkkt) Write labels once
      for (size_t i = 0; i != T::kSize; ++i) {
        builder.add(T::kName[i]);
        builder.add(coordinator_labels);
        builder.add(T::kToValue[i](data));
      }
    }
  }
