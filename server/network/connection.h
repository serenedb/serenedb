 ...
 protected:
  ...
  yaclib::Future<> SendWriter() {
    ...
    for (;;) {
      ...
      _send.FlushDone();
      _producer_gate.Kick();
      // Wake the SessionTask only when it declared interest (ArmSendWaiter): the
      ...
      // ArmSendWaiter -- progress-store then waiter-load here, waiter-store then
      // progress-load there -- so a wake is never lost.
      std::atomic_thread_fence(std::memory_order_seq_cst);
    ...
    // waiter, and close so the read side notices.
