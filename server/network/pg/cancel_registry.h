...
class CancelRegistry {
 public:
  uint64_t Register(std::shared_ptr<CancelToken> token) {
    ...
    for (;;) {
      // The high 32 bits are the "pid" reported by pg_backend_pid() and sent in
      // BackendKeyData; PG pids are positive int32, so keep that half in
      ...
      if (_tokens.try_emplace(key, std::move(token)).second) {
        return key;
      }
    }
  }
  ...
  CancelResult CancelByPid(uint32_t pid) {
    std::shared_ptr<CancelToken> token;
    int matches = 0;
    {
      absl::MutexLock lock{&_mu};
