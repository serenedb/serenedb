...
// Same as SetHbaFromText but persists the ruleset to the config file first and
// only then publishes it live (so the running config and the on-disk file never
// diverge). Returns a formatted "line N: <message>" string on error (nullopt on
// success); the string-typed boundary lets the query-layer GUC handler call in
// without sharing the ParseError type across a header seam.
std::optional<std::string> SetHbaFromTextString(std::string_view text);

// Set the path of the HBA config file (default <datadir>/pg_hba.conf). The
// server resolves this from the data directory and calls this once at startup,
// before the listeners accept, so the module owns file I/O without reaching the
// data-directory singleton.
void SetHbaConfig(std::string path);

// The configured HBA file path (empty if never set). Backs pg_hba_file_rules'
// file_name column.
std::string_view ConfigFilePath();

// Load the persisted HBA ruleset from the config file (if present) into the
// live ruleset. Called once at startup, after SetHbaConfig. A readable
// file overrides the built-in default; a missing file keeps the default. A file
// that exists but fails to read or parse is logged and the default is kept.
