...
// Drives one DuckDB query to completion as a DuckDB scheduler task, then
// resumes the awaiting io coroutine once.
//
...
// on that worker, so a query that finishes immediately (the common
// prepared-OLTP case) never leaves the thread -- no enqueue, no suspend.
//
...
// it has no runnable task (NO_TASKS_AVAILABLE -- the worker pool is executing
// the query) or is blocked (BLOCKED), it PARKS: it returns TASK_BLOCKED and
// suspends until the executor wakes it. The executor fires our reschedule
// callback when the query reaches a state the driver cares about -- a result
// chunk is ready (streaming collector blocks), the query finishes, it errors,
// or a blocked task unblocks. So instead of ~thousands of NO_TASKS reschedules
// per analytical query the pump sleeps through execution and wakes a handful of
// times. (This relies on the executor patch that stores on_reschedule on
// NO_TASKS and fires it on completion / result-ready / error; see
// Executor::ExecuteTask / CompletePipeline / AddToBeRescheduled / PushError.)
//
// Wakes arrive on arbitrary worker threads, so _run guards against the pump
// being run concurrently with itself: a wake that lands while we are
// mid-Execute is folded into a single re-run rather than a second concurrent
// Execute.
//
...
class QueryPump final : public duckdb::Task {
 public:
  ...
  class [[nodiscard]] DriveAwaiter {
   public:
    ...
    // wrapper job, no handle.resume() trampoline frame. The coroutine's
    // executor is already *_resume (the dispatch hopped onto it before Drive),
    // so we need not reset promise._executor.
 ...
 private:
  ...
  // The session owns the pump; we never store a self-ptr to be rescheduled
  // from. RequestRun re-enqueues a fresh aliasing ptr instead.
  void Deschedule() override {}

  // Reschedule callback handed to ExecuteTask. The executor fires it (on
  // result- ready / completion / error / unblock) possibly while we are still
  // running, so coalesce: enqueue only from kParked; a wake during kRunning is
  // remembered so Execute re-runs instead of parking.
  void RequestRun() {
