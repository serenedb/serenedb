...
asio_ns::ssl::context BuildServerTlsContext(const TlsOptions& options) {
  ...
  const int min_version = options.min_version == TlsMinVersion::Tls13
                            ? TLS1_3_VERSION
                            : TLS1_2_VERSION;
  SSL_CTX_set_min_proto_version(native, min_version);
  ...
  // TLS 1.2 cipher list + key-exchange groups come from --tls_ciphers /
  // --tls_groups (their defaults carry the values). TLS 1.3 suites are fixed by
  // OpenSSL and always on.
  SSL_CTX_set_cipher_list(native, options.ciphers.c_str());
  SSL_CTX_set1_groups_list(native, options.groups.c_str());

  ctx.use_certificate_chain_file(options.cert_file);
  ctx.use_private_key_file(options.key_file, asio_ns::ssl::context::pem);
  if (!options.ca_file.empty()) {
    ctx.load_verify_file(options.ca_file);
    auto mode = asio_ns::ssl::verify_peer;
    if (options.require_client_cert) {
      mode |= asio_ns::ssl::verify_fail_if_no_peer_cert;
    }
