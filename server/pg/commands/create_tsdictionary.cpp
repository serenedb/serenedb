...
class CreateTSDictionaryOptions : public OptionsParser {
 ...
 private:
  ...
  irs::analysis::NGramTokenizer::Options BuildNGram(
    std::string_view prefix,
    const irs::analysis::NGramTokenizer::Options* parent) {
  ...
  irs::analysis::GeoJsonTokenizer::Options BuildGeoJson(
    std::string_view prefix,
    const irs::analysis::GeoJsonTokenizer::Options* parent) {
  ...
  irs::analysis::WildcardTokenizer::Options BuildWildcard(
    std::string_view prefix,
    const irs::analysis::WildcardTokenizer::Options* parent) {
  ...
  irs::analysis::ShingleTokenizer::Options BuildShingle(
    ...
    opts.fallback_unigrams =
      Resolve<tokenizer_options::kOutputUnigramsIfNoShingles>(
        prefix, parent ? &parent->fallback_unigrams : nullptr);
    ...
    if (OptionsParser::HasOption(tokenizer_options::kTokenSeparator, prefix)) {
      const auto raw =
        OptionsParser::EraseOptionOrDefault<tokenizer_options::kTokenSeparator>(
          prefix);
      opts.token_separator.assign(
        reinterpret_cast<const irs::byte_type*>(raw.data()), raw.size());
    } else if (parent) {
      opts.token_separator = parent->token_separator;
    }
    return opts;
  }
  ...
  void Dispatch(std::string_view type, std::string_view prefix,
    ...
    } else if (type == WildcardTokenizer::type_name()) {
      out.config = BuildWildcard(
        prefix, ParentOptions<WildcardTokenizer::Options>(parent_cfg));
    ...
    } else if (type == GeoPointTokenizer::type_name()) {
      out.config = BuildGeoPoint(
        prefix, ParentOptions<GeoPointTokenizer::Options>(parent_cfg));
    } else if (type == GeoJsonTokenizer::type_name()) {
      out.config = BuildGeoJson(
        prefix, ParentOptions<GeoJsonTokenizer::Options>(parent_cfg));
