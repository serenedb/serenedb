...
void RejectForeignPlaceholders(const duckdb::ParsedExpression& expr) {
  if (expr.GetExpressionClass() == duckdb::ExpressionClass::PARAMETER &&
      !IsInputRef(expr)) {
    THROW_SQL_ERROR(
      ERR_CODE(ERRCODE_SYNTAX_ERROR),
      ERR_MSG("\"", expr.ToString(), "\" is not available in an SQL stage"),
      ERR_HINT("$1 is the value being analyzed and the only placeholder"));
  }
  if (expr.GetExpressionClass() == duckdb::ExpressionClass::COLUMN_REF) {
    ...
    if (absl::EqualsIgnoreCase(names.back().GetIdentifierName(), kInput)) {
      THROW_SQL_ERROR(ERR_CODE(ERRCODE_SYNTAX_ERROR),
                      ERR_MSG("column \"", names.back().GetIdentifierName(),
                              "\" is not available in an SQL stage"),
                      ERR_HINT("read the value being analyzed as $1"));
    }
  }
  duckdb::ParsedExpressionIterator::EnumerateChildren(
    expr, [](const duckdb::ParsedExpression& child) {
      RejectForeignPlaceholders(child);
    });
}
  ...
  Stage CompileStage(const duckdb::ParsedExpression& expr) {
    ...
    if (const auto* fn = AsPlainCall(expr)) {
      ...
      if (!ReferencesInput(expr)) {
        RejectForeignPlaceholders(expr);
