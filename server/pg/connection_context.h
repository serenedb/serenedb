...
class ConnectionContext final : public query::Transaction {
 public:
  ...
  // The same, for the database list: while it holds, the cluster-wide database
  // cache is neither read nor filled from this transaction.
  bool wrote_databases = false;

  // And for the schema namespaces, which are cached the same way.
  bool wrote_schemas = false;
