...
// log(base, value) -> double precision
...
struct PgSetSeed {
  VELOX_DEFINE_FUNCTION_TYPES(T);
  FOLLY_ALWAYS_INLINE void call(out_type<Varchar>& result, double seed) {
...
void registerMathExtraFunctions(const std::string& prefix) {
  registerFunction<PgLogBase, double, double, double>({prefix + "log"});
  registerFunction<PgDivInt, int64_t, int64_t, int64_t>({prefix + "div"});
  registerFunction<PgDivDouble, int64_t, double, double>({prefix + "div"});
  registerFunction<PgGcd, int32_t, int32_t, int32_t>({prefix + "gcd"});
  registerFunction<PgGcd, int64_t, int64_t, int64_t>({prefix + "gcd"});
  registerFunction<PgLcm, int32_t, int32_t, int32_t>({prefix + "lcm"});
  registerFunction<PgLcm, int64_t, int64_t, int64_t>({prefix + "lcm"});
  // Degree trig
  registerFunction<PgSinD, double, double>({prefix + "sind"});
  registerFunction<PgCosD, double, double>({prefix + "cosd"});
  registerFunction<PgTanD, double, double>({prefix + "tand"});
  registerFunction<PgCotD, double, double>({prefix + "cotd"});
  registerFunction<PgASinD, double, double>({prefix + "asind"});
  registerFunction<PgACosD, double, double>({prefix + "acosd"});
  registerFunction<PgATanD, double, double>({prefix + "atand"});
  registerFunction<PgATan2D, double, double, double>({prefix + "atan2d"});
  registerFunction<PgSetSeed, Varchar, double>({prefix + "setseed"});
  registerFunction<PgErf, double, double>({prefix + "erf"});
  registerFunction<PgErfc, double, double>({prefix + "erfc"});
  registerFunction<PgRandomNormal, double, double, double>(
