...
void SqlAnalyzer::ProcessCreateFunctionStmt(State& state,
  ...
  if (stmt.sql_body) {
    if (IsA(stmt.sql_body, ReturnStmt)) {
      ...
      auto signature = ToSignature(stmt.parameters, stmt.returnType);
...
void SqlAnalyzer::ProcessWithClause(State& state, const WithClause* clause) {
  ...
  VisitNodes(clause->ctes, [&](const CommonTableExpr& cte) {
    ...
    // For recursive CTEs, only process the non-recursive term
    // (the left branch of the UNION ALL). This is correct when
    // the recursive term produces no new rows (e.g. empty pg_auth_members).
...
lp::ExprPtr SqlAnalyzer::ProcessFuncCall(State& state, const FuncCall& expr) {
  ...
  if (lang == catalog::FunctionLanguage::Decorator) {
    ...
    if (logical_function.Options().table &&
      ...
      auto& arg = args[0];
      const auto& arg_type = *arg->type();
      auto ref_type = arg_type.size() != 0 ? MakePtrView(arg_type.childAt(0))
                                           : MakePtrView(arg->type());
      state.target_list_rows_from.emplace_back(std::move(arg),
...
lp::ExprPtr SqlAnalyzer::ProcessSQLValueFunction(State& state,
  ...
  switch (expr.op) {
    case SVFOP_CURRENT_SCHEMA:
      return std::make_shared<lp::CallExpr>(pg::PGNAME(), "pg_current_schema");
    case SVFOP_CURRENT_USER:
    case SVFOP_CURRENT_ROLE:
    case SVFOP_USER:
    case SVFOP_SESSION_USER:
      return std::make_shared<lp::CallExpr>(pg::PGNAME(), "pg_current_user");
    case SVFOP_CURRENT_CATALOG:
      return std::make_shared<lp::CallExpr>(pg::PGNAME(),
                                            "pg_current_database");
