...
class InvertedIndexStorage final
 ...
 public:
  ...
  // Movable gate over _refresh_mutex -- the REINDEX driver acquires it in
  // one scope and releases it in another. absl::Mutex is std-lockable, so
  // the standard movable guard does the job.
  using RefreshGateGuard = std::unique_lock<absl::Mutex>;
  RefreshGateGuard AcquireRefreshGate() {
    return RefreshGateGuard{_refresh_mutex};
  }
