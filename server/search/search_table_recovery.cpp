...
duckdb::vector<duckdb::shared_ptr<duckdb::AttachedDatabase>> SereneDatabases() {
  duckdb::vector<duckdb::shared_ptr<duckdb::AttachedDatabase>> result;
  auto& manager =
    duckdb::DatabaseManager::Get(DuckDBEngine::Instance().instance());
  for (auto& attached : manager.GetDatabases()) {
    if (attached->GetCatalog().GetCatalogType() ==
