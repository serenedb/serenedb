...
struct CpuUsage::SnapshotProvider {
  SnapshotProvider();
  ~SnapshotProvider();
...
CpuUsage::SnapshotProvider::SnapshotProvider() {
...
bool CpuUsage::SnapshotProvider::TryTakeSnapshot(
...
size_t CpuUsage::SnapshotProvider::ReadStatFile(char* buffer,
...
CpuUsageSnapshot CpuUsage::snapshot() {
  ...
  bool update_in_progress;
  {
    ...
    absl::MutexLock guard{_snapshot_mutex};
...
void StatisticsFeature::toPrometheus(std::string& result, double now,
  ...
  if (auto snapshot = _cpu_usage.snapshot(); snapshot.valid()) {
