...
/// A wrapper to control the lifetime of an object
...
class AsyncValue {
 public:
  /// Same semantics as shared_ptr, expect copy ctor and assign
  ...
  class Value {
   public:
    ...
    const T* get() const noexcept { return _self ? _self->_resource : nullptr; }
  ...
  /// If return true then reset was call
  [[nodiscard]] bool empty() const noexcept {
    return _count.load(std::memory_order_acquire) & kReset;
  }

  /// Has the same semantics as weak_ptr::lock
  ...
  /// Denies access to resource granted by a lock,
