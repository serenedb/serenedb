...
struct OperationOptions {
  /// behavior when inserting a document by _key using INSERT with
  ...
  std::string is_synchronous_replication_from;

  // name of attribute that contains a per-document version number (optional)
  std::string version_attribute;

  IndexOperationMode index_operation_mode = IndexOperationMode::Normal;

  // INSERT ... OPTIONS { overwrite: true } behavior:
  // - replace an existing document, update an existing document, or do nothing
  OverwriteMode overwrite_mode = OverwriteMode::Unknown;
  ...
  // restored by replicated and serenerestore
  ...
  bool check_unique_constraints_in_preflight = false;

  // when truncating - should we also run the compaction?
  // defaults to true.
  bool truncate_compact = true;
  ...
  bool document_call_from_aql = false;
  ...
  // index.
  bool can_disable_indexing = true;
  ...
  // `RestDatabaseBaseHandler::createTransaction`, which either continues
