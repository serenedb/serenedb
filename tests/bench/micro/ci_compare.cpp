// Case-insensitive compare: CIEquals (a==b) and CILessThan (a<b, lexicographic).
// Existing (absl, folly) vs our own SWAR / SIMD. Each candidate is correctness-checked
// against the current impl over adversarial inputs before timing.
//
...
inline uint8_t Lower8(uint8_t c) { return (c >= 'A' && c <= 'Z') ? uint8_t(c + 32) : c; }
...
// transitively validate the patched EqualsIgnoreCase / memcasecmp via the harness.
...
bool EqSimd(const char *a, size_t an, const char *b, size_t bn) {
	...
	size_t n = an, i = 0;
	for (; i + 16 <= n; i += 16) {
		__m128i va = Lower128(_mm_loadu_si128(reinterpret_cast<const __m128i *>(a + i)));
		__m128i vb = Lower128(_mm_loadu_si128(reinterpret_cast<const __m128i *>(b + i)));
...
bool LtSimd(const char *a, size_t an, const char *b, size_t bn) {
	...
	for (; i + 16 <= n; i += 16) {
		...
		if (m != 0xFFFFu) {
			unsigned d = unsigned(__builtin_ctz(~m & 0xFFFFu)); // first differing byte
...
bool CheckEq(bool (*fn)(const char *, size_t, const char *, size_t), const std::vector<std::string> &c) {
	for (auto &a : c)
		for (auto &b : c) {
			if (fn(a.data(), a.size(), b.data(), b.size()) != EqAbsl(a.data(), a.size(), b.data(), b.size())) {
				return false;
			}
		}
	return true;
}
bool CheckLt(bool (*fn)(const char *, size_t, const char *, size_t), const std::vector<std::string> &c) {
	for (auto &a : c)
		for (auto &b : c) {
			if (fn(a.data(), a.size(), b.data(), b.size()) != LtAbsl(a.data(), a.size(), b.data(), b.size())) {
				return false;
			}
		}
	return true;
...
// realistic query pairs: ~half equal-CI (hit), half same-len-differ / diff-len (miss),
// short (the hash/tree regime). runtime strings, power-of-2 pool.
struct Pairs {
	...
	static const Pairs p = [] {
		...
		for (int i = 0; i < 256; ++i) {
			...
			if (kind == 0) {
				...
				r.b.push_back(mixed(key.substr(0, key.size() / 2))); // prefix (diff length)
			} else {
...
void Bm(benchmark::State &s) {
	...
	for (auto _ : s) {
		i = (i + 1) & m;
		benchmark::DoNotOptimize(Fn(p.a[i].data(), p.a[i].size(), p.b[i].data(), p.b[i].size()));
	}
}
} // namespace
