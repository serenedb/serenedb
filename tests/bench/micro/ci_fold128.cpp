...
// reference
inline uint8_t RefLower(uint8_t c) {
...
// add 0x3F maps 'A'..'Z' (0x41..0x5A) to 0x80..0x99 = -128..-103 (signed); no
// other byte lands <= -103, so one cmpgt isolates the upper-case set.
inline __m128i FoldB(__m128i v) {
  // 'A'..'Z' (0x41..0x5A) + 0x3F -> 0x80..0x99 = -128..-103 (signed); threshold
  // is the byte just past 'Z': 'Z' + 0x3F + 1 = 0x9A = -102, so biased < -102
  // selects [A,Z].
...
// C: __uint128_t SWAR (folly bit-trick at 128 bits; lowers to ~2x 64-bit on
// x86)
inline __uint128_t FoldC(__uint128_t c) {
  constexpr __uint128_t k7f =
    (__uint128_t(0x7f7f7f7f7f7f7f7fULL) << 64) | 0x7f7f7f7f7f7f7f7fULL;
  constexpr __uint128_t k25 =
    (__uint128_t(0x2525252525252525ULL) << 64) | 0x2525252525252525ULL;
  constexpr __uint128_t k1a =
    (__uint128_t(0x1a1a1a1a1a1a1a1aULL) << 64) | 0x1a1a1a1a1a1a1a1aULL;
  constexpr __uint128_t k20 =
    (__uint128_t(0x2020202020202020ULL) << 64) | 0x2020202020202020ULL;
  __uint128_t r = c & k7f;
  r += k25;
  r &= k7f;
  r += k1a;
...
void BmSimd(benchmark::State& s) {
  ...
  for (auto _ : s) {
    ...
    for (size_t i = 0; i < kN; i += 16) {
      __m128i v =
        _mm_loadu_si128(reinterpret_cast<const __m128i*>(buf.data() + i));
...
// consecutive folds don't form a serial dependency (mimics the hash's 4
// parallel AES states consuming independent folds). Isolates load+fold
// throughput.
template<__m128i (*Fn)(__m128i)>
void BmLoadFold(benchmark::State& s) {
  ...
  for (auto _ : s) {
    ...
    __m128i a0 = _mm_setzero_si128(), a1 = a0, a2 = a0, a3 = a0, a4 = a0,
            a5 = a0, a6 = a0, a7 = a0;
    for (size_t i = 0; i < kN; i += 128) {  // 8 lanes per 128 bytes
      auto L = [&](size_t off) {
        return Fn(
          _mm_loadu_si128(reinterpret_cast<const __m128i*>(p + i + off)));
      };
    ...
    __m128i acc = _mm_xor_si128(
      _mm_xor_si128(_mm_xor_si128(a0, a1), _mm_xor_si128(a2, a3)),
