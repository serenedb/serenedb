...
//
// Current DuckDB CIHash is byte-by-byte Jenkins (32-bit, serial) over a per-byte
// lowercase -- it is the entire cost of case-insensitive hash-set lookups (proven
// by the "bad CI" experiment: native hash + CIEquals ~= CS speed). This compares
// faster CI hashes both in isolation (ns/hash) and end-to-end as the hash of a
// std::unordered_set / absl::flat_hash_set CI lookup.
//
//   A: current StringUtil::CIHash       (Jenkins byte-by-byte, 32-bit)
//   B: per-byte lowercase + absl::Hash   (LowLevelHash, 64-bit) -- isolates the hash
//   C: SWAR (word-at-a-time) lowercase + absl::Hash
//   D: SWAR lowercase + XXH3_64bits
//
...
inline uint64_t HashD(const char *d, size_t n) {
	SwarLowerInto(d, n, g_scratch);
	return XXH64(g_scratch, n, 0); // lz4 ships xxhash 0.6.5 (pre-XXH3); XXH64 is the fast 64-bit hash
}
// E: per-char absl::ascii_tolower (256-byte table lookup) + absl::Hash
inline uint64_t HashE(const char *d, size_t n) {
	for (size_t i = 0; i < n; ++i) {
		g_scratch[i] = static_cast<char>(absl::ascii_tolower(static_cast<unsigned char>(d[i])));
	}
	return absl::HashOf(std::string_view(g_scratch, n));
}
// F: abseil's own bulk string lowercase -- AsciiStrToLower (branchless, SIMD for >=16),
// the impl behind public absl::AsciiStrToLower(string_view), called into our scratch
// buffer (no std::string materialization, same as every other candidate) + absl::Hash.
inline uint64_t HashF(const char *d, size_t n) {
	absl::ascii_internal::AsciiStrToLower(g_scratch, d, n);
	return absl::HashOf(std::string_view(g_scratch, n));
}
// H: FUSED single pass -- SWAR-lowercase each 8-byte word inline, mix straight into
// abseil's Mix. No scratch buffer, no second read of the data.
inline uint64_t HashH(const char *d, size_t n) {
	constexpr uint64_t k0 = 0x2d358dccaa6c78a5ull, k1 = 0x8bb84b93962eacc9ull, k2 = 0x4b33a62ed433d4a3ull;
	uint64_t state = k0 ^ n;
	size_t i = 0;
	for (; i + 8 <= n; i += 8) {
		uint64_t w;
	...
	if (i < n) { // tail 1..7 bytes, zero-padded (zeros are unaffected by lowercasing)
		uint64_t w = 0;
...
// OVERLAPPING last-8 read; n<=8: branchless duplicating read (abseil-style). No buffer.
inline uint64_t HashI(const char *d, size_t n) {
	...
	if (n <= 8) {
		...
		uint64_t w = n >= 4 ? (static_cast<uint64_t>(Load32(d)) << 32) | Load32(d + n - 4)
	...
	uint64_t last = Load64(d + n - 8); // overlapping last 8 bytes -- fixed width, no branch
	ToLowerAscii64(last);
...
BENCHMARK_TEMPLATE(BmHash, HashA)->Arg(4)->Arg(8)->Arg(12)->Arg(16)->Arg(32)->Arg(64)->Arg(128)->Arg(256)->Arg(1024)->Name("hashA_jenkins");
BENCHMARK_TEMPLATE(BmHash, HashB)->Arg(4)->Arg(8)->Arg(12)->Arg(16)->Arg(32)->Arg(64)->Name("hashB_bytelower_absl");
BENCHMARK_TEMPLATE(BmHash, HashC)->Arg(4)->Arg(8)->Arg(12)->Arg(16)->Arg(32)->Arg(64)->Arg(128)->Arg(256)->Arg(1024)->Name("hashC_swar_absl");
BENCHMARK_TEMPLATE(BmHash, HashD)->Arg(4)->Arg(8)->Arg(12)->Arg(16)->Arg(32)->Arg(64)->Name("hashD_swar_xxh64");
BENCHMARK_TEMPLATE(BmHash, HashE)->Arg(4)->Arg(8)->Arg(12)->Arg(16)->Arg(32)->Arg(64)->Name("hashE_asciilower_absl");
BENCHMARK_TEMPLATE(BmHash, HashF)->Arg(4)->Arg(8)->Arg(12)->Arg(16)->Arg(32)->Arg(64)->Name("hashF_abslstrlower_absl");
BENCHMARK_TEMPLATE(BmHash, HashH)->Arg(4)->Arg(8)->Arg(12)->Arg(16)->Arg(32)->Arg(64)->Name("hashH_fused_swar_mix");
BENCHMARK_TEMPLATE(BmHash, HashI)->Arg(4)->Arg(8)->Arg(12)->Arg(16)->Arg(32)->Arg(64)->Arg(128)->Arg(256)->Arg(1024)->Name("hashI_fused_fixedtail");
...
const Data &GetData(size_t n) {
	...
	auto make = [&](bool upper) {
		...
		for (size_t i = 0; i < len; ++i) {
			...
			s.push_back(upper && (next() & 1) ? duckdb::StringUtil::CharacterToUpper(c) : c);
		}
		return s;
	};
	for (size_t i = 0; i < n; ++i) {
	...
	for (int i = 0; i < 128; ++i) {
		r.queries.push_back(make(true)); // mixed-case miss (overwhelmingly not a key)
	}
...
void BmSet(benchmark::State &state) {
	...
	Set<std::string, HashFn<Fn>, CaseInsensitiveStringEquality> set(d.keys.begin(), d.keys.end());
...
// Quality at scale: current Jenkins (A) vs abseil-of-lowered gold (C) vs fused (I).
// A clustering (poor-distribution) hash shows up as SLOW lookups as N grows.
BENCHMARK_TEMPLATE(BmSet, AbslSet, HashA)->Arg(100)->Arg(1000)->Arg(10000)->Name("set_absl_A_jenkins");
BENCHMARK_TEMPLATE(BmSet, AbslSet, HashC)->Arg(100)->Arg(1000)->Arg(10000)->Name("set_absl_C_abslOfLowered");
BENCHMARK_TEMPLATE(BmSet, AbslSet, HashI)->Arg(100)->Arg(1000)->Arg(10000)->Name("set_absl_I_fused");
BENCHMARK_TEMPLATE(BmSet, StdSet, HashI)->Arg(100)->Arg(1000)->Arg(10000)->Name("set_std_I_fused");
