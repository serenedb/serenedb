...
//
// The CI hash folds each loaded word before mixing. This compares fold
// approaches at 1/2/4/8/16/32-byte granularity to see which instruction
...
// ---- 1-byte ----
inline char Fold1Branch(char c) {
...
// fold an L-byte buffer in place (L assumed multiple of the unit width here; we
// use L=64)
template<int W>
...
// per-byte (branch / branchless), then SWAR 2/4/8, then SIMD 16/32, over buffer
// sizes
BENCHMARK_TEMPLATE(Bm, 0)->Arg(8)->Arg(16)->Arg(32)->Arg(64)->Name(
  "fold_perbyte_branch");
BENCHMARK_TEMPLATE(Bm, -1)->Arg(8)->Arg(16)->Arg(32)->Arg(64)->Name(
  "fold_perbyte_branchless");
BENCHMARK_TEMPLATE(Bm, 2)->Arg(8)->Arg(16)->Arg(32)->Arg(64)->Name(
  "fold_swar2");
BENCHMARK_TEMPLATE(Bm, 4)->Arg(8)->Arg(16)->Arg(32)->Arg(64)->Name(
  "fold_swar4");
BENCHMARK_TEMPLATE(Bm, 8)->Arg(8)->Arg(16)->Arg(32)->Arg(64)->Name(
  "fold_swar8");
