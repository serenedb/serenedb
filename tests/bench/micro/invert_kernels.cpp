...
template<typename Fill>
void RunScatterDrain(benchmark::State& state, IndexFeatures features,
                     Fill&& fill) {
  auto mem = DefaultMemory();
  FieldsInverter inv{mem};
  auto* field = inv.Emplace(1, features);
  ...
  std::vector<bytes_view> terms(TokenBatch::kCapacity);
  std::vector<PostingRows> rows(TokenBatch::kCapacity);
  for (auto _ : state) {
    scattered.Reset(*field);
    reader.Reset(scattered);
    auto it = reader.iterator();
    size_t occurrences = 0;
    while (const auto n = it->NextTermsWithPostings(terms, rows, features)) {
      for (size_t i = 0; i < n; ++i) {
        occurrences += rows[i].span.count;
      }
    }
...
void BM_ScatterDrainTermsHC(benchmark::State& s) {
  RunScatterDrain(s, IndexFeatures::Freq,
                  [](FieldInverter& f) { FillKeyword(f, GetCorpus()); });
}
void BM_ScatterDrainTermsUnique(benchmark::State& s) {
  RunScatterDrain(s, IndexFeatures::Freq,
                  [](FieldInverter& f) { FillKeyword(f, GetUniqueCorpus()); });
}
void BM_ScatterDrainUniqueDict(benchmark::State& s) {
  RunScatterDrain(s, IndexFeatures::None,
                  [](FieldInverter& f) { FillUniqueDict(f, 0); });
}
void BM_ScatterDrainUniqueDictDups(benchmark::State& s) {
  RunScatterDrain(s, IndexFeatures::None,
                  [](FieldInverter& f) { FillUniqueDict(f, 512); });
}
...
void RunDictRef(benchmark::State& state, const C& corpus, size_t reserve = 0) {
  ...
  for (auto _ : state) {
    ...
    for (size_t i = 0; i < kTokens; ++i) {
      benchmark::DoNotOptimize(
        dict.Insert(MakeTermView(irs::ViewCast<char>(corpus.Term(i)))));
    }
    benchmark::DoNotOptimize(dict.Size());
  }
  state.counters["tokens/s"] =
...
BENCHMARK(BM_ScatterDrainTermsHC)->Unit(benchmark::kMillisecond);
BENCHMARK(BM_ScatterDrainTermsUnique)->Unit(benchmark::kMillisecond);
BENCHMARK(BM_ScatterDrainUniqueDict)->Unit(benchmark::kMillisecond);
BENCHMARK(BM_ScatterDrainUniqueDictDups)->Unit(benchmark::kMillisecond);
