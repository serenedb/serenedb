...
// Refutation arm: numeric trie terms pushed one pair at a time through the
// streamed keyword entry -- quantifies what InvertNumericBlock's batched
// resolve and grouped doc pushes buy over the "just use InvertKeywords"
// alternative.
...
// Staged-pairs shape: values + docs slabs chunk-flushed through the
// pairs-form fused kernel (the streamed list/JSON staging path).
...
// Fused UVF kernel: encode straight from the vector projection inside the
// inverter, ramp docs, compile-time tokens_per_doc -- no driver slabs.
// Production-shaped: real BIGINT vectors, one call per chunk.
...
void BM_FieldsMapFlatDeque(benchmark::State& state) {
  ...
  for (size_t i = 0; i < nfields; ++i) {
    auto& f = fields.emplace_back(static_cast<field_id>(i), arena, resolve,
                                  mem.rm, kTermsFeatures);
...
// Block keyword: prebuilt VARCHAR chunks through the UVF column entry, one
// InvertKeywordBlock per chunk per iteration. Resolve strategy is the
// production adaptive one; the forced fused/pipelined comparison lives in the
// BM_Dict* arms.
template<class C>
...
// --- UVF column entries: the driver-facing forms. Chunks are prebuilt duckdb
// Vectors; ToUnifiedFormat runs inside the loop like the real per-chunk
// ingest. ---
