...
const std::vector<std::string>& TextCyrillicCorpus() {
  static const auto corpus = [] {
    const char* sentences[] = {
      "Быстрая Коричневая Лиса Прыгает Через Ленивую Собаку Номер ",
      "Съешь Же Ещё Этих Мягких Французских Булок Да Выпей Чаю ",
      "Αλφάβητο Ελληνικό Κείμενο Για Τον Έλεγχο Της Απόδοσης ",
    };
    std::vector<std::string> out;
    out.reserve(kValues);
    for (size_t i = 0; i < kValues; ++i) {
      out.push_back(std::string{sentences[i % 3]} + std::to_string(i % 97));
    }
    return out;
  }();
  return corpus;
...
void BM_FillResolve(benchmark::State& state, Factory make, CorpusFn corpus_fn) {
  ...
  TermDictionary dict{arena, IResourceManager::gNoop};
  size_t consumed = 0;
...
Tokenizer::ptr MakeGeoJsonPoint() {
  return GeoJsonTokenizer::Make({.type = GeoJsonTokenizer::Type::Point,
                                 .coding = GeoJsonTokenizer::Coding::S2Point});
}

Tokenizer::ptr MakeGeoJsonShape() {
  return GeoJsonTokenizer::Make({.type = GeoJsonTokenizer::Type::Shape,
                                 .coding = GeoJsonTokenizer::Coding::Source});
}

Tokenizer::ptr MakeGeoJsonPointWkb() {
  auto stream = MakeGeoJsonPoint();
  static_cast<GeoJsonTokenizer*>(stream.get())->SetWkbInput(true);
  return stream;
}

Tokenizer::ptr MakeGeoPoint() { return GeoPointTokenizer::Make({}); }

Tokenizer::ptr MakeGeoPointWkb() {
  auto stream = GeoPointTokenizer::Make({});
  static_cast<GeoPointTokenizer*>(stream.get())->SetWkbInput(true);
  return stream;
}
...
Tokenizer::ptr MakeNGram(NGramTokenizer::InputType input) {
...
Tokenizer::ptr MakeNgramVariable() {
  ...
  opts.max_gram = 4;
  opts.preserve_original = false;
  opts.stream_bytes_type = NGramTokenizer::InputType::Binary;
  return NGramTokenizer::Make(std::move(opts));
}
...
Tokenizer::ptr MakeNgramPrefixSuffixMarked() {
  ...
  opts.start_marker = irs::ViewCast<irs::byte_type>(std::string_view{"^"});
  opts.end_marker = irs::ViewCast<irs::byte_type>(std::string_view{"$"});
  opts.ngram_mode = NGramTokenizer::NGramMode::PrefixAndSuffix;
  return NGramTokenizer::Make(std::move(opts));
}
...
Tokenizer::ptr MakePipelineSegNgram() {
  ...
  o.stream_bytes_type = NGramTokenizer::InputType::UTF8;
  subs.push_back(NGramTokenizer::Make(std::move(o)));
  return std::make_unique<PipelineTokenizer>(std::move(subs));
}
