...
IRS_FORCE_INLINE __uint128_t
InlineTermHandle(const duckdb::string_t& term) noexcept {
  return std::bit_cast<__uint128_t>(term);
}

IRS_FORCE_INLINE __uint128_t InlineTermHandle(std::string_view term) noexcept {
...
template<typename F, bool Mixed = false, bool Opaque = false>
void RunFromView(benchmark::State& state) {
  const auto n = static_cast<size_t>(state.range(0));
  const auto hit = static_cast<int>(state.range(1));
  const auto& corpus = GetCorpus(n, hit, Mixed ? 0.3 : 0.0);
  const F probe{corpus.members};
  std::vector<duckdb::string_t> views;
  views.reserve(corpus.probes.size());
  for (const auto& value : corpus.probes) {
    views.push_back(MakeTermView(value));
  }
  for (auto _ : state) {
    size_t found = 0;
  ...
  if constexpr (requires { probe.MemoryBytes(); }) {
    state.counters["bytes/key"] =
      static_cast<double>(probe.MemoryBytes()) / static_cast<double>(n);
  }
}
...
template<typename F, bool Opaque = false>
...
IRS_NO_INLINE size_t HashLongTerm(const duckdb::string_t& term) noexcept {
  return absl::HashOf(std::string_view{term.GetData(), term.GetSize()});
}

IRS_FORCE_INLINE uint32_t TermHash32(const duckdb::string_t& term) noexcept {
  if (term.GetSize() <= duckdb::string_t::INLINE_LENGTH) [[likely]] {
    return static_cast<uint32_t>(absl::HashOf(InlineTermHandle(term)));
  }
  return static_cast<uint32_t>(HashLongTerm(term));
}

IRS_FORCE_INLINE size_t TableHash(uint32_t hash) noexcept {
...
IRS_FORCE_INLINE const duckdb::string_t& TermOf(const Entry& entry) noexcept {
  if constexpr (std::is_same_v<Entry, duckdb::string_t>) {
    return entry;
  } else {
    return entry.term;
  }
...
struct RefHashEq {
  ...
  bool operator()(Ref lhs, Ref rhs) const noexcept { return lhs.id == rhs.id; }
  bool operator()(Ref lhs, const HashedTerm& rhs) const noexcept {
    return lhs.hash == rhs.hash && TermOf((*entries)[lhs.id]) == rhs.term;
  }
...
class RefTable {
 public:
  RefTable()
    : _map{0, RefHashEq<Entry>{&_entries}, RefHashEq<Entry>{&_entries}} {}

  IRS_FORCE_INLINE const Entry* Find(const duckdb::string_t& v) const noexcept {
    const auto it = _map.find(HashedTerm{v, TermHash32(v)});
    return it == _map.end() ? nullptr : &_entries[it->id];
  }

  template<typename... Mapped>
  void Insert(std::string_view word, Mapped... mapped) {
    const auto term = MakeTermView(word);
    const HashedTerm key{term, TermHash32(term)};
    bool is_new = false;
    _map.lazy_emplace(key, [&](const auto& ctor) {
      ctor(Ref{static_cast<uint32_t>(_entries.size()), key.hash});
      is_new = true;
    });
    if (is_new) {
      _entries.push_back(Entry{Intern(term), mapped...});
    }
  }
  ...
  size_t MemoryBytes() const noexcept {
    return _map.capacity() * (sizeof(Ref) + 1) +
           _entries.capacity() * sizeof(Entry) + _arena.SizeInBytes();
  }
...
struct LongHashEq {
  size_t operator()(const LongKey& key) const noexcept {
    return TableHash(key.hash);
  }
  bool operator()(const LongKey& lhs, const LongKey& rhs) const noexcept {
    return lhs.hash == rhs.hash && lhs.size == rhs.size &&
           std::memcmp(lhs.data, rhs.data, lhs.size) == 0;
  }
};

IRS_FORCE_INLINE LongKey MakeLongKey(const duckdb::string_t& v) noexcept {
  return {v.GetData(), static_cast<uint32_t>(v.GetSize()),
          static_cast<uint32_t>(
            absl::HashOf(std::string_view{v.GetData(), v.GetSize()}))};
}

class HashSlotSet {
 public:
  explicit HashSlotSet(const std::vector<std::string>& words) {
    for (const auto& word : words) {
      if (word.size() <= duckdb::string_t::INLINE_LENGTH) {
        _inline.insert(InlineTermHandle(std::string_view{word}));
        continue;
      }
      auto* mem = _arena.Allocate(word.size());
      std::memcpy(mem, word.data(), word.size());
      _long.insert(
        MakeLongKey(duckdb::string_t{reinterpret_cast<const char*>(mem),
                                     static_cast<uint32_t>(word.size())}));
    }
  }

  IRS_FORCE_INLINE bool Contains(const duckdb::string_t& v) const noexcept {
    if (v.GetSize() <= duckdb::string_t::INLINE_LENGTH) [[likely]] {
      return _inline.contains(InlineTermHandle(v));
    }
    return _long.contains(MakeLongKey(v));
  }

  size_t MemoryBytes() const noexcept {
    return _inline.capacity() * (sizeof(__uint128_t) + 1) +
           _long.capacity() * (sizeof(LongKey) + 1) + _arena.SizeInBytes();
  }

 private:
  duckdb::ArenaAllocator _arena{duckdb::Allocator::DefaultAllocator()};
  absl::flat_hash_set<__uint128_t> _inline;
  absl::flat_hash_set<LongKey, LongHashEq, LongHashEq> _long;
};

struct Table {
  explicit Table(const std::vector<std::string>& words, bool reserve = false) {
    if (reserve) {
      set.Reserve(words.size());
    }
    for (const auto& word : words) {
  ...
  size_t MemoryBytes() const noexcept { return set.MemoryBytes(); }

  irs::analysis::dict::StringSet<std::string> set;
};

struct RefSet {
  explicit RefSet(const std::vector<std::string>& words) {
  ...
  IRS_FORCE_INLINE bool Contains(const duckdb::string_t& v) const noexcept {
    return table.Find(v) != nullptr;
  }

  size_t MemoryBytes() const noexcept { return table.MemoryBytes(); }

  RefTable<duckdb::string_t> table;
};

struct StringTHashEq {
  IRS_FORCE_INLINE size_t
  operator()(const duckdb::string_t& term) const noexcept {
    if (term.GetSize() <= duckdb::string_t::INLINE_LENGTH) [[likely]] {
      return absl::HashOf(InlineTermHandle(term));
    }
    return HashLongTerm(term);
  }
  bool operator()(const duckdb::string_t& lhs,
                  const duckdb::string_t& rhs) const noexcept {
    return lhs == rhs;
  }
};
...
class StringTArena {
 public:
  duckdb::string_t Intern(std::string_view word) {
    if (word.size() <= duckdb::string_t::INLINE_LENGTH) {
      return MakeTermView(word);
    }
    auto* mem = _arena.Allocate(word.size());
    std::memcpy(mem, word.data(), word.size());
    return duckdb::string_t{reinterpret_cast<const char*>(mem),
  ...
  size_t SizeInBytes() const noexcept { return _arena.SizeInBytes(); }
...
template<bool Words>
struct StringTImageHashEq {
  IRS_FORCE_INLINE size_t
  operator()(const duckdb::string_t& term) const noexcept {
    if (term.GetSize() <= duckdb::string_t::INLINE_LENGTH) [[likely]] {
      if constexpr (Words) {
        const auto image = std::bit_cast<std::array<uint64_t, 2>>(term);
        return absl::HashOf(image[1], image[0]);
      } else {
        return absl::HashOf(std::bit_cast<__uint128_t>(term));
      }
    }
    return HashLongTerm(term);
  }
  IRS_FORCE_INLINE bool operator()(const duckdb::string_t& lhs,
                                   const duckdb::string_t& rhs) const noexcept {
    if (std::bit_cast<__uint128_t>(lhs) == std::bit_cast<__uint128_t>(rhs)) {
      return true;
    }
    const auto size = lhs.GetSize();
    if (size <= duckdb::string_t::INLINE_LENGTH || size != rhs.GetSize()) {
      return false;
    }
    return std::memcmp(lhs.GetData(), rhs.GetData(), size) == 0;
  }
};

template<bool Words>
struct StrTImageSet {
  explicit StrTImageSet(const std::vector<std::string>& words,
                        bool reserve = true) {
    if (reserve) {
      set.reserve(words.size());
    }
    for (const auto& word : words) {
      set.insert(arena.Intern(word));
    }
  }
  ...
  size_t MemoryBytes() const noexcept {
    return set.capacity() * (sizeof(duckdb::string_t) + 1) +
           arena.SizeInBytes();
  }

  StringTArena arena;
  absl::flat_hash_set<duckdb::string_t, StringTImageHashEq<Words>,
                      StringTImageHashEq<Words>>
    set;
};

template<bool Words>
struct StrTImageFind {
  explicit StrTImageFind(const std::vector<std::string>& words) {
    map.reserve(words.size());
    uint32_t i = 0;
    for (const auto& word : words) {
      map.emplace(arena.Intern(word), i++);
    }
  }
  ...
  size_t MemoryBytes() const noexcept {
    return map.capacity() *
             (sizeof(std::pair<duckdb::string_t, uint32_t>) + 1) +
           arena.SizeInBytes();
  }

  StringTArena arena;
  absl::flat_hash_map<duckdb::string_t, uint32_t, StringTImageHashEq<Words>,
                      StringTImageHashEq<Words>>
    map;
};

IRS_FORCE_INLINE duckdb::string_t HashedImage(
  const duckdb::string_t& v) noexcept {
  if (v.GetSize() <= duckdb::string_t::INLINE_LENGTH) [[likely]] {
    return v;
  }
  auto words = std::bit_cast<std::array<uint32_t, 4>>(v);
  words[1] = static_cast<uint32_t>(HashLongTerm(v));
  return std::bit_cast<duckdb::string_t>(words);
}

struct HashedImageHashEq {
  IRS_FORCE_INLINE size_t
  operator()(const duckdb::string_t& term) const noexcept {
    if (term.GetSize() <= duckdb::string_t::INLINE_LENGTH) [[likely]] {
      return absl::HashOf(std::bit_cast<__uint128_t>(term));
    }
    return TableHash(std::bit_cast<std::array<uint32_t, 4>>(term)[1]);
  }
  IRS_FORCE_INLINE bool operator()(const duckdb::string_t& lhs,
                                   const duckdb::string_t& rhs) const noexcept {
    if (std::bit_cast<__uint128_t>(lhs) == std::bit_cast<__uint128_t>(rhs)) {
    ...
    const auto l = std::bit_cast<std::array<uint32_t, 4>>(lhs);
    const auto r = std::bit_cast<std::array<uint32_t, 4>>(rhs);
    if (l[0] <= duckdb::string_t::INLINE_LENGTH || l[0] != r[0] ||
        l[1] != r[1]) {
      return false;
    }
    return std::memcmp(lhs.GetData(), rhs.GetData(), l[0]) == 0;
  }
};

struct StrTHashSet {
  explicit StrTHashSet(const std::vector<std::string>& words,
                       bool reserve = true) {
    if (reserve) {
      set.reserve(words.size());
    }
    for (const auto& word : words) {
      set.insert(HashedImage(arena.Intern(word)));
    }
  }

  IRS_FORCE_INLINE bool Contains(const duckdb::string_t& v) const noexcept {
    return set.contains(HashedImage(v));
  }
  ...
  StringTArena arena;
  absl::flat_hash_set<duckdb::string_t, HashedImageHashEq, HashedImageHashEq>
    set;
};

struct StrTHashFind {
  explicit StrTHashFind(const std::vector<std::string>& words) {
    map.reserve(words.size());
    uint32_t i = 0;
    for (const auto& word : words) {
      map.emplace(HashedImage(arena.Intern(word)), i++);
    }
  }

  IRS_FORCE_INLINE const uint32_t* Find(
    const duckdb::string_t& v) const noexcept {
    const auto it = map.find(HashedImage(v));
    return it == map.end() ? nullptr : &it->second;
  }
  ...
  StringTArena arena;
  absl::flat_hash_map<duckdb::string_t, uint32_t, HashedImageHashEq,
                      HashedImageHashEq>
    map;
};

template<bool HashInImage>
struct TermKey {
  duckdb::string_t img;

  template<typename H>
  friend H AbslHashValue(H h, const TermKey& key) {
    const auto words = std::bit_cast<std::array<uint64_t, 2>>(key.img);
    if (key.img.GetSize() <= duckdb::string_t::INLINE_LENGTH) [[likely]] {
      return H::combine(std::move(h), words[1], words[0]);
    }
    if constexpr (HashInImage) {
      return H::combine(std::move(h), words[0]);
    } else {
      return H::combine(std::move(h), HashLongTerm(key.img));
    }
  }
};

template<bool HashInImage>
IRS_FORCE_INLINE TermKey<HashInImage> MakeTermKey(
  const duckdb::string_t& v) noexcept {
  if constexpr (HashInImage) {
    return {HashedImage(v)};
  } else {
    return {v};
  }
}
...
struct TermKeyEq {
  template<bool HashInImage>
  IRS_FORCE_INLINE bool operator()(
    const TermKey<HashInImage>& lhs,
    const TermKey<HashInImage>& rhs) const noexcept {
    return HashedImageHashEq{}(lhs.img, rhs.img);
  }
};

template<bool HashInImage>
struct StrTKeySet {
  using Key = TermKey<HashInImage>;

  explicit StrTKeySet(const std::vector<std::string>& words,
                      bool reserve = true) {
    if (reserve) {
      set.reserve(words.size());
    }
    for (const auto& word : words) {
      set.insert(MakeTermKey<HashInImage>(arena.Intern(word)));
    }
  }

  IRS_FORCE_INLINE bool Contains(const duckdb::string_t& v) const noexcept {
    return set.contains(MakeTermKey<HashInImage>(v));
  }

  size_t MemoryBytes() const noexcept {
    return set.capacity() * (sizeof(Key) + 1) + arena.SizeInBytes();
  }

  StringTArena arena;
  absl::flat_hash_set<Key, absl::Hash<Key>, TermKeyEq> set;
};
...
struct StrTKeyFind {
  ...
  explicit StrTKeyFind(const std::vector<std::string>& words,
                       bool reserve = true) {
    if (reserve) {
      map.reserve(words.size());
    }
    uint32_t i = 0;
    for (const auto& word : words) {
      map.emplace(MakeTermKey<HashInImage>(arena.Intern(word)), i++);
    }
  }

  IRS_FORCE_INLINE const uint32_t* Find(
    const duckdb::string_t& v) const noexcept {
    const auto it = map.find(MakeTermKey<HashInImage>(v));
    return it == map.end() ? nullptr : &it->second;
  }

  size_t MemoryBytes() const noexcept {
    return map.capacity() * (sizeof(std::pair<Key, uint32_t>) + 1) +
           arena.SizeInBytes();
  }

  StringTArena arena;
  absl::flat_hash_map<Key, uint32_t, absl::Hash<Key>, TermKeyEq> map;
};

struct StrTSet {
  explicit StrTSet(const std::vector<std::string>& words) {
    set.reserve(words.size());
    for (const auto& word : words) {
      set.insert(arena.Intern(word));
    }
  ...
  StringTArena arena;
  absl::flat_hash_set<duckdb::string_t, StringTHashEq, StringTHashEq> set;
};

struct StrTFind {
  explicit StrTFind(const std::vector<std::string>& words) {
    map.reserve(words.size());
    uint32_t i = 0;
    for (const auto& word : words) {
      map.emplace(arena.Intern(word), i++);
  ...
  StringTArena arena;
  absl::flat_hash_map<duckdb::string_t, uint32_t, StringTHashEq, StringTHashEq>
    map;
};

struct TableFind {
  explicit TableFind(const std::vector<std::string>& words) {
    uint32_t i = 0;
    for (const auto& word : words) {
      map[word] = i++;
    }
  ...
  size_t MemoryBytes() const noexcept { return map.MemoryBytes(); }

  irs::analysis::dict::StringMap<std::string, uint32_t> map;
};
...
struct RefFind {
  explicit RefFind(const std::vector<std::string>& words) {
    table.Reserve(words.size());
    uint32_t i = 0;
    for (const auto& word : words) {
      table.Insert(word, i++);
  ...
  IRS_FORCE_INLINE const uint32_t* Find(
    const duckdb::string_t& v) const noexcept {
    const auto* entry = table.Find(v);
    return entry ? &entry->mapped : nullptr;
  }

  size_t MemoryBytes() const noexcept { return table.MemoryBytes(); }

  RefTable<MappedEntry> table;
};

template<typename T>
struct Grown : T {
  explicit Grown(const std::vector<std::string>& words) : T{words, false} {}
};

template<typename T>
struct Reserved : T {
  explicit Reserved(const std::vector<std::string>& words) : T{words, true} {}
};

#define TABLE_BENCH(name, type)                           \
  void BM_Table##name(benchmark::State& state) {          \
    RunFromView<type, false, true>(state);                \
  }                                                       \
  BENCHMARK(BM_Table##name)                               \
    ->ArgsProduct({{40, 200, 5000, 150000}, {0, 16, 40}}) \
    ->ArgNames({"n", "hit"})

TABLE_BENCH(Str, KeyStr);
TABLE_BENCH(Handle, KeyHandle);
TABLE_BENCH(Prod, Table);
TABLE_BENCH(Ref, RefSet);
TABLE_BENCH(HashSlot, HashSlotSet);
TABLE_BENCH(StrT, StrTSet);
TABLE_BENCH(StrTImage, StrTImageSet<false>);
TABLE_BENCH(StrTWords, StrTImageSet<true>);
TABLE_BENCH(StrTHash, StrTHashSet);
TABLE_BENCH(StrTKey, StrTKeySet<false>);
TABLE_BENCH(StrTKeyHash, StrTKeySet<true>);
TABLE_BENCH(StrTKeyHashGrow, Grown<StrTKeySet<true>>);
TABLE_BENCH(StrTImageGrow, Grown<StrTImageSet<false>>);
TABLE_BENCH(ProdReserve, Reserved<Table>);

#undef TABLE_BENCH

#define TABLE_MIXED_BENCH(name, type)                 \
  void BM_TableMixed##name(benchmark::State& state) { \
    RunFromView<type, true, true>(state);             \
  }                                                   \
  BENCHMARK(BM_TableMixed##name)                      \
    ->ArgsProduct({{40, 5000, 150000}, {0, 16, 40}})  \
    ->ArgNames({"n", "hit"})

TABLE_MIXED_BENCH(Str, KeyStr);
TABLE_MIXED_BENCH(Prod, Table);
TABLE_MIXED_BENCH(Ref, RefSet);
TABLE_MIXED_BENCH(HashSlot, HashSlotSet);
TABLE_MIXED_BENCH(StrT, StrTSet);
TABLE_MIXED_BENCH(StrTImage, StrTImageSet<false>);
TABLE_MIXED_BENCH(StrTWords, StrTImageSet<true>);
TABLE_MIXED_BENCH(StrTHash, StrTHashSet);
TABLE_MIXED_BENCH(StrTKey, StrTKeySet<false>);
TABLE_MIXED_BENCH(StrTKeyHash, StrTKeySet<true>);
TABLE_MIXED_BENCH(StrTKeyHashGrow, Grown<StrTKeySet<true>>);
TABLE_MIXED_BENCH(ProdReserve, Reserved<Table>);

#undef TABLE_MIXED_BENCH

#define FIND_CLOBBER_BENCH(name, type)                      \
  void BM_FindClobber##name(benchmark::State& state) {      \
    RunFind<type, true>(state);                             \
  }                                                         \
  BENCHMARK(BM_FindClobber##name)                           \
    ->ArgsProduct({{40, 5000, 50000, 150000}, {0, 16, 40}}) \
    ->ArgNames({"n", "hit"})

FIND_CLOBBER_BENCH(Swiss, SwissFind);
FIND_CLOBBER_BENCH(Prod, TableFind);
FIND_CLOBBER_BENCH(Ref, RefFind);
FIND_CLOBBER_BENCH(StrT, StrTFind);
FIND_CLOBBER_BENCH(StrTImage, StrTImageFind<false>);
FIND_CLOBBER_BENCH(StrTWords, StrTImageFind<true>);
FIND_CLOBBER_BENCH(StrTHash, StrTHashFind);
FIND_CLOBBER_BENCH(StrTKey, StrTKeyFind<false>);
FIND_CLOBBER_BENCH(StrTKeyHash, StrTKeyFind<true>);
FIND_CLOBBER_BENCH(StrTKeyHashGrow, Grown<StrTKeyFind<true>>);

#undef FIND_CLOBBER_BENCH

template<typename F, bool Mixed>
void RunBuild(benchmark::State& state) {
  const auto n = static_cast<size_t>(state.range(0));
  const bool reserve = state.range(1) != 0;
  const auto& corpus = GetCorpus(n, 0, Mixed ? 0.3 : 0.0);
  for (auto _ : state) {
    const F table{corpus.members, reserve};
    benchmark::ClobberMemory();
  }
  state.SetItemsProcessed(static_cast<int64_t>(state.iterations()) *
...
#define BUILD_BENCH(name, type)                       \
  void BM_Build##name(benchmark::State& state) {      \
    RunBuild<type, false>(state);                     \
  }                                                   \
  BENCHMARK(BM_Build##name)                           \
    ->ArgsProduct({{200, 5000, 150000}, {0, 1}})      \
    ->ArgNames({"n", "reserve"});                     \
  void BM_BuildMixed##name(benchmark::State& state) { \
    RunBuild<type, true>(state);                      \
  }                                                   \
  BENCHMARK(BM_BuildMixed##name)                      \
    ->ArgsProduct({{200, 5000, 150000}, {0, 1}})      \
    ->ArgNames({"n", "reserve"})

BUILD_BENCH(Prod, Table);
BUILD_BENCH(StrTImage, StrTImageSet<false>);
BUILD_BENCH(StrTHash, StrTHashSet);
BUILD_BENCH(StrTKey, StrTKeySet<false>);
BUILD_BENCH(StrTKeyHash, StrTKeySet<true>);
