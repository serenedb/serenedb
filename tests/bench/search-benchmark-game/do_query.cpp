 ...
 public:
  ...
  size_t ExecuteTopKWithCount(size_t k, std::string_view query) {
    ...
    auto execute = [&]<size_t K> {
      return irs::ExecuteTopKWithCount(
        _reader, *filter, _scorers, k,
        std::span<std::pair<irs::doc_id_t, irs::score_t>, K>{_results});
    };

    _results.resize(irs::BlockSize(k));
    if (k == 10) {
      return execute.template operator()<irs::BlockSize(10)>();
    }
    else if (k == 100) {
      return execute.template operator()<irs::BlockSize(100)>();
    }
    else {
