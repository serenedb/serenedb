  ...
  private:
    ...
    struct symbol_kind
    {
      enum symbol_kind_type
      {
        ...
        S_input = 22,                            // input
        S_query = 23,                            // query
        S_or_expr = 24,                          // or_expr
        S_and_expr = 25,                         // and_expr
        S_implicit_expr = 26,                    // implicit_expr
        S_unary_expr = 27,                       // unary_expr
        S_modifier_expr = 28,                    // modifier_expr
        S_field_or_atom = 29,                    // field_or_atom
        S_atom = 30,                             // atom
        S_range = 31                             // range
      };
    };
...
switch (yykind)
    ...
    struct symbol_type : basic_symbol<by_kind>
    {
      ...
      {
#if !defined _MSC_VER || defined __clang__
        YY_ASSERT ((token::TERM <= tok && tok <= token::PHRASE));
#endif
      }
    };

    /// Build a parser object.
    parser (yyscan_t scanner_yyarg, ParserContext& ctx_yyarg, int& rootIndex_yyarg);
    virtual ~parser ();
    ...
    // Implementation of make_symbol for each token kind.
      ...
      static
      symbol_type
      make_TERM (std::string_view v)
      {
        return symbol_type (token::TERM, std::move (v));
      }
#else
      static
      symbol_type
      make_TERM (const std::string_view& v)
      {
        return symbol_type (token::TERM, v);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      symbol_type
      make_PHRASE (std::string_view v)
      {
        return symbol_type (token::PHRASE, std::move (v));
      }
#else
      static
      symbol_type
      make_PHRASE (const std::string_view& v)
      {
        return symbol_type (token::PHRASE, v);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      ...
      make_CARAT ()
      {
        return symbol_type (token::CARAT);
      }
#else
      static
      symbol_type
