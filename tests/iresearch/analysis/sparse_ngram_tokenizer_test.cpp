...
struct OffsetTok {
  ...
  bool operator==(const OffsetTok&) const = default;
};

std::vector<OffsetTok> CollectWithOffsets(irs::analysis::Tokenizer& stream,
                                          std::string_view value) {
  std::vector<OffsetTok> out;
  const auto collect = [&](irs::TokenBatch& batch,
                           std::span<const irs::DocRun>) {
    for (uint32_t i = 0; i < batch.count; ++i) {
      const auto& term = batch.terms[i];
      out.push_back({std::string{term.GetData(), term.GetSize()},
                     batch.offs_start[i], batch.offs_end[i]});
    }
  };
  tests::FnTokenSink sink{irs::TokenLayout::TermsPosOffs, collect};
  stream.Fill(tests::ToStringT(value), sink.writer, {sink.layout});
  sink.writer.Finish();
  return out;
}
...
TEST(sparse_ngram_tokenizer_test, column_fill_matches_per_value) {
  ...
  for (size_t v = 0; v < values.size(); ++v) {
    auto tokens = CollectWithOffsets(*per_value, values[v]);
    for (auto& t : tokens) {
      expected[v].push_back({std::move(t.term), t.start, t.end});
    }
