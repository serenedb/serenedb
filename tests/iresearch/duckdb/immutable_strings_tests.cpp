...
TEST(immutable_strings_test, certified_share_carries_the_certificate) {
  auto origin =
    std::make_unique<duckdb::Vector>(duckdb::LogicalType::VARCHAR, kRows);
  Fill(*origin, kRows);
  Mark(*origin);
  const auto expected = Snapshot(*origin, kRows);
  auto first =
    std::make_unique<duckdb::Vector>(duckdb::LogicalType::VARCHAR, kRows);
  ...
  EXPECT_EQ(CountAliases(*first, second, kRows), CountLong(kRows));
  origin.reset();
  first.reset();
  EXPECT_EQ(Snapshot(second, kRows), expected);
}

TEST(immutable_strings_test, fallback_rows_stay_shareable_after_a_share) {
  auto plain =
    std::make_unique<duckdb::Vector>(duckdb::LogicalType::VARCHAR, kRows);
  Fill(*plain, kRows);
  auto marked =
    std::make_unique<duckdb::Vector>(duckdb::LogicalType::VARCHAR, kRows);
  Fill(*marked, kRows, 1000);
  Mark(*marked);
  const auto plain_rows = Snapshot(*plain, kRows);
  const auto marked_rows = Snapshot(*marked, kRows);
  auto target =
    std::make_unique<duckdb::Vector>(duckdb::LogicalType::VARCHAR, 2 * kRows);
  duckdb::ImmutableStrings::Copy(*plain, *target, kRows, 0, 0);
  ASSERT_EQ(CountAliases(*plain, *target, kRows), 0);
  duckdb::ImmutableStrings::Copy(*marked, *target, kRows, 0, kRows);
  ASSERT_EQ(CountAliases(*marked, *target, kRows, 0, kRows), CountLong(kRows));
  duckdb::Vector third(duckdb::LogicalType::VARCHAR, 2 * kRows);
  duckdb::ImmutableStrings::Copy(*target, third, 2 * kRows, 0, 0);
  EXPECT_EQ(CountAliases(*target, third, 2 * kRows), 2 * CountLong(kRows));
  plain.reset();
  marked.reset();
  target.reset();
  EXPECT_EQ(Snapshot(third, kRows, 0), plain_rows);
  EXPECT_EQ(Snapshot(third, kRows, kRows), marked_rows);
}
