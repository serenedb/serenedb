...
uint32_t BlockAt(uint32_t* const* blocks, uint64_t i) {
  return blocks[i >> ScatterScratch::kBlockShift]
               [i & ScatterScratch::kBlockMask];
}
...
bytes_view TermBytes(const ScatteredField& scattered, size_t rank) {
  return AsBytesView(scattered.TermAt(rank));
}
...
TEST(TermViewTest, MakeTermViewMatchesDuckDbCtor) {
  for (const uint32_t size :
    ...
    const auto ours = MakeTermView(std::string_view{s.data(), size});
  ...
  const auto empty = MakeTermView(std::string_view{});
  ...
  for (uint32_t size = 0; size <= duckdb::string_t::INLINE_LENGTH; ++size) {
    ...
    for (uint32_t at = 0; at <= at_max; ++at) {
      for (int c = 0; c < 256; ++c) {
        ...
        const auto view = MakeTermView(std::string_view{s.data(), size});
...
TEST(InverterTermDictionaryTest, ResolveDedupIntern) {
  ...
  const auto id0 = dict.Insert(MakeTermView("quick"));
  const auto id1 = dict.Insert(MakeTermView("brown"));
  const auto id2 = dict.Insert(MakeTermView("quick"));
  ASSERT_EQ(id0, id2);
  ASSERT_NE(id0, id1);
  ASSERT_EQ(2, dict.Size());

  const std::string long_term(100000, 'x');
  const auto id3 = dict.Insert(MakeTermView(long_term));
  const auto& interned = dict.Entries()[id3];
  ASSERT_EQ(
...
TEST(InverterPostingLogTest, RunMerging) {
  ...
  EXPECT_EQ(2, doc_tokens[0]);
  EXPECT_EQ(1, doc_tokens[1]);
  EXPECT_EQ(0, doc_tokens[2]);
  EXPECT_EQ(0, doc_tokens[3]);
  EXPECT_EQ(1, doc_tokens[4]);
  EXPECT_EQ(1, doc_tokens[5]);
...
uint32_t RowDoc(const ScatteredField& scattered, uint64_t k) {
  if (scattered.AllInline()) {
    return scattered.RankedInlineDocs()[k];
  }
  return BlockAt(scattered.DocBlocks(), k);
}
...
std::vector<ReadPosting> ReadTermPostings(const ScatteredField& scattered,
  ...
  for (auto k = scattered.TermBegin(rank); k != end;) {
    ...
    do {
      ++p.freq;
      if (want_pos) {
        p.positions.push_back(BlockAt(scattered.PosBlocks(), k));
      }
      if (want_offs) {
        p.offsets.emplace_back(BlockAt(scattered.OffsStartBlocks(), k),
                               BlockAt(scattered.OffsEndBlocks(), k));
      }
      ++k;
...
TEST(InverterPipelineTest, KernelToPostings) {
  ...
  for (const auto& text : docs) {
    ...
    do {
      ...
      ASSERT_TRUE(::tests::InvertTokens(*field, doc, *batch, !more));
    } while (more);
    ++doc;
  }
...
void AssertSameScatter(const FieldInverter& a, const FieldInverter& b,
  ...
  if (sa.Field().Layout() != TokenLayout::Terms) {
    for (uint64_t k = 0; k < nocc; ++k) {
      ASSERT_EQ(BlockAt(sa.PosBlocks(), k), BlockAt(sb.PosBlocks(), k));
    }
  }
  if (sa.Field().Layout() == TokenLayout::TermsPosOffs) {
    for (uint64_t k = 0; k < nocc; ++k) {
      ASSERT_EQ(BlockAt(sa.OffsStartBlocks(), k),
                BlockAt(sb.OffsStartBlocks(), k));
      ASSERT_EQ(BlockAt(sa.OffsEndBlocks(), k), BlockAt(sb.OffsEndBlocks(), k));
    }
  }
}

TEST(InverterColumnKeywordTest, MatchesPerValue) {
  constexpr size_t kDistinct = TermDictionary::kBatch * 6;
  constexpr size_t kDocs = TermDictionary::kBatch * 10;
...
TEST(InverterScatterTest, PresortedKeysTieBreak) {
  ...
  ASSERT_EQ(ToBytesView("aaaaaaaa1"), TermBytes(scattered, 0));
  ASSERT_EQ(ToBytesView("aaaaaaaa10"), TermBytes(scattered, 1));
  ASSERT_EQ(ToBytesView("aaaaaaaa2"), TermBytes(scattered, 2));
  ASSERT_EQ(ToBytesView("aaaaaaab"), TermBytes(scattered, 3));
}
...
TEST(InverterScatterTest, SharedPrefixRekeyOrder) {
  ...
  ASSERT_EQ(ToBytesView("http://example.com/"), TermBytes(scattered, 0));
  ASSERT_EQ(ToBytesView("http://example.com/about"), TermBytes(scattered, 1));
  ASSERT_EQ(ToBytesView("http://example.com/products/41"),
            TermBytes(scattered, 2));
  ASSERT_EQ(ToBytesView("http://example.com/products/42"),
            TermBytes(scattered, 3));
  ASSERT_EQ(ToBytesView("http://example.com/products/7"),
            TermBytes(scattered, 4));
  ASSERT_EQ(ToBytesView("http://example.com/zzz"), TermBytes(scattered, 5));
}
...
TEST(InverterScatterTest, RekeyPrefixTermRanksFirst) {
  ...
  ASSERT_EQ(ToBytesView("aaaaaaaabbbbbbbb"), TermBytes(scattered, 0));
  ASSERT_EQ(ToBytesView("aaaaaaaabbbbbbbbX"), TermBytes(scattered, 1));
  ASSERT_EQ(ToBytesView("aaaaaaaabbbbbbbbY"), TermBytes(scattered, 2));
}
...
TEST(InverterScatterTest, SortedEqualKeyOutOfOrderResorts) {
  ...
  scattered.Reset(*field);
  ASSERT_EQ(3, scattered.TermCount());
  ASSERT_EQ(ToBytesView("za_anchor"), TermBytes(scattered, 0));
  ASSERT_EQ(ToBytesView("zz_abcdefgh_1"), TermBytes(scattered, 1));
  ASSERT_EQ(ToBytesView("zz_abcdefgh_2"), TermBytes(scattered, 2));
}
...
TEST(InverterScatterTest, UniqueDictNonAdjacentDupFolds) {
  ...
  ASSERT_EQ(ToBytesView("qqqqqqqq_a"), TermBytes(scattered, 0));
  ASSERT_EQ(ToBytesView("qqqqqqqq_b"), TermBytes(scattered, 1));
...
TEST(InverterScatterTest, UniqueDictSortedFillIdentity) {
  ...
  ASSERT_EQ(4, scattered.TermCount());
  for (size_t r = 0; r < 4; ++r) {
    ASSERT_EQ(ToBytesView(vals[r]), TermBytes(scattered, r));
...
TEST(InverterScatterTest, UniqueDictDescendingFillSorts) {
  ...
  ASSERT_TRUE(scattered.AllInline());
  ASSERT_EQ(3, scattered.TermCount());
  ASSERT_EQ(ToBytesView("pk_00000001"), TermBytes(scattered, 0));
  ASSERT_EQ(ToBytesView("pk_00000002"), TermBytes(scattered, 1));
  ASSERT_EQ(ToBytesView("pk_00000010"), TermBytes(scattered, 2));
...
TEST(InverterPipelineTest, MultiBatchLongValue) {
  ...
  do {
    ...
    ASSERT_TRUE(::tests::InvertTokens(*field, d, *batch, !more));
    ++rounds;
  } while (more);
  ASSERT_GT(rounds, 1);
...
TEST(InverterPipelineTest, MultiBatchOffsets) {
  ...
  EXPECT_EQ(ToBytesView("aaa"), TermBytes(scattered, 0));
...
TEST(InverterPipelineTest, DenseAfterExplicitDoc) {
  ...
  batch->pos[1] = 2;
  ASSERT_TRUE(::tests::InvertTokens(*field, d, *batch, true));
...
std::vector<DumpedTerm> DumpField(const FieldInverter& field,
  ...
  for (size_t rank = 0; rank < scattered.TermCount(); ++rank) {
    const auto tv = TermBytes(scattered, rank);
    DumpedTerm dt;
    dt.term.assign(reinterpret_cast<const char*>(tv.data()), tv.size());
    for (auto& p :
         ReadTermPostings(scattered, rank, field.Meta().index_features)) {
...
void NumericBlockMatchesPerValue(const std::vector<T>& vals) {
...

  {
    ...
    for (const auto v : vals) {
      ...
      ASSERT_TRUE(::tests::InvertTokens(*ref, d, *batch, true));
      ++d;
    }
  }
...
TEST(InverterFieldTest, MetaCarriesRequestedFeatures) {
  constexpr auto kFeatures =
    IndexFeatures::Freq | IndexFeatures::Pos | IndexFeatures::Offs;
  auto mem = DefaultMemory();
  FieldsInverter inv{mem};
  auto* field = inv.Emplace(1, kFeatures);
  EXPECT_EQ(kFeatures, field->Meta().index_features);
  EXPECT_EQ(IndexFeatures::None,
            inv.Emplace(2, IndexFeatures::None)->Meta().index_features);
  ...
  batch->offs_start[0] = 0;
  batch->offs_end[0] = 1;
  ASSERT_TRUE(::tests::InvertTokens(*field, doc_limits::min(), *batch, true));
  EXPECT_EQ(kFeatures, field->Meta().index_features);
}
...
TEST(InverterEndToEndTest, EmptyPositionalFieldWritesNoStreams) {
  ...
  const auto run_case = [&](bool text_tokens) {
    ...
    {
      auto trx = writer->GetBatch();
      for (const std::string_view keyword : {"alpha", "beta"}) {
        auto doc = trx.Insert();
        const auto d = doc.DocId();
        const duckdb::string_t kw{keyword.data(),
                                  static_cast<uint32_t>(keyword.size())};
        ASSERT_TRUE(
          doc.WithField(kKeyword, IndexFeatures::Freq, [&](FieldInverter& fld) {
            return fld.InvertKeywords([&](auto&& emit) { emit(kw, d); });
          }));
        ASSERT_TRUE(
          doc.WithField(kText, kTextFeatures, [&](FieldInverter& fld) {
            batch->count = 0;
            if (text_tokens && keyword == "beta") {
              size_t cursor = 0;
              FillSplitBatch("quick fox", cursor, TokenLayout::TermsPosOffs,
                             *batch);
            }
            const DocRun run{d, batch->count};
            return fld.InvertBlock(*batch, DocRuns{{&run, 1}});
          }));
      }
      ASSERT_TRUE(trx.Commit());
    }
    ...
    const auto& segment = (*reader)[0];
    ASSERT_NE(nullptr, segment.field(kKeyword));
    const auto* text = segment.field(kText);
    EXPECT_EQ(text_tokens, text != nullptr);
    if (text) {
      EXPECT_EQ(kTextFeatures, text->meta().index_features & kTextFeatures);
    }
    bool has_pos = false;
    bool has_pay = false;
    ASSERT_TRUE(dir.visit([&](std::string_view name) {
      has_pos |= name.ends_with(".pos");
      has_pay |= name.ends_with(".pay");
      return true;
    }));
    EXPECT_EQ(text_tokens, has_pos);
    EXPECT_EQ(text_tokens, has_pay);
  };
