...
std::string SerializeResults(const std::vector<QueryResult>& results) {
  // Drive the templated reflection-based writer through `sdb::utils::JsonSink`
...
std::vector<QueryResult> DeserializeResults(std::string_view json_str) {
  ...
  // simdjson::ondemand and feed it through `sdb::utils::JsonSource` + the
  // reflection reader.
  ...
  sdb::utils::JsonSource source{doc};
