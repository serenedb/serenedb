...
class Connection100kWritesF : public ::testing::Test {
 protected:
  ...
  virtual void SetUp() override {
    try {
      ...
      _connection = cbuilder.connect(_event_loop_service);

      // delete collection
      {
        auto request = sdb::fuerte::createRequest(sdb::fuerte::RestVerb::Delete,
      ...
      // create collection
      {
        ...
        builder.openObject();
        builder.add("name", "testobi");
        builder.close();
        sdb::fuerte::Request request =
 ...
 private:
  f::EventLoopService _event_loop_service;
  std::string _server;
  std::string _port;
};
...
const& e){ std::cout << e.what(); if( barrier.wait() ) {
  ...
  size_t i = 0;
  for(auto const& slice : vpack::ArrayIterator(builder->slice())){
    ++i;
