...
inline void SetupEndpointFromEnv(f::ConnectionBuilder& cbuilder) {
  const char* tmp = getenv("TEST_ENDPOINT");
  std::string str;
...
extern std::string gMyAuthentication;

inline void SetupAuthenticationFromEnv(f::ConnectionBuilder& cbuilder) {
  const char* tmp = getenv("TEST_AUTHENTICATION");
  std::string auth;
  if (!tmp) {
    // No authentication settings in environment, set a default
    auth = gMyAuthentication;
  } else {
    auth.assign(tmp);
  }

  std::vector<std::string_view> parts = absl::StrSplit(auth, ':');
  if (parts[0] == "basic") {
    if (parts.size() != 3) {
    ...
    throw std::invalid_argument(
      absl::StrCat("Unknown authentication: ", parts[0]));
  }
}
