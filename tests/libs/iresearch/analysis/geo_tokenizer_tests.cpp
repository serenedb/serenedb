...
using irs::analysis::GeoPointTokenizer;
...
irs::analysis::Tokenizer::ptr MakeGeoJson(GeoJsonTokenizer::Type type,
                                          GeoJsonTokenizer::Coding coding) {
  return GeoJsonTokenizer::Make({.type = type, .coding = coding});
}
...
TEST(GeoTokenizerTests, traits_reflect_configuration) {
  {
    auto a = GeoPointTokenizer::Make({});
  ...
  {
    auto a = MakeGeoJson(GeoJsonTokenizer::Type::Shape,
                         GeoJsonTokenizer::Coding::Source);
    const auto traits = a->Traits();
    EXPECT_FALSE(traits.explicit_pos);
    EXPECT_FALSE(traits.store);
  }
  for (const auto coding : {GeoJsonTokenizer::Coding::S2Point,
    ...
    auto a = MakeGeoJson(GeoJsonTokenizer::Type::Shape, coding);
    EXPECT_TRUE(a->Traits().store);
  }
}

TEST(GeoTokenizerTests, geojson_point_terms_match_indexer_oracle) {
  auto stream = MakeGeoJson(GeoJsonTokenizer::Type::Point,
                            GeoJsonTokenizer::Coding::S2Point);
  auto* geo = dynamic_cast<GeoJsonTokenizer*>(stream.get());
...
TEST(GeoTokenizerTests, geojson_json_and_wkb_paths_agree) {
  for (const auto type :
       {GeoJsonTokenizer::Type::Shape, GeoJsonTokenizer::Type::Centroid,
        GeoJsonTokenizer::Type::Point}) {
    SCOPED_TRACE(testing::Message() << "type=" << int(type));
    auto json_stream = MakeGeoJson(type, GeoJsonTokenizer::Coding::S2Point);
    const auto via_json =
      AnalyzeGeo(*json_stream, R"({"type":"Point","coordinates":[2.0,1.0]})");
    ASSERT_TRUE(via_json.has_value());

    auto wkb_stream = MakeGeoJson(type, GeoJsonTokenizer::Coding::S2Point);
    dynamic_cast<GeoJsonTokenizer*>(wkb_stream.get())->SetWkbInput(true);
...
TEST(GeoTokenizerTests, geojson_terms_independent_of_coding) {
  constexpr std::string_view kPoint =
    R"({"type":"Point","coordinates":[2.0,1.0]})";
  auto source = MakeGeoJson(GeoJsonTokenizer::Type::Point,
                            GeoJsonTokenizer::Coding::Source);
  auto s2point = MakeGeoJson(GeoJsonTokenizer::Type::Point,
                             GeoJsonTokenizer::Coding::S2Point);
...
TEST(GeoTokenizerTests, geojson_centroid_of_point_stores_point_bytes) {
  constexpr std::string_view kPoint =
    R"({"type":"Point","coordinates":[2.0,1.0]})";
  auto point_stream = MakeGeoJson(GeoJsonTokenizer::Type::Point,
                                  GeoJsonTokenizer::Coding::S2Point);
  auto centroid_stream = MakeGeoJson(GeoJsonTokenizer::Type::Centroid,
                                     GeoJsonTokenizer::Coding::S2Point);
...
TEST(GeoTokenizerTests, geojson_polygon_terms_match_region_oracle) {
  const std::vector<std::pair<double, double>> ring{
    {0.0, 0.0}, {2.0, 0.0}, {2.0, 2.0}, {0.0, 2.0}, {0.0, 0.0}};

  auto stream = MakeGeoJson(GeoJsonTokenizer::Type::Shape,
                            GeoJsonTokenizer::Coding::Source);
  auto* geo = dynamic_cast<GeoJsonTokenizer*>(stream.get());
  ASSERT_NE(nullptr, geo);
  ...
  auto wkb_stream = MakeGeoJson(GeoJsonTokenizer::Type::Shape,
                                GeoJsonTokenizer::Coding::Source);
  dynamic_cast<GeoJsonTokenizer*>(wkb_stream.get())->SetWkbInput(true);
...
TEST(GeoTokenizerTests, geojson_point_type_rejects_non_points) {
  ...
  auto wkb_stream = MakeGeoJson(GeoJsonTokenizer::Type::Point,
                                GeoJsonTokenizer::Coding::S2Point);
  dynamic_cast<GeoJsonTokenizer*>(wkb_stream.get())->SetWkbInput(true);
  EXPECT_FALSE(AnalyzeGeo(*wkb_stream, PolygonWkb(ring)).has_value());
}

TEST(GeoTokenizerTests, geojson_column_fill_matches_per_value) {
  ...
  for (const auto type :
       {GeoJsonTokenizer::Type::Point, GeoJsonTokenizer::Type::Shape,
        GeoJsonTokenizer::Type::Centroid}) {
    for (const auto coding : {GeoJsonTokenizer::Coding::Source,
                              GeoJsonTokenizer::Coding::S2Point}) {
...
TEST(GeoTokenizerTests, geojson_wkb_column_fill_matches_per_value) {
  ...
  for (const auto type :
    ...
    for (const auto coding : {GeoJsonTokenizer::Coding::Source,
      ...
      auto fill_stream = MakeGeoJson(type, coding);
      auto value_stream = MakeGeoJson(type, coding);
      dynamic_cast<GeoJsonTokenizer*>(fill_stream.get())->SetWkbInput(true);
      dynamic_cast<GeoJsonTokenizer*>(value_stream.get())->SetWkbInput(true);
      ExpectColumnFillMatchesPerValue(*fill_stream, *value_stream, values);
    }
  }
}

TEST(GeoPointTokenizerTests, geopoint_column_fill_matches_per_value) {
  const auto object_opts = [] {
    return GeoPointTokenizer::Options{.latitude = {"location", "lat"},
...

  {
    ...
    auto fill_stream = GeoPointTokenizer::Make({});
    auto value_stream = GeoPointTokenizer::Make({});
    const std::vector<std::string> values = {
  ...
  }
  {
    ...
    auto fill_stream = GeoPointTokenizer::Make({});
    auto value_stream = GeoPointTokenizer::Make({});
    dynamic_cast<GeoPointTokenizer*>(fill_stream.get())->SetWkbInput(true);
    dynamic_cast<GeoPointTokenizer*>(value_stream.get())->SetWkbInput(true);
    ExpectColumnFillMatchesPerValue(*fill_stream, *value_stream, wkb_values);
  }
  {
    SCOPED_TRACE("object json");
    auto fill_stream = GeoPointTokenizer::Make(object_opts());
    auto value_stream = GeoPointTokenizer::Make(object_opts());
    const std::vector<std::string> values = {
...
TEST(GeoPointTokenizerTests, array_and_object_forms_match_oracle) {
  auto array_stream = GeoPointTokenizer::Make({});
  auto* geo = dynamic_cast<GeoPointTokenizer*>(array_stream.get());
  ASSERT_NE(nullptr, geo);
  ...
  auto object_stream = GeoPointTokenizer::Make(
...
TEST(GeoPointTokenizerTests, wkb_point_matches_json_terms) {
  auto json_stream = GeoPointTokenizer::Make({});
  const auto via_json = AnalyzeGeo(*json_stream, "[1.0, 2.0]");
  ASSERT_TRUE(via_json.has_value());

  auto wkb_stream = GeoPointTokenizer::Make({});
  dynamic_cast<GeoPointTokenizer*>(wkb_stream.get())->SetWkbInput(true);
  const auto via_wkb = AnalyzeGeo(*wkb_stream, PointWkb(2.0, 1.0));
  ASSERT_TRUE(via_wkb.has_value());
  EXPECT_EQ(via_json->terms, via_wkb->terms);
}

TEST(GeoPointTokenizerTests, rejects_invalid_input) {
  const std::vector<std::pair<double, double>> ring{
    {0.0, 0.0}, {2.0, 0.0}, {2.0, 2.0}, {0.0, 2.0}, {0.0, 0.0}};

  auto stream = GeoPointTokenizer::Make({});
  ...
  auto mismatched = [] {
    return GeoPointTokenizer::Make({.latitude = {"lat"}, .longitude = {}});
  };
...
TEST(GeoTokenizerTests, json_point_sample_matches_oracle) {
  const auto cases = SampleLatLngs(512);
  for (const auto coding :
       {GeoJsonTokenizer::Coding::Source, GeoJsonTokenizer::Coding::S2Point,
        GeoJsonTokenizer::Coding::S2LatLngF64,
        GeoJsonTokenizer::Coding::S2LatLngU32}) {
    auto stream = MakeGeoJson(GeoJsonTokenizer::Type::Point, coding);
    auto* geo = dynamic_cast<GeoJsonTokenizer*>(stream.get());
    ASSERT_NE(nullptr, geo);
    S2RegionTermIndexer oracle{geo->options()};
    for (size_t i = 0; i < cases.size(); ++i) {
      ...
      EXPECT_EQ(irs::tests::BinaryTerms(oracle.GetIndexTerms(point, {})),
                analysis->terms);
      if (coding == GeoJsonTokenizer::Coding::S2Point) {
...
TEST(GeoTokenizerTests, rejects_malformed_points) {
  auto stream = MakeGeoJson(GeoJsonTokenizer::Type::Point,
                            GeoJsonTokenizer::Coding::S2Point);
  for (const char* bad :
...
TEST(GeoPointTokenizerTests, json_array_sample_matches_oracle) {
  const auto cases = SampleLatLngs(512);
  auto stream = GeoPointTokenizer::Make({});
  auto* geo = dynamic_cast<GeoPointTokenizer*>(stream.get());
  ASSERT_NE(nullptr, geo);
  S2RegionTermIndexer oracle{geo->options()};
  for (size_t i = 0; i < cases.size(); ++i) {
...
TEST(GeoTokenizerTests, term_encoding_is_marker_plus_big_endian_cell) {
...
TEST(GeoTokenizerTests, query_terms_match_s2_oracle) {
  const std::vector<std::pair<double, double>> ring{
    {0.0, 0.0}, {2.0, 0.0}, {2.0, 2.0}, {0.0, 2.0}, {0.0, 0.0}};
  std::vector<S2Point> vertices;
  for (size_t i = 0; i + 1 < ring.size(); ++i) {
...
TEST(GeoTokenizerTests, wkb_point_sample_matches_oracle) {
...
TEST(GeoTokenizerTests, covering_terms_match_region_oracle_across_options) {
  constexpr std::string_view kPolygon =
    R"({"type":"Polygon","coordinates":[[[0.0,0.0],[2.0,0.0],[2.0,2.0],[0.0,2.0],[0.0,0.0]]]})";
  const std::vector<std::pair<double, double>> ring{
    {0.0, 0.0}, {2.0, 0.0}, {2.0, 2.0}, {0.0, 2.0}, {0.0, 0.0}};
  ...
  for (const auto& opts : {sdb::geo::GeoOptions{.max_cells = 20,
    ...
    SCOPED_TRACE(testing::Message()
                 ...
                 << " space=" << opts.optimize_for_space);
    auto stream =
      GeoJsonTokenizer::Make({.options = opts,
                              .type = GeoJsonTokenizer::Type::Shape,
                              .coding = GeoJsonTokenizer::Coding::Source});
    auto* geo = dynamic_cast<GeoJsonTokenizer*>(stream.get());
    ASSERT_NE(nullptr, geo);
...
TEST(GeoTokenizerTests, memory_usage_accounts_scratch) {
  auto json_stream = MakeGeoJson(GeoJsonTokenizer::Type::Shape,
                                 GeoJsonTokenizer::Coding::S2Point);
  ...
  auto point_stream = GeoPointTokenizer::Make({});
