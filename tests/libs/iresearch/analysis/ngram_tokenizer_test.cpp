...
TEST(ngram_token_stream_test, construct) {
  ...
  {
    auto stream = irs::analysis::NGramTokenizer::Make(
      irs::analysis::NGramTokenizer::Options{
        .min_gram = 1, .max_gram = 3, .preserve_original = true});
    ASSERT_NE(nullptr, stream);
    ASSERT_EQ(irs::Type<irs::analysis::NGramTokenizer>::id(), stream->type());

    auto& impl = dynamic_cast<irs::analysis::NGramTokenizer&>(*stream);
  ...
  // 2-gram
  {
    auto stream = std::make_unique<irs::analysis::NGramTokenizer>(
      irs::analysis::NGramTokenizer::Options(2, 2, true));
    ASSERT_NE(nullptr, stream);
    ASSERT_EQ(irs::Type<irs::analysis::NGramTokenizer>::id(), stream->type());

    auto impl = dynamic_cast<irs::analysis::NGramTokenizer*>(stream.get());
  ...
  {
    auto stream = std::make_unique<irs::analysis::NGramTokenizer>(
      irs::analysis::NGramTokenizer::Options(0, 2, true));
    ASSERT_NE(nullptr, stream);
    ASSERT_EQ(irs::Type<irs::analysis::NGramTokenizer>::id(), stream->type());
  ...
  {
    auto stream = std::make_unique<irs::analysis::NGramTokenizer>(
      irs::analysis::NGramTokenizer::Options(std::numeric_limits<size_t>::max(),
                                             2, true));
    ASSERT_NE(nullptr, stream);
    ASSERT_EQ(irs::Type<irs::analysis::NGramTokenizer>::id(), stream->type());
...
TEST(ngram_token_stream_test, next_utf8) {
  ...
  auto assert_utf8tokens = [](const std::vector<Utf8token>& expected,
                              std::string_view data,
                              irs::analysis::NGramTokenizer& stream) {
    ...
    const auto collect = [&](irs::TokenBatch& batch, irs::DocRuns runs) {
      ...
      for (uint32_t i = 0; i < batch.count; ++i) {
        ...
        terms.emplace_back(reinterpret_cast<const irs::byte_type*>(t.GetData()),
                           t.GetSize());
        if (dense) {
          ASSERT_EQ(poss.size() + 1, batch.pos[i]);
        }
    ...
    for (size_t k = 0; k < expected.size(); ++k) {
      ...
      const auto* const data_end = start + data.size();
      for (uint32_t p = 0; p < pos; ++p) {
        start = irs::utf8_utils::Next(start, data_end);
      }
      const auto size =
      ...
      if (!e.end_marker.empty()) {
        bs.append(reinterpret_cast<const irs::byte_type*>(e.end_marker.data()),
                  e.end_marker.size());
      }
  ...
  {
    SCOPED_TRACE("1-gram");
    irs::analysis::NGramTokenizer stream(irs::analysis::NGramTokenizer::Options(
      1, 1, false, irs::analysis::NGramTokenizer::InputType::UTF8,
...

  {
    SCOPED_TRACE("2-gram");
    irs::analysis::NGramTokenizer stream(irs::analysis::NGramTokenizer::Options(
      2, 2, false, irs::analysis::NGramTokenizer::InputType::UTF8,
      irs::bstring{}, irs::bstring{}));
...

  {
    ...
    irs::analysis::NGramTokenizer stream(irs::analysis::NGramTokenizer::Options(
      1, 2, false, irs::analysis::NGramTokenizer::InputType::UTF8,
      irs::bstring{}, irs::bstring{}));
...

  {
    ...
    irs::analysis::NGramTokenizer stream(irs::analysis::NGramTokenizer::Options(
      4, 4, false, irs::analysis::NGramTokenizer::InputType::UTF8));
...

  {
    SCOPED_TRACE("5-gram");
    irs::analysis::NGramTokenizer stream(irs::analysis::NGramTokenizer::Options(
      5, 5, false, irs::analysis::NGramTokenizer::InputType::UTF8,
      irs::bstring{}, irs::bstring{}));
...

  {
    ...
    irs::analysis::NGramTokenizer stream(irs::analysis::NGramTokenizer::Options(
      5, 5, true, irs::analysis::NGramTokenizer::InputType::UTF8,
      irs::bstring{}, irs::bstring{}));
...

  {
    ...
    irs::analysis::NGramTokenizer stream(irs::analysis::NGramTokenizer::Options(
      6, 6, true, irs::analysis::NGramTokenizer::InputType::UTF8,
      irs::bstring{}, irs::bstring{}));
    constexpr std::u8string_view kUtf8data = u8"\u00C0\u00C1\u00C2\u00C3\u00C4";
    const auto data = irs::ViewCast<char>(kUtf8data);
...

  {
    ...
    irs::analysis::NGramTokenizer stream(irs::analysis::NGramTokenizer::Options(
      6, 6, false, irs::analysis::NGramTokenizer::InputType::UTF8,
      irs::bstring{}, irs::bstring{}));
    const std::u8string_view utf8data = u8"\u00C0\u00C1\u00C2\u00C3\u00C4";
    const auto data = irs::ViewCast<char>(utf8data);
  ...
  }
  {
    ...
    irs::analysis::NGramTokenizer stream(irs::analysis::NGramTokenizer::Options(
      1, 2, true, irs::analysis::NGramTokenizer::InputType::UTF8,
      irs::bstring{reinterpret_cast<const irs::byte_type*>("\xc2\xa1"), 2},
      irs::bstring{}));
  ...
  }
  {
    ...
    irs::analysis::NGramTokenizer stream(irs::analysis::NGramTokenizer::Options(
      2, 3, true, irs::analysis::NGramTokenizer::InputType::UTF8,
      irs::bstring{},
      irs::bstring{reinterpret_cast<const irs::byte_type*>("\xc2\xa1"), 2}));
...

  {
    ...
    irs::analysis::NGramTokenizer stream(irs::analysis::NGramTokenizer::Options(
      1, 3, true, irs::analysis::NGramTokenizer::InputType::UTF8,
      irs::bstring{},
      irs::bstring{reinterpret_cast<const irs::byte_type*>("\xc2\xa1"), 2}));
...

  {
    ...
    irs::analysis::NGramTokenizer stream(irs::analysis::NGramTokenizer::Options(
      1, 3, false, irs::analysis::NGramTokenizer::InputType::UTF8,
      irs::bstring{reinterpret_cast<const irs::byte_type*>("\xc2\xa2"), 2},
      irs::bstring{reinterpret_cast<const irs::byte_type*>("\xc2\xa1"), 2}));
...
TEST(ngram_token_stream_test, reset_too_big) {
  irs::analysis::NGramTokenizer stream(
    irs::analysis::NGramTokenizer::Options(1, 1, false));
...
TEST(ngram_token_stream_test, next) {
  ...
  auto assert_tokens = [](const std::vector<Token>& expected,
  ...
  }
  {
    ...
    irs::analysis::NGramTokenizer stream(irs::analysis::NGramTokenizer::Options(
      1, 1, false, irs::analysis::NGramTokenizer::InputType::Binary, "$"_b,
      "^"_b));
...

  {
    ...
    irs::analysis::NGramTokenizer stream(
      irs::analysis::NGramTokenizer::Options(1, 1, true));
...

  {
    ...
    irs::analysis::NGramTokenizer stream(irs::analysis::NGramTokenizer::Options(
      1, 1, true, irs::analysis::NGramTokenizer::InputType::Binary, "$"_b,
      "^"_b));
...

  {
    SCOPED_TRACE("2-gram");
    irs::analysis::NGramTokenizer stream(
      irs::analysis::NGramTokenizer::Options(2, 2, false));
...

  {
    SCOPED_TRACE("1..2-gram");
    irs::analysis::NGramTokenizer stream(
      irs::analysis::NGramTokenizer::Options(1, 2, false));
...

  {
    SCOPED_TRACE("3-gram");
    irs::analysis::NGramTokenizer stream(
      irs::analysis::NGramTokenizer::Options(3, 3, false));
...

  {
    ...
    irs::analysis::NGramTokenizer stream(
      irs::analysis::NGramTokenizer::Options(1, 3, false));
  ...
  }
  {
    ...
    irs::analysis::NGramTokenizer stream(irs::analysis::NGramTokenizer::Options(
      1, 3, false, irs::analysis::NGramTokenizer::InputType::Binary, "$"_b,
      "^"_b));
...

  {
    ...
    irs::analysis::NGramTokenizer stream(
      irs::analysis::NGramTokenizer::Options(2, 3, false));
...

  {
    ...
    irs::analysis::NGramTokenizer stream(
      irs::analysis::NGramTokenizer::Options(2, 3, true));
...

  {
    ...
    irs::analysis::NGramTokenizer stream(irs::analysis::NGramTokenizer::Options(
      2, 3, true, irs::analysis::NGramTokenizer::InputType::Binary, "$"_b,
      irs::bstring{}));
...

  {
    ...
    irs::analysis::NGramTokenizer stream(irs::analysis::NGramTokenizer::Options(
      2, 3, true, irs::analysis::NGramTokenizer::InputType::Binary,
      irs::bstring{}, "^"_b));
...

  {
    SCOPED_TRACE("4-gram invalid utf8");
    irs::analysis::NGramTokenizer stream(
      irs::analysis::NGramTokenizer::Options(4, 4, false));
...

  {
    ...
    irs::analysis::NGramTokenizer stream(
      irs::analysis::NGramTokenizer::Options(1, 4, false));
...

  {
    SCOPED_TRACE("5-gram");
    irs::analysis::NGramTokenizer stream(
      irs::analysis::NGramTokenizer::Options(5, 5, false));
...

  {
    SCOPED_TRACE("5-gram, preserve original");
    irs::analysis::NGramTokenizer stream(
      irs::analysis::NGramTokenizer::Options(5, 5, true));
...

  {
    ...
    irs::analysis::NGramTokenizer stream(
      irs::analysis::NGramTokenizer::Options(4, 5, true));
...

  {
    ...
    irs::analysis::NGramTokenizer stream(
      irs::analysis::NGramTokenizer::Options(4, 5, false));
...

  {
    SCOPED_TRACE("6-gram, preserve original");
    irs::analysis::NGramTokenizer stream(
      irs::analysis::NGramTokenizer::Options(6, 6, true));
...

  {
    SCOPED_TRACE("6-gram no output");
    irs::analysis::NGramTokenizer stream(
      irs::analysis::NGramTokenizer::Options(6, 6, false));
...

  {
    ...
    irs::analysis::NGramTokenizer stream(
      irs::analysis::NGramTokenizer::Options(1, 5, false));
...

  {
    ...
    irs::analysis::NGramTokenizer stream(
      irs::analysis::NGramTokenizer::Options(3, 5, false));
...

  {
    ...
    irs::analysis::NGramTokenizer stream(
      irs::analysis::NGramTokenizer::Options(1, 6, false));
...

  {
    ...
    irs::analysis::NGramTokenizer stream(
      irs::analysis::NGramTokenizer::Options(1, 6, true));
...
TEST(ngram_token_stream_test, test_out_of_range_pos_issue) {
  auto stream =
    irs::analysis::NGramTokenizer::Make(irs::analysis::NGramTokenizer::Options{
...
TEST(ngram_token_stream_test, test_load) {
  {
    ...
    auto stream = irs::analysis::NGramTokenizer::Make(
      irs::analysis::NGramTokenizer::Options{
        ...
        .stream_bytes_type = irs::analysis::NGramTokenizer::InputType::Binary});
...
std::vector<PulledNgram> FillNgrams(irs::analysis::NGramTokenizer::Options opts,
  ...
  const auto collect = [&](irs::TokenBatch& batch, irs::DocRuns runs) {
    ...
    for (uint32_t i = 0; i < batch.count; ++i) {
      const auto& t = batch.terms[i];
      if (dense) {
        EXPECT_EQ(out.size() + 1, batch.pos[i]);
      }
      out.push_back(
...
TEST(ngram_token_stream_test, column_fill_runs) {
  ...
  opts.stream_bytes_type = irs::analysis::NGramTokenizer::InputType::UTF8;
  opts.start_marker =
...
TEST(ngram_token_stream_test, column_flush_structure) {
  ...
  const auto drain = [&](irs::TokenBatch& batch, irs::DocRuns runs) {
    ...
    const bool open = runs.tail_open;
...
void AssertModeFill(irs::analysis::NGramTokenizer::Options opts,
  ...
  for (size_t i = 0; i < expected.size(); ++i) {
    ...
    const irs::bstring term{
      reinterpret_cast<const irs::byte_type*>(expected[i].term.data()),
      expected[i].term.size()};
    ASSERT_EQ(term, filled[i].term);
    ASSERT_EQ(expected[i].pos, filled[i].pos);
    ASSERT_EQ(expected[i].start, filled[i].start);
    ASSERT_EQ(expected[i].end, filled[i].end);
    ASSERT_EQ(filled[i].term, pulled[i].term);
    ASSERT_EQ(filled[i].pos, pulled[i].pos);
    ASSERT_EQ(filled[i].start, pulled[i].start);
    ASSERT_EQ(filled[i].end, pulled[i].end);
  }
}

irs::analysis::NGramTokenizer::Options ModeOptions(
  size_t min, size_t max, bool preserve,
  irs::analysis::NGramTokenizer::NGramMode mode) {
  irs::analysis::NGramTokenizer::Options opts;
  opts.min_gram = min;
  opts.max_gram = max;
  opts.preserve_original = preserve;
...
TEST(ngram_token_stream_test, only_prefix_ascii) {
  using Mode = irs::analysis::NGramTokenizer::NGramMode;
  AssertModeFill(ModeOptions(1, 3, false, Mode::Prefix), "abcd",
                 {{"a", 1, 0, 1}, {"ab", 1, 0, 2}, {"abc", 1, 0, 3}});
}

TEST(ngram_token_stream_test, only_prefix_utf8_cyrillic) {
  using Mode = irs::analysis::NGramTokenizer::NGramMode;
  const std::string_view data = "\xd0\xbf\xd1\x80\xd0\xb8";
  AssertModeFill(ModeOptions(1, 10, false, Mode::Prefix), data,
                 {{std::string_view("\xd0\xbf", 2), 1, 0, 2},
                  {std::string_view("\xd0\xbf\xd1\x80", 4), 1, 0, 4},
                  {data, 1, 0, 6}});
}

TEST(ngram_token_stream_test, only_prefix_utf8_mixed_ascii_cyrillic) {
  using Mode = irs::analysis::NGramTokenizer::NGramMode;
  const std::string_view data = "a\xd0\xb1";
  AssertModeFill(ModeOptions(1, 2, false, Mode::Prefix), data,
                 {{"a", 1, 0, 1}, {data, 1, 0, 3}});
}

TEST(ngram_token_stream_test, only_prefix_utf8_supplementary_emoji) {
  using Mode = irs::analysis::NGramTokenizer::NGramMode;
  const std::string_view data = "A\xf0\x9f\x98\x80";
  AssertModeFill(ModeOptions(1, 2, false, Mode::Prefix), data,
                 {{"A", 1, 0, 1}, {data, 1, 0, 5}});
}

TEST(ngram_token_stream_test, only_prefix_utf8_cjk) {
  using Mode = irs::analysis::NGramTokenizer::NGramMode;
  const std::string_view data = "\xe6\x97\xa5\xe6\x9c\x88";
  AssertModeFill(
    ModeOptions(1, 2, false, Mode::Prefix), data,
    {{std::string_view("\xe6\x97\xa5", 3), 1, 0, 3}, {data, 1, 0, 6}});
}

TEST(ngram_token_stream_test, only_prefix_utf8_precomposed) {
  using Mode = irs::analysis::NGramTokenizer::NGramMode;
  AssertModeFill(ModeOptions(1, 3, false, Mode::Prefix), "caf\xc3\xa9",
                 {{"c", 1, 0, 1}, {"ca", 1, 0, 2}, {"caf", 1, 0, 3}});
}

TEST(ngram_token_stream_test, only_prefix_single_codepoint_min_two) {
  using Mode = irs::analysis::NGramTokenizer::NGramMode;
  AssertModeFill(ModeOptions(2, 4, false, Mode::Prefix), "\xd0\xaf", {});
}

TEST(ngram_token_stream_test, only_prefix_max_unbounded) {
  using Mode = irs::analysis::NGramTokenizer::NGramMode;
  AssertModeFill(
    ModeOptions(2, 4294967294u, false, Mode::Prefix), "hello",
    {{"he", 1, 0, 2}, {"hel", 1, 0, 3}, {"hell", 1, 0, 4}, {"hello", 1, 0, 5}});
}

TEST(ngram_token_stream_test, only_prefix_preserve_original_after_max) {
  using Mode = irs::analysis::NGramTokenizer::NGramMode;
  AssertModeFill(ModeOptions(1, 2, true, Mode::Prefix), "abcd",
                 {{"a", 1, 0, 1}, {"ab", 1, 0, 2}, {"abcd", 1, 0, 4}});
}

TEST(ngram_token_stream_test, only_prefix_min_greater_than_input_no_preserve) {
  using Mode = irs::analysis::NGramTokenizer::NGramMode;
  AssertModeFill(ModeOptions(3, 5, false, Mode::Prefix), "ab", {});
}

TEST(ngram_token_stream_test, only_prefix_min_greater_than_input_preserve) {
  using Mode = irs::analysis::NGramTokenizer::NGramMode;
  AssertModeFill(ModeOptions(3, 5, true, Mode::Prefix), "ab",
                 {{"ab", 1, 0, 2}});
}

TEST(ngram_token_stream_test, only_prefix_empty_input) {
  using Mode = irs::analysis::NGramTokenizer::NGramMode;
  AssertModeFill(ModeOptions(1, 2, false, Mode::Prefix), "", {});
}

TEST(ngram_token_stream_test, only_suffix_ascii) {
  using Mode = irs::analysis::NGramTokenizer::NGramMode;
  AssertModeFill(ModeOptions(1, 3, false, Mode::Suffix), "abcd",
                 {{"bcd", 1, 1, 4}, {"cd", 1, 2, 4}, {"d", 1, 3, 4}});
}

TEST(ngram_token_stream_test, only_suffix_preserve_original_after_max) {
  using Mode = irs::analysis::NGramTokenizer::NGramMode;
  AssertModeFill(ModeOptions(1, 2, true, Mode::Suffix), "abcd",
                 {{"abcd", 1, 0, 4}, {"cd", 1, 2, 4}, {"d", 1, 3, 4}});
}

TEST(ngram_token_stream_test, only_suffix_empty_input) {
  using Mode = irs::analysis::NGramTokenizer::NGramMode;
  AssertModeFill(ModeOptions(1, 2, false, Mode::Suffix), "", {});
}

TEST(ngram_token_stream_test, only_suffix_min_greater_than_input_preserve) {
  using Mode = irs::analysis::NGramTokenizer::NGramMode;
  AssertModeFill(ModeOptions(3, 5, true, Mode::Suffix), "ab",
                 {{"ab", 1, 0, 2}});
}

TEST(ngram_token_stream_test, only_prefix_and_suffix_ascii) {
  using Mode = irs::analysis::NGramTokenizer::NGramMode;
  AssertModeFill(
    ModeOptions(1, 2, false, Mode::PrefixAndSuffix), "abcd",
    {{"a", 1, 0, 1}, {"ab", 1, 0, 2}, {"cd", 2, 2, 4}, {"d", 2, 3, 4}});
}

TEST(ngram_token_stream_test, only_prefix_and_suffix_preserve) {
  using Mode = irs::analysis::NGramTokenizer::NGramMode;
  AssertModeFill(ModeOptions(1, 2, true, Mode::PrefixAndSuffix), "abcd",
                 {{"a", 1, 0, 1},
                  {"ab", 1, 0, 2},
                  {"abcd", 2, 0, 4},
                  {"cd", 2, 2, 4},
...
TEST(ngram_token_stream_test, only_prefix_and_suffix_overlapping) {
  ...
  const std::vector<ExpectedNgram> expected = {{"a", 1, 0, 1},
                                               ...
                                               {"bc", 2, 1, 3},
                                               {"c", 2, 2, 3}};
  AssertModeFill(ModeOptions(1, 3, false, Mode::PrefixAndSuffix), "abc",
                 expected);
  AssertModeFill(ModeOptions(1, 3, true, Mode::PrefixAndSuffix), "abc",
                 expected);
}

TEST(ngram_token_stream_test, only_prefix_and_suffix_empty_input) {
  using Mode = irs::analysis::NGramTokenizer::NGramMode;
  AssertModeFill(ModeOptions(1, 2, false, Mode::PrefixAndSuffix), "", {});
}

TEST(ngram_token_stream_test,
     only_prefix_and_suffix_min_greater_than_input_preserve) {
  using Mode = irs::analysis::NGramTokenizer::NGramMode;
  AssertModeFill(ModeOptions(3, 5, true, Mode::PrefixAndSuffix), "ab",
                 {{"ab", 1, 0, 2}});
}

TEST(ngram_token_stream_test, only_prefix_and_suffix_markers) {
  using Mode = irs::analysis::NGramTokenizer::NGramMode;
  const auto marked = [](size_t min, size_t max, bool preserve) {
    auto opts = ModeOptions(min, max, preserve, Mode::PrefixAndSuffix);
    opts.start_marker = irs::ViewCast<irs::byte_type>(std::string_view{"^"});
    opts.end_marker = irs::ViewCast<irs::byte_type>(std::string_view{"$"});
    return opts;
  };
  AssertModeFill(
    marked(1, 2, false), "abcd",
    {{"^a", 1, 0, 1}, {"^ab", 1, 0, 2}, {"cd$", 2, 2, 4}, {"d$", 2, 3, 4}});
  AssertModeFill(marked(1, 2, true), "abcd",
                 {{"^a", 1, 0, 1},
                  {"^ab", 1, 0, 2},
                  {"^abcd", 2, 0, 4},
                  {"abcd$", 2, 0, 4},
  ...
  AssertModeFill(marked(1, 3, true), "abc", whole_in_range);
  AssertModeFill(marked(3, 5, true), "ab",
                 {{"^ab", 1, 0, 2}, {"ab$", 1, 0, 2}});
}

TEST(ngram_token_stream_test, traits_by_config) {
  using NB = irs::analysis::NGramTokenizer;
  using Mode = NB::NGramMode;
  const auto traits = [](NB::Options opts) {
    opts.stream_bytes_type = NB::InputType::Binary;
    return irs::analysis::NGramTokenizer{std::move(opts)}.Traits();
  };
  EXPECT_FALSE(traits(ModeOptions(3, 3, false, Mode::All)).explicit_pos);
  EXPECT_TRUE(traits(ModeOptions(2, 3, false, Mode::All)).explicit_pos);
  EXPECT_TRUE(traits(ModeOptions(3, 3, true, Mode::All)).explicit_pos);
  EXPECT_TRUE(traits(ModeOptions(3, 3, false, Mode::Prefix)).explicit_pos);
  EXPECT_TRUE(traits(ModeOptions(3, 3, false, Mode::Suffix)).explicit_pos);
  EXPECT_TRUE(
    traits(ModeOptions(3, 3, false, Mode::PrefixAndSuffix)).explicit_pos);
  auto marked = ModeOptions(3, 3, false, Mode::All);
  marked.start_marker =
    irs::bstring{reinterpret_cast<const irs::byte_type*>("$"), 1};
  EXPECT_TRUE(traits(std::move(marked)).explicit_pos);
}

TEST(ngram_token_stream_test, memory_usage_accounts_scratch) {
  irs::analysis::NGramTokenizer stream(irs::analysis::NGramTokenizer::Options(
    1, 2, false, irs::analysis::NGramTokenizer::InputType::UTF8, irs::bstring{},
    irs::bstring{}));
  EXPECT_EQ(0, stream.MemoryUsage());
  const std::u8string data = u8"a¢b£c";
  ASSERT_TRUE(
