...
void ReferenceScanAsciiWords(
  std::string_view value,
  const std::function<void(const irs::analysis::AsciiSegment&)>& emit) {
  ...
  while (i < n) {
    ...
    emit(irs::analysis::AsciiSegment{static_cast<uint32_t>(seg_begin),
...
TEST(ascii_words_test, MaskScanMatchesReference) {
  using irs::analysis::AsciiSegment;
  const auto collect_ref = [](std::string_view v) {
    std::vector<AsciiSegment> out;
    ReferenceScanAsciiWords(v, [&](const AsciiSegment& s) {
      out.push_back(s);
    });
    return out;
  };
