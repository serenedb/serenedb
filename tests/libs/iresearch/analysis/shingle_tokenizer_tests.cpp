 ...
 public:
  ...
  bool DoFill(duckdb::string_t raw, irs::TokenSink& sink) {
    ...
    while (p < data.size()) {
      ...
      skipped = 0;
      sink.Emit<L>(
        irs::MakeTermView(w.data(), static_cast<uint32_t>(w.size())), pos);
    }
    return true;
  }
};

// Uppercasing whitespace splitter: emits generated bytes into the emitter arena
// (never a view into the input), so the shingle takes its `needs_intern` copy
// path.
class UpperTokenizer final
  : public irs::analysis::TypedTokenizer<UpperTokenizer> {
 public:
  static constexpr std::string_view type_name() noexcept { return "test_upper"; }
  template<irs::TokenLayout L>
  bool DoFill(duckdb::string_t raw, irs::TokenSink& sink) {
    const std::string_view data{raw.GetData(), raw.GetSize()};
    size_t p = 0;
    while (p < data.size()) {
      ...
      for (size_t i = 0; i < w.size(); ++i) {
        _buf[i] = static_cast<irs::byte_type>(std::toupper(
          static_cast<unsigned char>(w[i])));
      }
...
TEST(ShingleTokenizerTest, needs_intern_generated_tokens) {
  // A base that emits generated (uppercased) bytes into its own arena forces
  // the shingle's needs_intern copy path; long tokens keep the terms correct.
  ...
  std::string A = a;
  std::string B = b;
  for (auto& c : A) c = static_cast<char>(std::toupper(c));
...
TEST(ShingleTokenizerTest, column_fill_matches_per_value) {
  auto analyzer = MakeAnalyzer(2, 3, /*output_unigrams=*/true);
  const std::vector<std::string> values{"quick brown fox", "a b c d",
                                        "lonely", "", "one two"};
...
TEST(ShingleTokenizerTest, token_codec_sequence) {
  ...
  for (const auto& t : tokens) {
    ShingleTokenizer::WriteToken(irs::ViewCast<irs::byte_type>(std::string_view{t}),
                                 buf);
  }
