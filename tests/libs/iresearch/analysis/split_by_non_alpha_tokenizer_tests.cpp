...
TEST(classify_block_test, ExhaustiveAgainstScalar) {
  irs::byte_type block[irs::classify::kClassifyBlock];
  const irs::byte_type targets[] = {' ', ',', 0x00, 0xFF};
  for (int c = 0; c < 256; ++c) {
    for (size_t at = 0; at < irs::classify::kClassifyBlock; ++at) {
      for (size_t i = 0; i < irs::classify::kClassifyBlock; ++i) {
        block[i] = static_cast<irs::byte_type>(i * 7 + 13);
      }
      block[at] = static_cast<irs::byte_type>(c);

      irs::WordMasks expect{0, 0, 0};
      ...
      const auto m = irs::ClassifyWordBlock(block);
      ASSERT_EQ(expect.word, m.word) << "c=" << c << " at=" << at;
      ASSERT_EQ(expect.alpha, m.alpha) << "c=" << c << " at=" << at;
      ASSERT_EQ(expect.digit, m.digit) << "c=" << c << " at=" << at;
      ASSERT_EQ(expect_eq, irs::ClassifyEqBlock(block, ' '));
      ASSERT_EQ(expect_any, irs::ClassifyAnyEqBlock(block, targets));
    }
  }
}
