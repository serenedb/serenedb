...
std::vector<std::vector<std::string>> FillColumn(
  ...
  a.Fill(vals, docs, sink.writer, layout);
  sink.writer.Finish();
  return got;
}
...
TEST(SqlTokenizerTest, parseErrors) {
  ...
  ASSERT_THROW(SqlTokenizer::Make({.expression = "upper($1)"}),
               sdb::SqlException);
  ASSERT_THROW(
    SqlTokenizer::Make({.expression = "memory.main.upper(input)"}),
    sdb::SqlException);
...
TEST(SqlTokenizerTest, userMacroRejected) {
  ...
  ASSERT_THROW(SqlTokenizer::Make({.expression =
                                     "sql_tok_userdb.main.sql_tok_test_up("
                                     "input)"}),
               sdb::SqlException);
