...
TEST(string_token_stream_tests, native_column_fill) {
  ...
  for (duckdb::idx_t i = 1; i < 4; ++i) {
    validity.SetInvalid(i);
  }
  duckdb::UnifiedVectorFormat fmt;
  vec.ToUnifiedFormat(5, fmt);
  ...
  tests::FnTokenSink sink{irs::TokenLayout::TermsPosOffs, check};
  stream.Fill(fmt, 5, 1, sink.writer, {sink.layout});
  sink.writer.Finish();
  ASSERT_EQ(1, flushes);
}

TEST(string_token_stream_tests, native_column_fill_dictionary) {
  irs::KeywordTokenizer stream;

  duckdb::Vector vec{duckdb::LogicalType::VARCHAR};
  auto* slots = duckdb::FlatVector::GetDataMutable<duckdb::string_t>(vec);
  slots[0] = duckdb::string_t{"aa", 2};
  slots[1] = duckdb::string_t{"bb", 2};
  ...
  vec.Slice(sel, 4);
  duckdb::UnifiedVectorFormat fmt;
  vec.ToUnifiedFormat(4, fmt);

  const std::string_view expected[] = {"aa", "bb", "bb", "aa"};
  size_t flushes = 0;
  const auto check = [&](irs::TokenBatch& batch, irs::DocRuns runs) {
    ++flushes;
    ASSERT_EQ(4, runs.size());
    ASSERT_EQ(4, batch.count);
    for (uint32_t i = 0; i < 4; ++i) {
      ASSERT_EQ(7 + i, runs[i].doc);
      ASSERT_EQ(1, runs[i].ntokens);
      const auto& t = batch.terms[i];
      ASSERT_EQ(expected[i], std::string_view(t.GetData(), t.GetSize()));
    }
  };
  tests::FnTokenSink sink{irs::TokenLayout::Terms, check};
  stream.Fill(fmt, 4, 7, sink.writer, {sink.layout});
  sink.writer.Finish();
  ASSERT_EQ(1, flushes);
}
...
TEST(string_token_stream_tests, column_suspension) {
  ...
  size_t flushes = 0;
  size_t consumed = 0;
  const auto on_flush = [&](irs::TokenBatch& batch, irs::DocRuns runs) {
...
class BulkMockTokenizer final
 ...
 public:
  ...
  bool DoFill(duckdb::string_t raw, irs::TokenSink& sink) {
    ...
    const auto* base = reinterpret_cast<const irs::byte_type*>("x");
    sink.EmitK<Layout>(value.size(), base, base + 1,
                       [](size_t) { return irs::EmitKSlotPos{0, 1, 1}; });
    return true;
  }
};
...
TEST(token_sink_tests, intern) {
  ...
  tests::FnTokenSink fn{irs::TokenLayout::Terms, check};
  auto& sink = fn.writer;
  tests::EmitCopy<irs::TokenLayout::Terms>(sink, view(small));
  tests::EmitCopy<irs::TokenLayout::Terms>(sink, view(exact));
  tests::EmitCopy<irs::TokenLayout::Terms>(sink, view(big));
  tests::EmitCopy<irs::TokenLayout::Terms>(sink, view(huge));
  ...
  sink.Finish();
  ASSERT_EQ(1, flushes);
  ASSERT_EQ(4, seen.size());

  ASSERT_EQ(small_copy, seen[0]);
  ASSERT_EQ(exact_copy, seen[1]);
  ASSERT_EQ(big_copy, seen[2]);
  ASSERT_EQ(huge_copy, seen[3]);
}

TEST(token_sink_tests, value_run_sink) {
  constexpr auto kCap = irs::TokenBatch::kCapacity;
  std::vector<std::vector<irs::DocRun>> flushed_runs;
  std::vector<bool> flushed_open;
  const auto on_flush = [&](irs::TokenBatch& batch, irs::DocRuns runs) {
    flushed_runs.emplace_back(runs.begin(), runs.end());
    flushed_open.push_back(runs.tail_open);
  };
  ...
  w.BeginValue(7, 0);
  for (int i = 0; i < 3; ++i) {
    w.Emit<irs::TokenLayout::Terms>(duckdb::string_t{"a", 1});
  }
  w.EndValue();
  ...
  for (size_t i = 0; i < kCap + 5; ++i) {
    w.Emit<irs::TokenLayout::Terms>(duckdb::string_t{"b", 1});
  }
  w.EndValue();
  ASSERT_EQ(1, flushed_runs.size());
  ASSERT_EQ(2, flushed_runs[0].size());
  ASSERT_EQ(7, flushed_runs[0][0].doc);
  ASSERT_EQ(8, flushed_runs[0][1].doc);
  ASSERT_EQ(kCap - 3, flushed_runs[0][1].ntokens);
  ASSERT_TRUE(flushed_open[0]);
  ASSERT_EQ(1, w.Runs().size());
  ASSERT_EQ(8, w.Runs()[0].doc);
  ASSERT_EQ(8, w.Runs()[0].ntokens);

  w.BeginValue(8, 0);
  for (size_t staged = 8; staged < kCap; ++staged) {
    w.Emit<irs::TokenLayout::Terms>(duckdb::string_t{"c", 1});
  }
  w.EndValue();
  w.BeginValue(9, 0);
  w.Emit<irs::TokenLayout::Terms>(duckdb::string_t{"d", 1});
  w.EndValue();
  ASSERT_EQ(2, flushed_runs.size());
  ASSERT_FALSE(flushed_open[1]);
...
class OneTokenMockTokenizer final
 ...
 public:
  ...
  irs::TokenTraits Traits() const noexcept final { return {.unique = true}; }
  ...
  bool DoFill(duckdb::string_t raw, irs::TokenSink& sink) {
    ...
    sink.Emit<Layout>(irs::MakeTermView(value), irs::Offs{0, size});
    return true;
  }
};
...
TEST(token_sink_tests, unique_hint) {
  ...
  ASSERT_TRUE(stream.Traits().unique);
  ASSERT_TRUE(irs::KeywordTokenizer{}.Traits().unique);
  ASSERT_FALSE(tests::EmptyTokenizer{}.Traits().unique);
  ...
  {
    tests::FnTokenSink sink{irs::TokenLayout::Terms,
                            [](irs::TokenBatch&, irs::DocRuns) {}};
    ASSERT_TRUE(stream.Fill(a, sink.writer, {sink.layout}));
    ASSERT_TRUE(sink.writer.Runs().empty());
  }

  {
    tests::EmptyTokenizer empty;
    ...
    tests::FnTokenSink sink{irs::TokenLayout::Terms, check};
    tests::FillColumn(empty, vals, 1, sink.writer, sink.layout);
    sink.writer.Finish();
    ASSERT_EQ(1, flushes);
  }
}

TEST(token_sink_tests, rejected_value_records_empty_run) {
  tests::EmptyTokenizer empty;
  ...
  const auto check = [&](irs::TokenBatch& batch, irs::DocRuns runs) {
    ...
    ASSERT_EQ(0, batch.count);
    ASSERT_EQ(1, runs.size());
    ASSERT_EQ(42, runs[0].doc);
    ASSERT_EQ(0, runs[0].ntokens);
  };
  tests::FnTokenSink sink{irs::TokenLayout::Terms, check};
  ASSERT_FALSE(
    empty.Fill(duckdb::string_t{a.data(), 3}, 42, sink.writer, {sink.layout}));
  ASSERT_EQ(1, sink.writer.Runs().size());
  ASSERT_EQ(0, sink.writer.Runs()[0].ntokens);
  sink.writer.Finish();
  ASSERT_EQ(1, flushes);
}
...
TEST(token_sink_tests, interned_spike_survives_arena_reclaim) {
  ...
  for (const auto v : values) {
    w.BeginValue(doc++, 0);
    tests::EmitCopy<irs::TokenLayout::Terms>(w,
                                             irs::ViewCast<irs::byte_type>(v));
    w.EndValue();
    w.Finish();
  }
...
TEST(token_sink_tests, run_capacity_forced_cycle) {
  ...
  for (uint32_t d = 1; d <= kCap; ++d) {
    w.BeginValue(d, 0);
    if ((d % 2) == 0) {
      w.Emit<irs::TokenLayout::Terms>(duckdb::string_t{"x", 1});
    }
    w.EndValue();
  }
  ...
  w.BeginValue(kCap + 1, 0);
  w.Emit<irs::TokenLayout::Terms>(duckdb::string_t{"y", 1});
  w.EndValue();
  w.Finish();
