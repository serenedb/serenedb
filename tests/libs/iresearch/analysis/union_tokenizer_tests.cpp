...
irs::analysis::Tokenizer::ptr MakeNGram(size_t min_gram, size_t max_gram,
  ...
  return irs::analysis::NGramTokenizer::Make(
...
TEST(union_tokenizer_test, empty_union) {
  irs::analysis::UnionTokenizer u({});
  const auto tokens =
    tests::Analyze(u, "hello world", irs::TokenLayout::TermsPos);
  ASSERT_TRUE(tokens.has_value());
  ASSERT_TRUE(tokens->empty());

  auto made = irs::analysis::UnionTokenizer::Make(
    irs::analysis::UnionTokenizer::Options{}, tests::Cache());
  ASSERT_NE(nullptr, made);
  const auto made_tokens =
    tests::Analyze(*made, "hello world", irs::TokenLayout::TermsPos);
...
TEST(union_tokenizer_test, VisitMembers) {
  ...
  AssertUnionMembers(u, {irs::Type<irs::analysis::TextTokenizer>::id(),
                         irs::Type<irs::analysis::NGramTokenizer>::id()});
}
...
TEST(union_tokenizer_test, options_construction) {
  ...
  auto stream =
    irs::analysis::UnionTokenizer::Make(std::move(opts), tests::Cache());
...
class OverflowingFailingTokenizer final
  : public irs::analysis::TypedTokenizer<OverflowingFailingTokenizer> {
 public:
  ...
  bool DoFill(duckdb::string_t, irs::TokenSink& sink) {
    for (size_t i = 0; i < 2 * irs::TokenBatch::kCapacity; ++i) {
      sink.Emit<Layout>(irs::MakeTermView(std::string_view{"leak"}));
    }
    return false;
  }
};
...
TEST(union_tokenizer_test, failed_child_contributes_nothing_after_flush) {
  std::vector<irs::analysis::Tokenizer::ptr> subs;
  subs.push_back(std::make_unique<OverflowingFailingTokenizer>());
  subs.push_back(std::make_unique<irs::KeywordTokenizer>());
  irs::analysis::UnionTokenizer u(std::move(subs));

  const UnionTokens expected{{"x", 1, 1}};
  AssertUnion(&u, "x", expected);
}
