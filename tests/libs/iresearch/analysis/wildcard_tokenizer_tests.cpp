...
std::unique_ptr<irs::analysis::WildcardTokenizer> MakeWildcard(
  ...
  return std::make_unique<irs::analysis::WildcardTokenizer>(std::move(base),
                                                            ngram_size);
}
...
std::vector<irs::bstring> PullTokens(irs::analysis::WildcardTokenizer& stream,
...
std::vector<irs::bstring> FillTokens(irs::analysis::WildcardTokenizer& stream,
...
TEST(wildcard_tokenizer_tests, native_fills_match_pull_multi_term_base) {
  std::string many;
...
TEST(wildcard_tokenizer_tests, fill_flushes_across_batches) {
  std::string many;
...
TEST(wildcard_tokenizer_tests, store_survives_fill_reset) {
  auto stream = MakeWildcard(3);
  ASSERT_TRUE(stream->Traits().store);
...
TEST(wildcard_tokenizer_tests, column_fill_matches_per_value) {
  auto stream = MakeWildcard(3);
...
TEST(wildcard_tokenizer_tests, base_fill_failure_leaves_no_residue) {
  ...
  irs::analysis::WildcardTokenizer fresh{std::make_unique<FlakyBaseTokenizer>(),
                                         3};
...
TEST(wildcard_tokenizer_tests, base_without_tokens_is_not_a_failure) {
  irs::analysis::WildcardTokenizer stream{
...
TEST(wildcard_tokenizer_tests, grams_follow_the_base_output_not_the_input) {
  irs::analysis::WildcardTokenizer via_base{
