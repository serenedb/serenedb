...
class InsertSink final : public irs::TokenConsumer {
 public:
  InsertSink(irs::TokenLayout layout, InsertBlockFn& insert_block)
    : layout{layout}, _insert_block(insert_block) {
...
bool InsertFieldImpl(irs::doc_id_t doc, FieldT&& field,
  ...
  analyzer.Fill(tests::ToStringT(field.Value()), doc, sink.writer,
                {sink.layout});
  sink.writer.Finish();
  return sink.ok();
}
...
bool InsertFieldTokens(const irs::IndexWriter::Document& doc, FieldT&& field) {
  return doc.WithTokens(field.Id(), field.GetIndexFeatures(), nullptr,
                        [&](irs::FieldInverter& fld, irs::TokenSink& w) {
                          ...
                          field.GetTokens().Fill(
                            tests::ToStringT(field.Value()), doc.DocId(), w,
                            {fld.Layout()});
                        });
}
...
bool InsertField(irs::SegmentWriter& writer, irs::doc_id_t doc,
                 FieldT&& field) {
  return writer.WithField(
    field.Id(), field.GetIndexFeatures(), [&](irs::FieldInverter& fld) {
      fld.Configure(field.GetTokens().Traits());
      return InsertFieldImpl(doc, field,
                             [&](irs::TokenBatch& batch, auto& runs) {
