...
TEST_P(BlockScoringTestCase, TfidfBytermBlockScoring) {
  ...
  std::vector<irs::score_t> scores(irs::BlockSize(kTopK));
  std::vector<irs::doc_id_t> docs(irs::BlockSize(kTopK));
...
TEST_P(BlockScoringTestCase, TfidfVsBm25Comparison) {
  ...
  // Get TFIDF results
  std::vector<irs::score_t> tfidf_scores(irs::BlockSize(kTopK));
  std::vector<irs::doc_id_t> tfidf_docs(irs::BlockSize(kTopK));
  size_t tfidf_count =
    irs::ExecuteTopKWithCount(reader, *filter, tfidf_order, kTopK,
                              std::span{tfidf_scores}, std::span{tfidf_docs});

  // Get BM25 results
  std::vector<irs::score_t> bm25_scores(irs::BlockSize(kTopK));
  std::vector<irs::doc_id_t> bm25_docs(irs::BlockSize(kTopK));
  size_t bm25_count =
...
TEST_P(BlockScoringTestCase, TfidfWithWithoutNorms) {
  ...
  // Test with norms
  {
    ...
    std::vector<irs::score_t> scores_with_norms(irs::BlockSize(kTopK));
    std::vector<irs::doc_id_t> docs_with_norms(irs::BlockSize(kTopK));
  ...
  // Test without norms
  {
    ...
    std::vector<irs::score_t> scores_without_norms(irs::BlockSize(kTopK));
    std::vector<irs::doc_id_t> docs_without_norms(irs::BlockSize(kTopK));
