...
struct SegmentReaderMock final : irs::SubReader {
  ...
  const irs::ColumnReader* column(std::string_view) const final { return nullptr; }
...
TEST(block_disjunction_test, min_match_next) {
  ...
  // single iterator case, unreachable condition
  {
    ...
    {
      Disjunction it(detail::ExecuteAll<irs::ScoreAdapter>(docs), 2U, irs::doc_limits::eof());
      auto* doc = irs::get<irs::DocAttr>(it);
      ASSERT_TRUE(bool(doc));
      ASSERT_FALSE(irs::doc_limits::valid(doc->value));
      ASSERT_EQ(std::accumulate(docs.begin(), docs.end(), size_t(0), sum),
  ...
  // early break
  {
    ...
    std::vector<irs::doc_id_t> result;
    {
      Disjunction it(detail::ExecuteAll<irs::ScoreAdapter>(docs), 3U, irs::doc_limits::eof());
      auto* doc = irs::get<irs::DocAttr>(it);
      ASSERT_TRUE(bool(doc));
      ASSERT_FALSE(irs::doc_limits::valid(it.value()));
      ASSERT_EQ(std::accumulate(docs.begin(), docs.end(), size_t(0), sum),
  ...
  // values don't fit single block
  {
    ...
    std::vector<irs::doc_id_t> result;
    {
      Disjunction it(detail::ExecuteAll<irs::ScoreAdapter>(docs), 1U, irs::doc_limits::eof());
      auto* doc = irs::get<irs::DocAttr>(it);
      ASSERT_FALSE(irs::doc_limits::valid(it.value()));
      ASSERT_TRUE(bool(doc));
      ASSERT_EQ(std::accumulate(docs.begin(), docs.end(), size_t(0), sum),
  ...
  // min_match == 0
  {
    ...
    std::vector<irs::doc_id_t> result;
    {
      Disjunction it(detail::ExecuteAll<irs::ScoreAdapter>(docs), 0U, irs::doc_limits::eof());
      auto* doc = irs::get<irs::DocAttr>(it);
      ASSERT_FALSE(irs::doc_limits::valid(it.value()));
      ASSERT_TRUE(bool(doc));
      ASSERT_EQ(std::accumulate(docs.begin(), docs.end(), size_t(0), sum),
