 ...
 protected:
  ...
  void SimpleSequentialOrder() {
    ...
    // 'seq' column
    {
      ...
      sort.scorer_score = [&](irs::ScoreFunctionImpl* ctx, irs::score_t* score,
                              size_t n) -> void {
        ASSERT_EQ(1, n);
        ++scorer_score_count;
...
TEST_P(ColumnExistenceLongFilterTestCase, mixed_seeks) {
  ...
  class PatternDocGenerator : public tests::DocGeneratorBase {
   public:
    PatternDocGenerator(std::string_view all_docs_field,
                        std::string_view some_docs_field, size_t total_docs,
                        std::span<irs::doc_id_t> with_field)
      : all_docs_field{all_docs_field},
        some_docs_field{some_docs_field},
        with_field{with_field},
        max_doc_id(total_docs + 1) {}
    const tests::Document* next() final {
      if (produced_docs >= max_doc_id) {
        return nullptr;
      }
      ++produced_docs;
      doc.clear();
    ...
    tests::Document doc;
    std::string all_docs_field;
    std::string some_docs_field;
    std::span<irs::doc_id_t> with_field;
    size_t produced_docs{0};
    size_t max_doc_id;
    size_t span_index{0};
  };
