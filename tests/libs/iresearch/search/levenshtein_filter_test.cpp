...
TEST_P(ByEditDistanceTestCase, parallel_prepare_parity) {
  {
  ...
  auto run = [&](const irs::ByEditDistance& filter) {
    MaxMemoryCounter counter;
  ...
  auto with_boost = [](irs::ByEditDistance q, irs::score_t b) {
    q.boost(b);
    return q;
  };

  // distance 0 == exact term match
  run(MakeFilter("prefix", "abc", 0));
  run(with_boost(MakeFilter("prefix", "abc", 0), 2.5f));
  // distance >0, all-terms variant (max_terms=0)
  run(MakeFilter("prefix", "abc", 1, 0));
  run(with_boost(MakeFilter("prefix", "abc", 1, 0), 0.5f));
  // distance >0, top-terms variant (max_terms>0)
  run(MakeFilter("prefix", "abc", 1, 5));
  run(with_boost(MakeFilter("prefix", "abc", 1, 5), 1.75f));
}
