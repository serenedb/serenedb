...
std::vector<std::pair<irs::doc_id_t, irs::score_t>> ExecuteScored(
  ...
  MaxMemoryCounter counter;
  auto prepared =
    q.prepare({.index = *reader, .memory = counter, .scorer = &scorer});
  EXPECT_NE(nullptr, prepared);
  ...
  for (const auto& segment : *reader) {
    auto docs = prepared->execute({.segment = segment, .scorer = &scorer});
...
TEST(PhraseNgramFilterTest, phrase_frequency_scoring) {
  ...
  const irs::TFIDF scorer;  // freq-only (no norms): score is freq-monotonic
  const auto scored =
  ...
  EXPECT_EQ(scored[0].second, scored[1].second);  // same phrase freq
  EXPECT_GT(scored[2].second, scored[1].second);  // higher phrase freq
...
TEST(PhraseNgramFilterTest, phrase_frequency_scoring_with_filler) {
  ...
  EXPECT_EQ(Ids({0, 1}), (std::vector<irs::doc_id_t>{scored[0].first,
                                                     scored[1].first}));
  EXPECT_GT(scored[0].second, 0.f);
  EXPECT_GT(scored[1].second, scored[0].second);
}
