...
TEST_P(PrefixFilterTestCase, parallel_prepare_parity) {
  {
    auto writer = open_writer(irs::kOmCreate);
  ...
  auto run = [&](const irs::ByPrefix& filter) {
    MaxMemoryCounter counter;
  ...
  auto with_boost = [](irs::ByPrefix q, irs::score_t b) {
    q.boost(b);
    return q;
  };

  run(MakeFilter("prefix", "ab"));
  run(with_boost(MakeFilter("prefix", "ab"), 2.5f));
  run(MakeFilter("prefix", "no_such_prefix_xyz"));
  run(with_boost(MakeFilter("prefix", "ab", 2), 0.5f));
}
