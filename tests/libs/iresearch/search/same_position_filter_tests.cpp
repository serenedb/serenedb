...
TEST_P(SamePositionFilterTestCase, parallel_prepare_parity) {
  tests::JsonDocGenerator gen(
    resource("same_position.json"),
    [](tests::Document& doc, const std::string& name,
       const tests::JsonDocGenerator::JsonValue& data) {
      ...
      if (data.is_string()) {
        ...
        auto& field = (doc.indexed.end() - 1).as<tests::LongField>();
        field.Name(name);
        field.value(data.as_number<int64_t>());
      }
    });
  add_segment(gen);
  gen.reset();
  ...
  tests::RunParallelPrepareParity(make(irs::kNoBoost), rdr);
  tests::RunParallelPrepareParity(make(2.5f), rdr);
  tests::RunParallelPrepareParity(make(0.5f), rdr);
}
