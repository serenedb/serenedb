...
TEST_P(TfidfTestCase, test_collector_merge) {
  {
    tests::JsonDocGenerator gen(resource("simple_sequential.json"),
                                &tests::PayloadedJsonFieldFactory);
  ...
  {
    auto a = prepared_sort.PrepareFieldCollector();
    auto b = prepared_sort.PrepareFieldCollector();
    auto c = prepared_sort.PrepareFieldCollector();
    ASSERT_NE(nullptr, a);
    ASSERT_NE(nullptr, b);
    ASSERT_NE(nullptr, c);
  ...
  {
    auto populated = prepared_sort.PrepareFieldCollector();
    auto fresh = prepared_sort.PrepareFieldCollector();
  ...
  {
    irs::FieldCollectors a{&prepared_sort};
    irs::FieldCollectors b{&prepared_sort};
    irs::FieldCollectors c{&prepared_sort};
  ...
  {
    auto a = prepared_sort.PrepareTermCollector();
    auto b = prepared_sort.PrepareTermCollector();
    auto c = prepared_sort.PrepareTermCollector();
    ASSERT_NE(nullptr, a);
    ASSERT_NE(nullptr, b);
    ASSERT_NE(nullptr, c);
  ...
  {
    irs::TermCollectors a{&prepared_sort, 1};
    irs::TermCollectors b{&prepared_sort, 1};
    irs::TermCollectors c{&prepared_sort, 1};
