...
TEST(fst_table_matcher_test, static_const) {
  static_assert(fst::MATCH_INPUT ==
                fst::TableMatcher<
                  fst::fsa::Automaton<fst::fsa::BooleanWeight>>::kMatchType);
...
TEST(fst_table_matcher_test, test_matcher) {
  ...
  // complex automaton
  {
    ...
    // check automaton
    {
      using MatcherT = fst::TableMatcher<decltype(a)>;
      using ExpectedMatcherT = fst::SortedRangeExplicitMatcher<decltype(a)>;

      ExpectedMatcherT expected_matcher(&a);
      MatcherT matcher(a, true);
      for (decltype(a)::StateId state = 0; state < a.NumStates(); ++state) {
        expected_matcher.SetState(state);
        matcher.SetState(state);

        for (MatcherT::Label min = 1, max = 2049; min < max; ++min) {
