...
void AssertDescription(
  const irs::ParametricDescription& description, const irs::bytes_view& prefix,
  ...
  for (auto& entry : candidates) {
    ...
    const std::span<const uint32_t> utf8_target_ref(utf8_target.data(),
                                                    utf8_target.size());
    const std::span<const uint32_t> utf8_candidate_ref(utf8_candidate.data(),
                                                       utf8_candidate.size());

    ASSERT_EQ(
      expected_distance_precise,
      irs::EditDistance(utf8_candidate_ref.data(), utf8_candidate_ref.size(),
                        utf8_target_ref.data(), utf8_target_ref.size()));
    ASSERT_EQ(
      expected_distance_precise,
      irs::EditDistance(utf8_target_ref.data(), utf8_target_ref.size(),
                        utf8_candidate_ref.data(), utf8_candidate_ref.size()));

    {
      size_t actual_distance;
      if (irs::EditDistance(actual_distance, description, candidate, target)) {
        ASSERT_EQ(expected_distance, actual_distance);
        ASSERT_EQ(expected_distance,
                  irs::EditDistance(description, candidate, target));
      }
    }

    {
      size_t actual_distance;
      if (irs::EditDistance(actual_distance, description, target, candidate)) {
        ASSERT_EQ(expected_distance, actual_distance);
        ASSERT_EQ(expected_distance,
                  irs::EditDistance(description, target, candidate));
      }
    }
...
void AssertDescription(
  const irs::ParametricDescription& description, const irs::bytes_view& target,
  const std::vector<std::tuple<irs::bytes_view, size_t, size_t, size_t>>&
    candidates) {
  return AssertDescription(description, irs::kEmptyStringView<irs::byte_type>,
                           target, candidates);
}

void AssertReadWrite(const irs::ParametricDescription& description) {
  irs::bstring buf;
  {
  ...
  }
  {
    ...
    ASSERT_EQ(description, irs::ParametricDescription::Read(in));
  }
}
...
TEST(levenshtein_utils_test, test_distance) {
  {
    ...
    ASSERT_EQ(
      2, irs::EditDistance(lhs.data(), lhs.size(), rhs.data(), rhs.size()));
    ASSERT_EQ(
      2, irs::EditDistance(rhs.data(), rhs.size(), lhs.data(), lhs.size()));
  }

  {
    ...
    ASSERT_EQ(
      3, irs::EditDistance(lhs.data(), lhs.size(), rhs.data(), rhs.size()));
    ASSERT_EQ(
      3, irs::EditDistance(rhs.data(), rhs.size(), lhs.data(), lhs.size()));
  }

  {
    ...
    ASSERT_EQ(4, irs::EditDistance(lhs_utf8.data(), lhs_utf8.size(),
                                   rhs_utf8.data(), rhs_utf8.size()));
    ASSERT_EQ(4, irs::EditDistance(rhs_utf8.data(), rhs_utf8.size(),
...

  {
    ...
    ASSERT_EQ(
      0, irs::EditDistance(lhs.data(), lhs.size(), rhs.data(), rhs.size()));
    ASSERT_EQ(
      0, irs::EditDistance(rhs.data(), rhs.size(), lhs.data(), lhs.size()));
  }
...
TEST(levenshtein_utils_test, test_static_const) {
  ASSERT_EQ(31, decltype(irs::ParametricDescription::kMaxDistance)(
                  irs::ParametricDescription::kMaxDistance));
}

TEST(levenshtein_utils_test, test_description_0) {
  auto assert_distance = [](const irs::ParametricDescription& d) {
    // state 0
  ...
  auto assert_transitions = [](const irs::ParametricDescription& d) {
    // state 0
    ASSERT_EQ(irs::ParametricDescription::transition_t(0, 0),
              d.transition(0, 0));
    ASSERT_EQ(irs::ParametricDescription::transition_t(0, 0),
              d.transition(0, 1));
    // state 1
    ASSERT_EQ(irs::ParametricDescription::transition_t(0, 0),
              d.transition(1, 0));
    ASSERT_EQ(irs::ParametricDescription::transition_t(1, 1),
              d.transition(1, 1));
  };

  // no transpositions
  {
    auto description = irs::MakeParametricDescription(0, false);
    ...
    ASSERT_EQ(description, irs::MakeParametricDescription(0, true));
    ASSERT_EQ(description, irs::MakeParametricDescription(0, false));
    assert_distance(description);
    assert_transitions(description);
    AssertReadWrite(description);
  ...
  // no transpositions with prefix
  {
    ...
    AssertDescription(description, irs::ViewCast<irs::byte_type>("alpha"sv),
  ...
  // transpositions
  {
    auto description = irs::MakeParametricDescription(0, true);
    ASSERT_TRUE(bool(description));
    ASSERT_EQ(2, description.size());
    ASSERT_EQ(1, description.chi_size());
    ASSERT_EQ(2, description.chi_max());
    ...
    AssertDescription(description, irs::ViewCast<irs::byte_type>("alphabet"sv),
                      {
                        {irs::ViewCast<irs::byte_type>("alphabet"sv), 0, 0, 0},
                        {irs::ViewCast<irs::byte_type>("alphabez"sv), 1, 1, 1},
                        {irs::ViewCast<irs::byte_type>("alphaet"sv), 1, 1, 1},
  ...
  // transpositions with prefix
  {
    ...
    AssertDescription(description, irs::ViewCast<irs::byte_type>("al"sv),
...
TEST(levenshtein_utils_test, test_description_1) {
  // no transpositions
  {
    ...
    auto assert_transitions = [](const irs::ParametricDescription& d) {
      ...
      ASSERT_EQ(irs::ParametricDescription::transition_t(0, 0),
                d.transition(0, 7));
      // state 1
      ASSERT_EQ(irs::ParametricDescription::transition_t(2, 0),
                d.transition(1, 0));
      ASSERT_EQ(irs::ParametricDescription::transition_t(1, 1),
                d.transition(1, 1));
      ASSERT_EQ(irs::ParametricDescription::transition_t(3, 0),
                d.transition(1, 2));
      ASSERT_EQ(irs::ParametricDescription::transition_t(1, 1),
                d.transition(1, 3));
      ASSERT_EQ(irs::ParametricDescription::transition_t(2, 0),
      ...
      ASSERT_EQ(irs::ParametricDescription::transition_t(0, 0),
                d.transition(2, 0));
      ASSERT_EQ(irs::ParametricDescription::transition_t(4, 1),
                d.transition(2, 1));
      ASSERT_EQ(irs::ParametricDescription::transition_t(4, 2),
                d.transition(2, 2));
      ASSERT_EQ(irs::ParametricDescription::transition_t(2, 1),
                d.transition(2, 3));
      ASSERT_EQ(irs::ParametricDescription::transition_t(0, 0),
                d.transition(2, 4));
      ASSERT_EQ(irs::ParametricDescription::transition_t(4, 1),
      ...
      ASSERT_EQ(irs::ParametricDescription::transition_t(2, 1),
                d.transition(3, 3));
      ASSERT_EQ(irs::ParametricDescription::transition_t(4, 3),
                d.transition(3, 4));
      ASSERT_EQ(irs::ParametricDescription::transition_t(5, 1),
                d.transition(3, 5));
      ASSERT_EQ(irs::ParametricDescription::transition_t(2, 2),
                d.transition(3, 6));
      ASSERT_EQ(irs::ParametricDescription::transition_t(3, 1),
                d.transition(3, 7));
      // state 4
      ASSERT_EQ(irs::ParametricDescription::transition_t(0, 0),
                d.transition(4, 0));
    ...
    auto description = irs::MakeParametricDescription(1, false);
    ASSERT_TRUE(bool(description));
    ...
    ASSERT_NE(description, irs::MakeParametricDescription(1, true));
    ASSERT_EQ(description, irs::MakeParametricDescription(1, false));
    assert_distance(description);
    assert_transitions(description);
    AssertReadWrite(description);
  ...
  // transpositions
  {
    auto description = irs::MakeParametricDescription(1, true);
    ASSERT_TRUE(bool(description));
    ASSERT_EQ(8, description.size());
    ASSERT_EQ(3, description.chi_size());
    ASSERT_EQ(8, description.chi_max());
    ASSERT_EQ(1, description.max_distance());
    ASSERT_EQ(description, irs::MakeParametricDescription(1, true));
    ASSERT_NE(description, irs::MakeParametricDescription(1, false));
    AssertReadWrite(description);
    ...
    // with prefix
    AssertDescription(description, irs::ViewCast<irs::byte_type>("alph"sv),
...
TEST(levenshtein_utils_test, test_description_2) {
  // no transpositions
  {
    auto description = irs::MakeParametricDescription(2, false);
    ASSERT_TRUE(bool(description));
    ...
    ASSERT_NE(description, irs::MakeParametricDescription(2, true));
    AssertReadWrite(description);
  ...
  // transpositions
  {
    auto description = irs::MakeParametricDescription(2, true);
    ASSERT_TRUE(bool(description));
    ASSERT_EQ(68, description.size());
    ASSERT_EQ(5, description.chi_size());
    ASSERT_EQ(32, description.chi_max());
    ASSERT_EQ(2, description.max_distance());
    ASSERT_NE(description, irs::MakeParametricDescription(2, false));
    AssertReadWrite(description);
    ...
    // with prefix
    AssertDescription(description, irs::ViewCast<irs::byte_type>("alp"sv),
...
TEST(levenshtein_utils_test, test_description_3) {
  // no transpositions
  {
    auto description = irs::MakeParametricDescription(3, false);
    ASSERT_TRUE(bool(description));
    ...
    // with prefix
    AssertDescription(description, irs::ViewCast<irs::byte_type>("a"sv),
                      irs::ViewCast<irs::byte_type>("lphabet"sv),
                      {
                        {irs::ViewCast<irs::byte_type>("alphabet"sv), 0, 0, 0},
                        {irs::ViewCast<irs::byte_type>("alphabez"sv), 1, 1, 1},
    ...
    // "aec" vs "abc"
    ASSERT_EQ(
      1, irs::EditDistance(description, irs::ViewCast<irs::byte_type>("aec"sv),
                           irs::ViewCast<irs::byte_type>("abc"sv)));
    ASSERT_EQ(
      1, irs::EditDistance(description, irs::ViewCast<irs::byte_type>("abc"sv),
    ...
    ASSERT_EQ(
      1, irs::EditDistance(description, irs::ViewCast<irs::byte_type>("ac"sv),
                           irs::ViewCast<irs::byte_type>("aec"sv)));
    // "aec" vs "zaec"
    ASSERT_EQ(
      1, irs::EditDistance(description, irs::ViewCast<irs::byte_type>("aec"sv),
    ...
    ASSERT_EQ(
      1, irs::EditDistance(description, irs::ViewCast<irs::byte_type>("zaec"sv),
                           irs::ViewCast<irs::byte_type>("aec"sv)));
    // "aec" vs "abcd"
    ASSERT_EQ(
      2, irs::EditDistance(description, irs::ViewCast<irs::byte_type>("aec"sv),
                           irs::ViewCast<irs::byte_type>("abcd"sv)));
    ASSERT_EQ(
      2, irs::EditDistance(description, irs::ViewCast<irs::byte_type>("abcd"sv),
                           irs::ViewCast<irs::byte_type>("aec"sv)));
    // "aec" vs "abcdz"
    ASSERT_EQ(
      3, irs::EditDistance(description, irs::ViewCast<irs::byte_type>("aec"sv),
                           irs::ViewCast<irs::byte_type>("abcdz"sv)));
    ASSERT_EQ(3, irs::EditDistance(description,
                                   irs::ViewCast<irs::byte_type>("abcdz"sv),
                                   irs::ViewCast<irs::byte_type>("aec"sv)));
    ...
    ASSERT_EQ(
      ...
      irs::EditDistance(description, irs::ViewCast<irs::byte_type>("aec"sv),
  ...
  // transpositions
  {
    auto description = irs::MakeParametricDescription(3, true);
    ASSERT_TRUE(bool(description));
...
TEST(levenshtein_utils_test, test_description_4) {
  // no transpositions
  {
    auto description = irs::MakeParametricDescription(4, false);
    ASSERT_TRUE(bool(description));
  ...
  //  // with transpositions
  //  {
  //    auto description = irs::MakeParametricDescription(4, true);
  //    ASSERT_TRUE(bool(description));
...
TEST(levenshtein_utils_test, test_description_invalid) {
  ASSERT_FALSE(irs::ParametricDescription());

  // no transpositions
  {
    auto description = irs::MakeParametricDescription(
      irs::ParametricDescription::kMaxDistance + 1, false);
    ASSERT_FALSE(bool(description));
    ASSERT_EQ(0, description.size());
    ASSERT_EQ(0, description.chi_size());
    ASSERT_EQ(0, description.chi_max());
    ASSERT_EQ(0, description.max_distance());
    ASSERT_EQ(irs::ParametricDescription(), description);
    ASSERT_NE(description, irs::MakeParametricDescription(0, false));
  }

  // transpositions
  {
    auto description = irs::MakeParametricDescription(
      irs::ParametricDescription::kMaxDistance + 1, true);
    ASSERT_FALSE(bool(description));
    ASSERT_EQ(0, description.size());
    ASSERT_EQ(0, description.chi_size());
    ASSERT_EQ(0, description.chi_max());
