...
TEST_F(RegexpUtilsTest, unescape_single_metacharacters) {
  irs::bstring out;
  EXPECT_EQ(".", irs::ViewCast<char>(
    irs::UnescapeRegexp(ToBytesView("\\."), out)));
  EXPECT_EQ("*", irs::ViewCast<char>(
    irs::UnescapeRegexp(ToBytesView("\\*"), out)));
  EXPECT_EQ("+", irs::ViewCast<char>(
    irs::UnescapeRegexp(ToBytesView("\\+"), out)));
...
TEST_F(RegexpUtilsTest, invariant_properties_broad_coverage) {
  constexpr std::string_view kPatterns[] = {
    ...
    "a*", "a+", "a?", "a{3}", "a{2,5}", "a{3,}",
    // Alternation
    "a|b", "cat|dog|bird", "foo|foobar", "a|", "|a",
    // Groups
    "(ab)+", "(a|b)*", "(?:ab)+", "(?P<n>ab)", "((a|b)*c)+",
    // Char classes
    ...
    "(?i:a)", "(?i:A)", "(?i:k)", "(?i:s)", "(?i:abc)",
    "(?i:ab)(?i:cd)", "(?i:foo|bar)",
    // Combined
    "foo.*bar", ".*foo.*", "(a|b)c*d?", "\\p{Cyrillic}+",
    // Unicode ranges spanning byte-length boundaries
    "[\\x{70}-\\x{A0}]", "[\\x{7F0}-\\x{810}]",
    "[\\x{FFFE}-\\x{10001}]",
    // Any-byte
    "a\\Cb",
  };
...
TEST_F(RegexpUtilsTest, char_class_all_four_byte_tiers) {
  ...
  EXPECT_TRUE(Accepts(a, "a\xD0\xB1\xE4\xB8\xAD\xF0\x9F\x98\x80"));  // all together
  EXPECT_FALSE(Accepts(a, "b"));
  EXPECT_FALSE(Accepts(a, ""));
}
...
TEST_F(RegexpUtilsTest, char_class_ranges_across_tiers) {
  ...
  EXPECT_TRUE(Accepts(a, "a\xD0\xBF\xE4\xB8\xAD"));        // mixed: a + п + 中 all in range
  EXPECT_FALSE(Accepts(a, "ABC"));                         // uppercase not in range
  EXPECT_FALSE(Accepts(a, "\xE4\xB8\xAD\xE6\x96\x87"));    // 中文 - 文 (U+6587) outside range
  EXPECT_FALSE(Accepts(a, ""));
}
...
TEST_F(RegexpUtilsTest, extract_utf8_prefix) {
  ...
  EXPECT_EQ("привет",
    irs::ViewCast<char>(irs::ExtractRegexpPrefix(ToBytesView("привет.*"))));
  EXPECT_EQ("中文",
    irs::ViewCast<char>(irs::ExtractRegexpPrefix(ToBytesView("中文.*"))));
}
