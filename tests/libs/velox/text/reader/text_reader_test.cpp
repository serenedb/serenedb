...
TEST_F(TextReaderTest, headerAndCustomNullString) {
  auto expected = makeRowVector({
      ...
      makeNullableFlatVector<double>(
          {std::nullopt, // " 1.123 " has trailing space → null
           2.333,
           -6.1,
           4.2,
           std::nullopt, // " 47.2 " has trailing space → null
           79.5,
           std::nullopt, // "3.1415926\t\t  " has trailing whitespace → null
           -221.145,
           93.12,
           std::nullopt, // " \t-4123.11  " has trailing spaces → null
           950.2,
           43.66,
           std::nullopt}),
...
TEST_F(TextReaderTest, escapeCharacter) {
  // File layout (SOH = 0x01 field delimiter, \ = escape char):
  //   hello\<SOH>world<SOH>1   → col_str="hello\x01world",   col_int=1
  //   back\\slash<SOH>2        → col_str="back\slash",       col_int=2
  //   new\nline<SOH>3          → col_str="new\nline",        col_int=3
  //   carriage\rreturn<SOH>4   → col_str="carriage\rreturn", col_int=4
  //   plain<SOH>5              → col_str="plain",            col_int=5
  auto expected = makeRowVector({
      makeFlatVector<std::string>(
          {"hello\x01world",
           "back\\slash",
  ...
  auto type = ROW({{"col_str", VARCHAR()}, {"col_int", INTEGER()}});
  auto factory = dwio::common::getReaderFactory(dwio::common::FileFormat::TEXT);

  auto path = velox::test::getDataFilePath(
      "velox/dwio/text/tests/reader/", "examples/escape_test");
  auto readFile = std::make_shared<LocalReadFile>(path);

  auto readerOptions = dwio::common::ReaderOptions(pool());
  readerOptions.setFileSchema(type);
  auto serDeOptions = readerOptions.serDeOptions();
  serDeOptions.isEscaped = true;
  serDeOptions.escapeChar = '\\';
  readerOptions.setSerDeOptions(serDeOptions);
...
TEST_F(TextReaderTest, escapeCharacterCommaDelimiter) {
  // File layout (comma as field delimiter, \ as escape char):
  //   one\,two,1   → col_str="one,two", col_int=1  (\, = literal comma)
  //   hello,2      → col_str="hello",   col_int=2
  //   a\\b,3       → col_str="a\b",     col_int=3  (\\ = single backslash)
  auto expected = makeRowVector({
      makeFlatVector<std::string>({"one,two", "hello", "a\\b"}),
  ...
  auto path = velox::test::getDataFilePath(
      "velox/dwio/text/tests/reader/", "examples/escape_comma_test");
  auto readFile = std::make_shared<LocalReadFile>(path);

  auto readerOptions = dwio::common::ReaderOptions(pool());
  readerOptions.setFileSchema(type);
  auto serDeOptions = readerOptions.serDeOptions();
  serDeOptions.separators[0] = ',';
  serDeOptions.isEscaped = true;
  serDeOptions.escapeChar = '\\';
  readerOptions.setSerDeOptions(serDeOptions);
