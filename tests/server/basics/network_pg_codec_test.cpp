...
TEST(NetworkPgCodec, TypedFrameFragmentation) {
  ...
  const auto whole = PgFrameCodec::Parse(frame, true, kMax);
  ASSERT_EQ(whole.status, FrameStatus::Ok);
  EXPECT_EQ(whole.consumed, frame.size());
  EXPECT_EQ(whole.frame.type, PQ_MSG_QUERY);
  EXPECT_EQ(whole.frame.payload, std::string_view("SELECT 1\0", 9));
}
...
TEST(NetworkPgCodec, StartupAndSslAndCancelCodes) {
  ...
  const auto ssl_parsed =
    PgFrameCodec::Parse(ssl, false, MAX_STARTUP_PACKET_LENGTH);
  ...
  EXPECT_EQ(PgFrameCodec::StartupCode(cancel_parsed.frame.payload),
...
TEST(NetworkPgCodec, TooLargeAndMalformed) {
  ...
  bad.append(length, 4);
  const auto malformed = PgFrameCodec::Parse(bad, true, kMax);
