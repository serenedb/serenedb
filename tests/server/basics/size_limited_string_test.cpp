...
TEST(SizeLimitedStringTest, test_append) {
  ...
  constexpr std::string_view kValue("the fox");
  testee.append(kValue);
  EXPECT_EQ(kValue, testee.view());
  EXPECT_EQ(kValue.size(), testee.size());
...
TEST(SizeLimitedStringTest, test_too_long_string) {
  ...
  constexpr std::string_view kValue("the quick brown fox jumped");
  testee.append(kValue);
  EXPECT_EQ(kValue.substr(0, 10), testee.view());
