...
TEST(PrimaryKeyTermTest, KeyTermMatchesStringEncoders) {
  for (const int64_t v :
    ...
    const auto term = connector::duckdb_primary_key::SignedKeyTerm(v);
    ASSERT_EQ(key, std::string_view(term.GetData(), term.GetSize())) << v;
  }
  for (const uint64_t g : {uint64_t{0}, uint64_t{1}, uint64_t{1} << 63,
                           std::numeric_limits<uint64_t>::max()}) {
    std::string key;
    connector::duckdb_primary_key::AppendGenerated(key, g);
    const auto term = connector::duckdb_primary_key::GeneratedKeyTerm(g);
...
TEST(TokenizerPoolTest, ReturnedGeoLeaseIsUnbound) {
  catalog::Tokenizer dict(
    catalog::Permissions{}, ObjectId{0}, ObjectId{54321}, "test_geojson", {},
