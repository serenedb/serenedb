...
launch_external() {
	...
	# Generation needs the docker daemon with HOST paths: skip inside any
	# container (compose runners get the fixture generated by run_in_docker.sh
	# on the host before compose starts).
	if [[ "$needs_iceberg_fixture" == "true" && -z "${COMPOSE_NETWORK:-}" &&
