...
if ! test -f "$SQLLOGIC_DIR/run_recovery_in_docker.sh"; then
...
docker compose -f docker-compose.recovery.yml up --attach tests --exit-code-from tests --remove-orphans
test_exit_code=$?
if ! test "${test_exit_code}" -eq "0"; then
  echo "Recovery tests failed!"
  echo "serenedb-single container log begin:"
  docker compose logs serenedb-single
