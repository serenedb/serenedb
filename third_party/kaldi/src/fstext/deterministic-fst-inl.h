...
bool BackoffDeterministicOnDemandFst<Arc>::GetArc(StateId s, Label ilabel,
  ...
  if (sm.Find(ilabel)) {
    ...
    *oarc = arc;
    ...
    if (backoff_state == kNoStateId)
      return false;
    if (!this->GetArc(backoff_state, ilabel, oarc))
      return false;
...
bool ComposeDeterministicOnDemandFst<Arc>::GetArc(StateId s, Label ilabel,
  ...
  Arc arc1;
  if (!fst1_->GetArc(pr.first, ilabel, &arc1))
    return false;
  ...
  // second fst also.
  Arc arc2;
  if (!fst2_->GetArc(pr.second, arc1.olabel, &arc2))
    return false;
  std::pair<const std::pair<StateId, StateId>, StateId> new_value(
  ...
  while (!state_queue.empty()) {
    ...
    for (ArcIterator<Fst<Arc>> aiter(right, q_right); !aiter.Done();
      ...
      if (arc_right.ilabel == 0) {
        ...
        fst_composed->AddArc(
          state_map[q], Arc(0, arc_right.olabel, arc_right.weight, next_state));
      } else {
        fst_composed->AddArc(
          state_map[q],
