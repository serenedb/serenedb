...
class LatticeStringRepository {
 public:
  ...
  bool IsPrefixOf(const Entry* a, const Entry* b) const {
    if (a == NULL)
      return true;  // empty string prefix of all.
    if (a == b)
      return true;
    if (b == NULL)
      return false;
...
class LatticeDeterminizer {
 ...
 private:
  ...
  struct TempArc {
    Label ilabel;
    StringId
      string;  // Look it up in the StringRepository, it's a sequence of Labels.
  ...
  bool
    determinized_;  // set to true when user called Determinize(); used to make
  // sure this object is used correctly.
  MinimalSubsetHash
    minimal_hash_;  // hash from Subset to OutputStateId.  Subset is "minimal
                    // representation" (only include final and states and states
                    // with nonzero ilabel on arc out of them.  Owns the
                    // pointers in its keys.
