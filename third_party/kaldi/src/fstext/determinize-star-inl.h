...
class StringRepository {
 ...
 public:
  ...
  class VectorKey {  // Hash function object.
   public:
    size_t operator()(const Key& value) const noexcept { return value.second; }
  };
 ...
 private:
  ...
  static constexpr StringId string_start =
    (StringId)0;  // This must not change.  It's assumed.
  static constexpr StringId string_end =
    (std::numeric_limits<StringId>::max() / 2) -
    1;  // all hash values must be <= this.
  static constexpr StringId no_symbol =
    (std::numeric_limits<StringId>::max() / 2);  // reserved for empty sequence.
  static constexpr StringId single_symbol_start =
    (std::numeric_limits<StringId>::max() / 2) + 1;
  static constexpr StringId single_symbol_range =
...
class DeterminizerStar {
 ...
 private:
  ...
  static size_t hash(
    const std::vector<Element>* subset) {  // hashes only the state and string.
    assert(subset);
    size_t hash = 0, factor = 1;
  ...
  class SubsetKey {
   public:
    size_t operator()(
      const Key& subset) const noexcept {  // hashes only the state and string.
  ...
  class EpsilonClosure {
   public:
    ...
    bool FstSorted() const noexcept { return sorted_; }
   ...
   private:
    ...
    // the input set
    void ExpandOneElement(const Element& elem, const Weight& unprocessed_weight,
  ...
  void ProcessTransitions(OutputStateId state) {
    all_elems_.clear();

    {
      // Push back into "all_elems", elements corresponding to all
      // non-epsilon-input transitions out of all states in "closed_subset".
      for (const auto& elem : closed_subset_) {
        for (ArcIterator<Fst<Arc>> aiter(*ifst_, elem.state); !aiter.Done();
          ...
          if (arc.ilabel !=
            ...
            if (arc.olabel == 0) {
              // output epsilon -- this is simple case so handle separately for
              // efficiency
              element.string = elem.string;
            } else {
              seq_.clear();
              repository_.SeqOfId(elem.string, &seq_);
    ...
    if (!epsilon_closure_.FstSorted() || closed_subset_.size() > 1) {
      std::sort(all_elems_.begin(), all_elems_.end(),
                [](const auto& lhs, const auto& rhs) {
                  if (lhs.bound != rhs.bound) {
                    return lhs.bound < rhs.bound;
                  }
                  const bool lhs_max = lhs.IsMax();
                  const bool rhs_max = rhs.IsMax();
                  if (lhs_max != rhs_max) {
  ...
  OutputStateId SubsetToStateId(
    ...
    const auto [iter, is_new] = hash_.try_emplace(
      Key{&subset, hash(&subset)}, (OutputStateId)output_arcs_.size());
  ...
  void ProcessSubset(
    ...
    epsilon_closure_.GetEpsilonClosure(*pair.first, &closed_subset_);

    // Now follow non-epsilon arcs [and also process final states]
    ProcessFinal(closed_subset_, state);

    // Now handle transitions out of these states.
    ProcessTransitions(state);
  }
...
void DeterminizerStar<F>::EpsilonClosure::ExpandOneElement(
  const Element& elem, const Weight& unprocessed_weight, bool save_to_queue_2) {
  StringId str =
