...
void GetStateProperties(const Fst<Arc>& fst, typename Arc::StateId max_state,
  ...
  for (StateId s = 0; s <= max_state; s++) {
    ...
    for (ArcIterator<Fst<Arc>> aiter(fst, s); !aiter.Done(); aiter.Next()) {
      ...
      if (arc.ilabel != 0)
        s_info |= kStateIlabelsOut;
      if (arc.olabel != 0)
        s_info |= kStateOlabelsOut;
      StateId nexts = arc.nextstate;
      assert(nexts <= max_state);  // or input was invalid.
      StatePropertiesType& nexts_info = (*props)[nexts];
      if (s_info & kStateArcsOut)
        s_info |= kStateMultipleArcsOut;
      s_info |= kStateArcsOut;
      if (nexts_info & kStateArcsIn)
        nexts_info |= kStateMultipleArcsIn;
...
void Factor(const Fst<Arc>& fst, MutableFst<Arc>* ofst,
  ...
  // (kStateArcsIn|kStateArcsOut|kStateIlabelsOut), then it is in the middle of
  // a chain.  This eliminates state with multiple input or output arcs, final
  // states, and states with arcs out that have olabels [we assume these are
  // pushed to the left, so occur on the 1st arc of a chain.
  ...
  for (size_t i = 0; i < order.size(); i++) {
    ...
    if (!remove[state]) {  // Process this state...
      ...
      if (new_state == kNoStateId)
        new_state = ofst->AddState();
      for (ArcIterator<Fst<Arc>> aiter(fst, state); !aiter.Done();
        ...
        while (remove[arc.nextstate]) {
          ...
          if (nextarc.ilabel != 0)
            this_sym.push_back(nextarc.ilabel);
        ...
        if (new_nextstate == kNoStateId)
          new_nextstate = ofst->AddState();
...
void ExpandInputSequences(const std::vector<std::vector<I>>& sequences,
  ...
  if (sequences.size() > 0)
    assert(sequences[0].size() == 0);  // should be eps.
