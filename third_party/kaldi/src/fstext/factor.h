...
/// "factors" that are the indices of the "sequences" array, into linear
/// sequences of symbols.  There is a single start and end state (state 0), and
/// for each nonzero index i into the array "sequences", there is an arc from
/// state 0 that has output-label i, and enters a chain of states with output
/// epsilons and input labels corresponding to the remaining elements of the
/// sequences, terminating again in state 0.  This FST is output-deterministic
/// and sorted on olabel. Composing an FST on the left with the output of this
/// function, should be the same as calling "ExpandInputSequences".  Use
/// TableCompose (see table-matcher.h) for efficiency.
template<class Arc, class I>
...
class DfsOrderVisitor {
 ...
 public:
  DfsOrderVisitor(std::vector<StateId>* order) : order_(order) {
    order->clear();
  }
