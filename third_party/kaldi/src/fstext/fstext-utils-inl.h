 ...
 public:
  ...
  uint64 Properties(uint64 props) const {  // Not tested.
    ...
    uint64 props_to_remove = kAcceptor | kNotAcceptor | kIDeterministic |
                             ...
                             kNotILabelSorted;
    if (remove_epsilons)
      props_to_remove |= kEpsilons | kIEpsilons;
    if (add_epsilons)
      props_to_remove |= kNoEpsilons | kNoIEpsilons;
  ...
  ~MapInputSymbolsMapper() {
    if (owned && symbol_mapping_ != NULL)
      delete symbol_mapping_;
  }
...
inline bool IsStochasticFst(const Fst<Arc>& fst, float delta,
  ...
  bool ans = true;
  if (min_sum)
    *min_sum = Arc::Weight::One();
  if (max_sum)
    *max_sum = Arc::Weight::One();
  for (StateIterator<Fst<Arc>> siter(fst); !siter.Done(); siter.Next()) {
    StateId s = siter.Value();
    ...
    if (!ApproxEqual(Weight::One(), sum, delta))
      ans = false;
    if (first_time) {
      first_time = false;
      if (max_sum)
      ...
      if (max_sum && nl(*max_sum, sum))
        *max_sum = sum;
      if (min_sum && nl(sum, *min_sum))
        *min_sum = sum;
    }
  }
...
inline bool IsStochasticFst(const Fst<LogArc>& fst, float delta,
  ...
  bool ans = true;
  if (min_sum)
    *min_sum = LogArc::Weight::One();
  if (max_sum)
    *max_sum = LogArc::Weight::One();
  for (StateIterator<Fst<Arc>> siter(fst); !siter.Done(); siter.Next()) {
    StateId s = siter.Value();
    Weight sum = fst.Final(s);
    for (ArcIterator<Fst<Arc>> aiter(fst, s); !aiter.Done(); aiter.Next()) {
    ...
    if (first_time) {
      ...
      if (max_sum && sum.Value() < max_sum->Value())
        *max_sum = sum;
      if (min_sum && sum.Value() > min_sum->Value())
        *min_sum = sum;
    }
  }
  if (first_time) {  // just avoid NaNs if FST was empty.
...
inline bool IsStochasticFstInLog(const Fst<StdArc>& fst, float delta,
  ...
  if (min_sum)
    *min_sum = StdArc::Weight(log_min.Value());
  if (max_sum)
    *max_sum = StdArc::Weight(log_max.Value());
  return ans;
}
