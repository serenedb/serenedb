...
void PreDeterminize(MutableFst<Arc>* fst, typename Arc::Label first_new_sym,
  ...
  if (fst->Start() == kNoStateId)
    return;  // for empty FST, nothing to do.
  assert(symsOut != NULL &&
  ...
  {  // D(ii): computing the array p. ["problematic states, i.e. states with >1
    ...
    std::vector<bool> seen_vec(
      max_state + 1, false);  // rather than counting incoming transitions we
                              // just have a bool that says we saw at least one.
  ...
  {
    std::vector<StateId>
      all_seed_states;  // all "problematic" states, plus initial state (if not
                        // problematic).
    if (!p_vec[fst->Start()])
      all_seed_states.push_back(fst->Start());
    for (StateId s = 0; s <= max_state; s++)
      if (p_vec[s])
        all_seed_states.push_back(s);
    ...
    for (size_t idx = 0; idx < all_seed_states.size(); idx++) {
      ...
      pre_determinize_helpers::Closure(
        fst, &closure_s, p_vec);  // follow epsilons to non-problematic states.
    ...
    // (D)(v)(e)
    {
      for (auto miter = arc_hash.begin(); miter != arc_hash.end(); ++miter) {
        ...
        if (t != 0) {             // For t != epsilon,
          ...
          if (V_t.size() != 0) {
            pre_determinize_helpers::Closure(
              fst, &V_t, p_vec);  // follow epsilons to non-problematic states.
