...
We were able to find an iterative algorithm that does this very efficiently in
practice; it is based on the power method for finding the top eigenvalue of a
matrix. Both for the math and the implementation, we find it more convenient to
...
where $p_{ij}$ is the sum of all the probabilities of transitions between state
$i$ and state $j$. As a special case, if $j$ is the initial state, then $p_{ij}$
is the final-probability of state $i$. In our method we find the dominant
eigenvector $\mathbf{v}$ of the matrix $\mathbf{P}$, by starting from a random
positive vector and iterating with the power method: each time we let
$\mathbf{v} \leftarrow \mathbf{P} \mathbf{v}$ and then renormalize the length of
$\mathbf{v}$. It is convenient to renormalize $\mathbf{v}$ so that $v_I$ is 1,
...
we modify the iteration to $\mathbf{v} \leftarrow \mathbf{P} \mathbf{v} + 0.1
\mathbf{v}$.}. This generally converges within several tens of iterations. At
the end we have a vector $\mathbf{v}$ with $v_I = 1$, and a scalar $\lambda >
0$, such that
\begin{equation}
...
class PushSpecialClass {
 ...
 public:
  ...
  PushSpecialClass(VectorFst<StdArc>* fst, float delta) : fst_(fst) {
    ...
    for (StateId s = 0; s < num_states_; s++) {
      for (ArcIterator<VectorFst<StdArc>> aiter(*fst, s); !aiter.Done();
           aiter.Next()) {
 ...
 private:
  double TestAccuracy() {  // returns the error (the difference
    ...
    for (StateId s = 0; s < num_states_; s++) {
      double sum = 0.0;
      for (ArcIterator<VectorFst<StdArc>> aiter(*fst_, s); !aiter.Done();
           aiter.Next()) {
        const Arc& arc = aiter.Value();
        StateId t = arc.nextstate;
      ...
      sum +=
        kaldi::Exp(-(fst_->Final(s).Value())) * occ_[initial_state_] / occ_[s];
      if (s == 0) {
        min_sum = sum;
        max_sum = sum;
      } else {
  ...
  void Iterate(float delta) {
    ...
    for (iter = 0; iter < max_iter; iter++) {
      ...
      double sumsq = 0.0;
      for (int i = 0; i < num_states_; i++)
        sumsq += new_occ[i] * new_occ[i];
      lambda_ = std::sqrt(sumsq);
      double inv_lambda = 1.0 / lambda_;
      for (int i = 0; i < num_states_; i++)
        occ_[i] = new_occ[i] * inv_lambda;
  ...
  // Modifies the FST weights and the final-prob to take account of these
  // potentials.
  void ModifyFst() {
    ...
    for (StateId s = 0; s < num_states_; s++) {
      for (MutableArcIterator<VectorFst<StdArc>> aiter(fst_, s); !aiter.Done();
           aiter.Next()) {
        Arc arc = aiter.Value();
        StateId t = arc.nextstate;
 ...
 private:
  ...
  std::vector<double>
    occ_;          // the top eigenvector of (matrix of weights) transposed.
  double lambda_;  // our current estimate of the top eigenvalue.

  std::vector<std::vector<std::pair<StateId, double>>>
    pred_;  // List of transitions
...
/*
  ...
  version of this method (without the 0.1 * old_occ) we had a problem with the
following FST. 0    2    3    3    0 1    3    1    4    0.5 2    1    0    0
0.5 3    0.25
